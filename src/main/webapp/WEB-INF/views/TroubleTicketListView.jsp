   ...
   <!--  end of general head page -->
			...
			<div class="col-md-4" style="text-align: right;">
		 		<div class="btn-group pull-right">
		 			<div class="glyph">
						...
						<button type="button" id="searchTroubleTicket" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
	...
	<div class="modal-body">
	 <ul class="nav nav-tabs" id="myTab" role="tablist" style="background-color: #00757C;">
  		...
  		<li class="nav-item">
   			 <a class="nav-link " id="troubleinfo-tab" style="color: gold;" data-toggle="tab" href="#troubleinfo" role="tab" aria-controls="troubleinfo" aria-selected="false">Trouble Ticket</a>
  		</li>
...
<div class="container-fluid">
  ...
  <div class="tab-pane" id="troubleinfo" role="tabpanel" aria-labelledby="troubleinfo-tab">
<p></p>
	<form>
	<div class="container-fluid">
	<div class="row">
  			...
  			<div class="form-group">
  				<div class="input-group-prepend">
  					<span class="input-group-text" >Department </span>
   					<input type="text" id="popupdepartment"  style="width:675px; height:37px" class="ui-widget ui-widget-content ui-corner-all form-control text-input" />
  				</div>
  			</div>
  		</div>	
  		<div class="col-sm-6">
  			<div class="form-group">
  				<div class="input-group-prepend">
  					<span class="input-group-text" >Subject </span>
   					<input type="text" id="popupsubject"  style="width:675px; height:37px" class="ui-widget ui-widget-content ui-corner-all form-control text-input" />
  				</div>
  			</div>
  		</div>	
  		<div class="col-sm-6">
  			<div class="form-group">
  				<div class="input-group-prepend">
  					<span class="input-group-text" >Priority </span>
   					<input type="text" id="popuppriority"  style="width:675px; height:37px" class="ui-widget ui-widget-content ui-corner-all form-control text-input" />
  				</div>
  			</div>
  		</div>	
  		<div class="col-sm-6">
	...
	<p></p>
		...
		<div class="col-12 col-sm-12 col-lg-12">	
		      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #007b7c; margin-top:0px">
		             <li class="nav-item"><class="nav-link inactive" id="custom-tabs-one-home-tab"style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">TROUBLE TICKET LIST</li>
	 <li class="nav-item ml-auto">
								...
								<button type="button" id="saveButton"
									onclick='window.location.href = "${pageContext.request.contextPath}/TroubleTicketFormView?type=addNew"'
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="troubleticketgridtable"class="table-responsive almgrid-table-div">
	...
	$(document).ready(function () {
		...
		$.each(arListData, function (i, value) {
			ReportArrayGlobal.push({
				ID: value[0],
				tkID: value[0], // This is mandatory for value of checkbox, the rest are drawn in table
				tkDepartment: value[1],
				tkSubject: value[2],
				TklastModifiedDate: value[5],
				TkCreationDate: value[6],
				tkPriority: value[13]			
			});
		...
		$(".almgrid-table").on("click", ".almgrid-link", function (e) {
			...
			var param = "${pageContext.request.contextPath}/TroubleTicketFormView?tkID=" + param1+"&NavAction=2";
			window.location.href = param;
			e.preventDefault();

			console.log('date is: ;'+ param1);
		});
		...
		$("#deleteButton").click(function () {
			...
			$.ajax({
				...
				success: function (data) {
					location.reload();
					location.replace("${pageContext.request.contextPath}/TroubleTicketListView");
		...
		$("#Fview").click(function () {
			...
			//console.log("tkID " + param1);
			if (obj[0] == null){
				var param = "${pageContext.request.contextPath}/TroubleTicketFormView?type=addNew&NavAction=2"
				}else{
			console.log("tkID " + obj[0].tkID);
			
			console.log("dateis " + obj[0].lastModifiedDate);
			var param1 = obj[0].tkID;
			console.log("tkID " + param1);
			var param = "${pageContext.request.contextPath}/TroubleTicketFormView?tkID=" + param1+"&NavAction=2";}
			window.location.href = param;
...
download_link.download = "TroubleTicket_List_Export";
...
exportArrayGrid.push(["Ticket ID", "Department", "Subject","Created Date","Last Modified Date","Priority"]);
...
//location.reload();
	...
	$("#searchTroubleTicket").click(function() {
 		$("#poModal").modal("show");
 		document.getElementById('searchTroubleTicket').className = 'btn btn-danger';
     	document.getElementById('Lview').className = 'btn btn-light';
     	$("#Lview").css('background', '#FFFFFF');	
 		$("#searchTroubleTicket").css('background', '#da6815');	
  	...
  	$("#popUpSubmit").click(function() {
  		$("#poModal").modal("show");
 		$("#gridTable").remove();
 		$("#troubleticketgridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
 				+'<tr class="header">'
 				+'<th class="table-select-all"></th>'
 				+'<th>Ticket ID<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
 				+'<th>Department<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
 				+'<th>Subject<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
 				+'<th>Created Date<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 				...
 				+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
 				+'<th>Priority<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 		...
 		$.ajax({
			...
			url : "${pageContext.request.contextPath}/FilteredTroubleTicketListView",
			
			data : {
				"startDate":$("#popupstartdate").val(),
				"endDate":$("#popupenddate").val(),
				"department":$("#popupdepartment").val(),
				"subject":$("#popupsubject").val(),
				"priority":$("#popuppriority").val(),
		...
		success : function(data) {
				...
				if(data!=null){
					...
					document.getElementById("alertMsgDiv").style.display = "none";
             		...
             		$(".almgrid-table").on("click", ".almgrid-link", function (e) {
             			var param1 = $(this).attr('id');
             			var param = "${pageContext.request.contextPath}/TroubleTicketFormView?ID=" + param1 +"&NavAction=2";
             			window.location.href = param;
             			e.preventDefault();
             		});
				}	
 ...
 // Resize and drag the popup
	...
	//{$("#popupstartdate").val('');$("#popupenddate").val('');$("#popupdepartment").val('');$("#popupsubject").val('');$("#popuppriority").val('');$("#popupstatus").val('');}
