 ...
 <!--  end of general head page -->
		 		...
		 		<div class="btn-group pull-right">
		 			...
		 			<button class="btn btn-secondary" type="button" id="export">Export</button>
			        	...
			        	<button type="button" id="Lview"  class="btn btn-danger" data-toggle="tooltip"
							...
							<button type="button" id="searchCapital" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
	...
	<div class="modal-body">
	 <ul class="nav nav-tabs" id="myTab" role="tablist" style="background-color: #00757C;">
  		...
  		<li class="nav-item">
   			 <a class="nav-link " id="supplierinfo-tab" style="color: gold;" data-toggle="tab" href="#supplierinfo" role="tab" aria-controls="supplierinfo" aria-selected="false">Supplier</a>
  		</li>
		...
		<div class="col-sm-6">
			<div class="form-group">
				<div class="input-group-prepend">
					...
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popupsupplierid" style="width:675px; height:37px"  />					
				</div>
			</div>
		</div>
		<div class="col-sm-6">
			<div class="form-group">
				<div class="input-group-prepend">
					...
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popupsuppliername" style="width:675px; height:37px"  />					
				</div>
			</div>
		</div>
	...
	<p></p>
		...
		<div class="col-12 col-sm-12 col-lg-12">	
		      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #007b7c; margin-top: 0px;">
		             <li class="nav-item"><class="nav-link inactive" id="custom-tabs-one-home-tab"  style="color: gold;center;line-height:2.75em; padding-left:5px;">CAPITAL IN PROGRESS LIST</li>
		            ...
		            <li class="nav-item ml-auto">
						...
						<button type="button" id="saveButton" onclick='window.location.href = "${pageContext.request.contextPath}/CapitalInProgressFormView?type=addNew"'
						class="btn btn-primary BtnActive">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="capitalgridtable" class="table-responsive almgrid-table-div">
...
$(document).ready(function() {
	...
	$(".almgrid-table").on("click", ".almgrid-link", function (e) {
		...
		var param = "${pageContext.request.contextPath}/CapitalInProgressFormView?cipID=" + param1+"&NavAction=2";
	...
	$("#deleteButton").click(  function() {
		...
		$("#gridTable").find(".table-select-checkbox:checked").each(function () {
		...
		$.ajax({
			type : "GET",
			url : "${pageContext.request.contextPath}/CapitalInProgressListViewDelete",
			dataType : "json",
			data : {
				"cipID" : deleteArray
			},
	 ...
	 $('#Fview').click(function(e) {
		 ...
		 if (ReportArrayGlobal.length == 0) {
			...
			var param = "${pageContext.request.contextPath}/CapitalInProgressFormView?type=addNew";
			window.location.href = param;
			e.preventDefault();
		 }
	       ...
	       var param = "${pageContext.request.contextPath}/CapitalInProgressFormView?cipID=" + firstCell+"&NavAction=2";
		   window.location.href = param;
		   e.preventDefault();
		  }
...
download_link.download = "CapitalInProgress_List_Export";
...
exportArrayGrid.push(["CIP ID", "Item Code", "Item Name", "Last Modified Date", "Supplier ID","Supplier Name","PO ID"]);
...
//location.reload();
	 ...
	 $("#searchCapital").click(function() {
			$("#poModal").modal("show");
			document.getElementById('searchCapital').className = 'btn btn-danger';
	 	document.getElementById('Lview').className = 'btn btn-light';
	 	$("#Lview").css('background', '#FFFFFF');	
			$("#searchCapital").css('background', '#da6815');	
		...
		$("#popUpSubmit").click(function() {
			$("#poModal").modal("show");
			$("#gridTable").remove();
			$("#capitalgridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
					+'<tr class="header">'
					+'<th class="table-select-all"></th>'
					+'<th>CIP ID<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
					...
					+'<th>Supplier ID<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
					+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
					+'<th>Supplier Name<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
					+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
			...
			url : "${pageContext.request.contextPath}/FilteredCapitalListView",
			...
			data : {
				...
				"suppliername":$("#popupsuppliername").val(),
		},
		...
		success : function(data) {
				...
				if(data!=null){
					...
					selectCheckbox: true
	         		...
	         		$(".almgrid-table").on("click", ".almgrid-link", function (e) {
	         			var param1 = $(this).attr('id');
	         			var param = "${pageContext.request.contextPath}/CapitalInProgressFormView??ID=" + param1 +"&NavAction=2";
	         			window.location.href = param;
	         			e.preventDefault();
	         		});
				}	
	...
	//{$("#popupstartdate").val('');	$("#popupenddate").val('');$("#popupitemcode").val('');$("#popupitemname").val('');
		//$("#popupitemmodel").val('');$("#popupsupplierid").val('');	$("#popupsuppliername").val('');}
});
