...
<head>
<meta charset="utf-8">
    <title>Module Field Form View</title>
...
<div class="container-fluid">
			...
			<div class="col-md-3">
                   <div class="form-group">
						<div class="input-group-prepend">
							<span class="input-group-text" style="color:green">Module Field</span>
							<input type="text" id="moduleFieldId" readonly  value="${id}" class="form-control text-input"  />						
						</div>
		...
		<div class="form-group">
			<div class="input-group-prepend">
				...
				<input type="text" id="screenName"  value="${screenName}" style="width:600px;" class="form-control text-input" readonly/>
			</div>
		</div>
	</div>
		...
		<div class="col-md-3 nextprvItems">
			<div class="form-group">
				<div class="input-group-prepend">
					<span class="input-group-text">Other Module Field</span>
...
<div class="col-12 col-sm-12 col-lg-12">	
      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #00757c; margin-top: 0px;">
            ...
            <li class="nav-item ml-auto">
    	        ...
    	        <button type="button" 
				onclick='window.location.href = "${pageContext.request.contextPath}/ModuleFieldFormView?type=addNew"'
...
<div class="row">
		...
		<div class="form-group">
			<div class="input-group-prepend">
				...
				<input type="text" id="screenTable"  value="${screenTable}" style="width:600px;" class="form-control text-input" readonly/>
			</div>
		</div>
	</div>
    ...
    <div class="form-group">
        <div class="input-group-prepend">
            ...
            <!-- Default Text Input -->
            <select id="fieldNameDropdown" class="form-control" style="width:600px; display:none;"></select>
            <input type="text" id="fieldName" value="${fieldName}" style="width:600px;" class="form-control text-input" readonly/>
        </div>
    </div>
</div>
	...
	<div class="col-md-4">
		<div class="form-group">
			<div class="input-group-prepend">
				<span class="input-group-text">Field Index</span>
				<input type="number" id="fieldIndex"  value="${fieldIndex}" style="width:600px;" class="form-control text-input" />
			</div>
		</div>
	</div>
		...
		<div class="form-group">
			<div class="input-group-prepend">
				<span class="input-group-text">Field Level</span>
				<input type="number" id="fieldLevel"  value="${fieldLevel}" style="width:600px;" class="form-control text-input" />
			</div>
		</div>
	</div>
		...
		<div class="form-group">
			<div class="input-group-prepend">
				...
				<input type="text" id="fieldType"  value="${fieldType}" style="width:600px;" class="form-control text-input" />
			</div>
		</div>
	</div>
         <div class="col-md-4">
...
function getFieldValuesArray() {
 ...
 document.addEventListener("DOMContentLoaded", function() {
     ...
     // Get the missing columns from Thymeleaf context
     var missingColumns = ${missingColumns};
if(missingColumns.length<=0 && '${status}' == "AddNew"){

	alert("All this screen fields are initialized on the Module Field!");
	 var param = "${pageContext.request.contextPath}/ModuleFieldListView";
      location.replace(param);
}
     // Get the elements
     ...
     if (missingColumns.length > 0) {
         ...
         fieldNameDropdown.style.display = "block";
         
         // Populate the dropdown with missing columns
         missingColumns.forEach(function(column) {
             var option = document.createElement("option");
             option.value = column;
             option.textContent = column;
             fieldNameDropdown.appendChild(option);
         });
     } else {
         // No missing columns, keep the text input visible
         fieldNameInput.style.display = "block";
         fieldNameDropdown.style.display = "none";
     }
 });

 function getFieldNameValue() {
	 var fieldNameInput = document.getElementById("fieldName");
	    var fieldNameDropdown = document.getElementById("fieldNameDropdown");
		 
     if (fieldNameInput.style.display === "none") {
         // Dropdown is visible, return its value
         return fieldNameDropdown.value;
     } else {
         // Text input is visible, return its value
         return fieldNameInput.value;
     }
 }
	...
	$("#saveButton").click(function() {
		    ...
		    var fieldNameValue = getFieldNameValue();
		    ...
		    $.ajax({
 		        ...
 		        success: function(data) {
 		        	$("#moduleFieldId").val(data.ID);
 				
 		            console.log("The returned data is " + data.BassamTest);
 		            var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + $("#moduleFieldId").val() + "&NavAction=2";
 		            location.replace(param);
 		        },
            ...
            if (SelectedIndex === moduleScreenCount) {
                ...
                if (!$("#btnNexta").hasClass("disabled")) {
                    $("#btnNext").click(function() {
                        var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + $("#moduleFieldId").val() + "&NavAction=1";
                        window.location.href = param;
                    });
                }
                if (!$("#btnLst").hasClass("disabled")) {
                    $("#btnLst").click(function() {
                        var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + $("#moduleFieldId").val() + "&NavAction=4";
                        window.location.href = param;
                    });
                }
            }
            ...
            if (SelectedIndex === 1) { // First record in database
                ...
                if (!$("#btnPrva").hasClass("disabled")) {
                    $("#btnPrv").click(function() {
                        var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + $("#moduleFieldId").val() + "&NavAction=0";
                        window.location.href = param;
                    });
                }
                $("#btnFrst").click(function() {
                    if (!$("#btnFrst").hasClass("disabled")) {
                        var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + $("#moduleFieldId").val() + "&NavAction=3";
                        window.location.href = param;
                    }
                });
            }
    ...
    $("#selectnav").autocomplete({
        source: function(request, response) {
            $.ajax({
                ...
                url: '${pageContext.request.contextPath}/GetAllModuleField',
        ...
        select: function(event, ui) {
            ...
            var param = "${pageContext.request.contextPath}/ModuleFieldFormView?ID=" + ui.item[0] + "&NavAction=2";
            window.location.href = param;
            return false;
        }
