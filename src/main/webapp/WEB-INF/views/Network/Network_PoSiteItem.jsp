...
*/
	...
	CreateTree_PoStItem(listPO,map);
		...
		map.setZoom(6);
		var str="<ul><li id='initial_ul' class='Initial'><input type='checkbox' id='POs' class='AllPOs' style='margin-left: 15px' onclick='AllSitesCheckFilter()'></input><span class='folder' id='test1'><i class='fa fa-folder' style='color: #08526D'></i></span><span class='TreeSpan' style='width:395px'> PO </span></li></ul>";
...
function CreateTree_PoStItem(listPO,map){
	...
	str="<li id='"+listPO[n]+"' class='PO' style='display:none;width:100px;'><span class='folder' onclick='PoStItemCore("+listPO[n]+")' ><i class='fa fa-folder' style='color: #08526D'></i></span><span class='TreeSpan' style='width:395px'><i class='fa fa-file-invoice-dollar fa-2x'></i> "+listPO[n]+"</span>";
	str+= "<ul><li id='" +listPO[n]+"_f' class='SiteFolder parent_li' style='display:none; margin-left:-40px'><input type='checkbox' id='" +listPO[n]+"_Site' class='AllSites' style='margin-left: 15px' onclick='showMarkersAllSitesOneNt("+listPO[n]+")'></input><span class='folder'> <i class='fa fa-folder' style='color: #08526D'></i></span><span class='TreeSpan' style='width:395px'> Sites </span></li></ul></li></ul>";	
...
function PoStItemCore(id)
{ 
	//console.log(".PoStItemCore.");
	//console.log(".id...."+id);
	//var POSCreated=[];
	var selectedPo=id.id;
	//console.log("selectedItem.....",selectedPo);
	//tree_prop_general();
	...
	//console.log("POChildrenLength......."+POChildrenLength);
		...
		$.ajax({
			...
			if (data != null) {
				...
				for (n = 0; n < listSites.length; n++) {
					//console.log("lst sites[n] >>>> "+listSites[n]);
					str="<li id='"+listSites[n][2]+"_"+listSites[n][3]+"' class='SingleSite' style='display:none; margin-left:-30px'><input type='checkbox' id='" + listSites[n][2] + "_" + listSites[n][3] +"_SingleSite' class='SingleSite' style='margin-left: 25px' onclick='showMarkerSingleSite("+listSites[n][2]+"_"+listSites[n][3]+"_SingleSite)'></input><span class='folder' onclick='PoStItemCore2("+listSites[n][2]+"_"+listSites[n][3]+")'><i class='fa fa-folder' style='color: #08526D'></i></span><span class='TreeSpan' style='width:395px' onclick='PanTreeSites("+listSites[n][2]+"_"+listSites[n][3]+")'><img src='"+getContext()+"/resources/NetworkImages/site.png'> "+listSites[n][1]+"</span>";					
					str+= "<ul><li id='" +listSites[n][2]+"_"+listSites[n][3]+"_f' class='ItemFolder parent_li' style='display:none; margin-left:20px'><span class='folder' onclick='PoStItemCore2("+listSites[n][2]+"_"+listSites[n][3]+")'><i class='fa fa-folder' style='color: #08526D'></i></span><span class='TreeSpan' style='width:395px'>Item </span></span></li></ul></li>";						   									
					const div = document.createElement('ul');
					div.innerHTML = str;
					dFrag.appendChild(div);                                                      
				}
				document.getElementById(selectedPo+"_f").appendChild(dFrag);	
				//console.log("document.getElementById.....", document.getElementById(selectedPo+"_f"));
				tree_prop_selection("#" +selectedPo+"_f .SingleSite .TreeSpan");
		        Tree_PropagationAppendedNodes(selectedPo+"_f .SingleSite");
...
function showMarkerSingleSite(id) {
	 ...
	 var itemId= id.split('_').slice(3).join('_').split("_SingleSite")[0];
	    ...
	    if ($("#" + id).is(":checked")) {
	     ...
	     if (allChecked) {
	    	document.getElementById(itemId+'_Site').checked = true;
		 }
	      markersSites[selectedSiteId].setMap(map);
	      markerClusterSites.addMarker(markersSites[selectedSiteId]);
	      markerClusterSites.repaint();
	    }else{
		      document.getElementById(itemId+'_Site').checked = false;		
...
function PoStItemCore2(id)
{
  //console.log(".PoStItemCore2.");
	var selectedId= id.id;
	var selectedItem = selectedId.split("_")[0] +"_"+ selectedId.split("_")[1] +"_"+ selectedId.split("_")[2];
	//console.log("selectedItem....."+selectedItem);
	var selectedPO = selectedId.split("_")[3]+"_"+ selectedId.split("_")[4] +"_"+ selectedId.split("_")[5];
	//console.log("selectedPO....."+selectedPO);	
	/*
	tree_prop_general();
	...
	*/
			//Site_Boq(selectedItem);		
				...
				//console.log("siteChildren....."+siteChildren);
				if(siteChildren == 0){
							...
							if (data != null) {
								...
								if(siteChildren<listItem.length){
									for (k = 0; k <listItem.length; k++) {										
										...
										tree_prop_selection("#" +selectedItem+ "_"+ selectedPO+ "_f .Items .TreeSpan");
								   }			
								}
							}
							data=null;
