	...
	<p></p>
		...
		<div class="col-12 col-sm-12 col-lg-12">	
		      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #007b7c; margin-top: 0px;">
		             <li class="nav-item"><class="nav-link inactive" id="custom-tabs-one-home-tab"  style="color: gold;center;line-height:2.75em; padding-left:5px;">SUPPLIER LIST</li>
	                 <!--  
		              <li class="nav-item ml-auto">
						<button type="button" id="getseqButton"
						class="btn btn-primary BtnActive">
						<i class="fa fa-trash"></i> GetseqValue
						</button> 
						
		             <li class="nav-item ml-auto">
						<button type="button" id="seqButton"
						class="btn btn-primary BtnActive">
						<i class="fa fa-trash"></i> insert_using_seq
						</button> -->
		            ...
		            <li class="nav-item ml-auto">
						...
						<button type="button" id="saveButton" onclick='window.location.href = "${pageContext.request.contextPath}/SupplierFormView"'
						class="btn btn-primary BtnActive">
...
$(document).ready(function() {
    ...
    "aoColumns":[
        ...
        
        {
           ...
           // how to pass parameter to ItemFormView
            	return    '<a href="'+ '${pageContext.request.contextPath}/SupplierFormView?supplierID='+full[1] +'&supplierName='+full[2] +'">' + url + '</a>';
				}
	...
	$('#multiselect').on('click', function() {
 	...
 	$("#deleteButton").click(  function() {
		...
		$.ajax({
			type : "GET",
			url : "${pageContext.request.contextPath}/SupplierListViewDelete",
			dataType : "json",
			data : {
				"supplierID" : slct
			},
		...
		console.log('slct is: ' + slct);
 		 		 ...
 		 		 $("#seqButton").click(  function() {
	 					console.log('seq now');
	 		           $.ajax({
					        url: "${pageContext.request.contextPath}/ggg",
					        type: "GET",
					        dataType: "html",               
					        cache: false,
					        success: function(data){             
	 			 ...
	 			 $("#getseqButton").click(  function() {
	 					console.log('seq now');
	 		           $.ajax({
					        url: "${pageContext.request.contextPath}/getseqvalue",
					        type: "GET",
					        dataType: "html",               
					        cache: false,
					        success: function(data){             
