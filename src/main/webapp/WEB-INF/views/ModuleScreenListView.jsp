...
<head>
<meta charset="utf-8">
    <title>Module Screen List View</title>
			...
			<div class="row second">
					...
					<div class="col-md-3" style="text-align: right;">
						<div class="btn-group pull-right">
							<div class="glyph">
							<!--  -->
						   <button style="display:none;" class="btn btn-secondary" type="button" id="export">Export</button>
    	                  ...
    	                  <!--  -->
								...
								<button type="button" id="searchPurchase" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
						...
						<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
							...
							<li class="nav-item">
								<class="nav-link inactive" id="custom-tabs-one-home-tab"
									style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">MODULE SCREEN LIST
...
<!--------------------------------------------------  END card-header ------------------------------------------>
								...
								<div id="ModuleScreenGridTable" class="table-responsive almgrid-table-div">
									<table id="ModuleScreenTable" class="table table-striped table-bordered almgrid-table">
										<thead>
											<tr class="header">
...
var PurchaseOrderData = ${ ListGridTable };



	
	var almgrid = new Almgrid({
		tableId: "ModuleScreenTable",
		dataArray: PurchaseOrderData,
		columnLinkNb: [1],
		selectCheckbox: true
	});
...
$( "#Fview" ).click(function() {	
	var id =  $('#ModuleScreenTable tbody tr:eq(0) td:eq(1)').text();
	
	location.href="${pageContext.request.contextPath}/ModuleScreenFormView?ID="+id+"&NavAction=2";
});  

////////////////////////////////////////for link
	$(".almgrid-table").on("click", ".almgrid-link", function (e) {
		var param1 = $(this).attr('id');
		var param = "${pageContext.request.contextPath}/ModuleScreenFormView?ID=" + param1 +"&NavAction=2";
...
$("#deleteButton").click(function () {
	...
	$("#ModuleScreenTable").find(".table-select-checkbox").each(function () {
		if ($(this).is(":checked")) {
	...
	$.ajax({
		...
		type: "GET",
		url: "${pageContext.request.contextPath}/DeleteModuleScreenListView",
		dataType: "json",
		data: {
			"ID": deleteArray
		},
...
//method to export the data into excel sheet
	function exportGrid() {
		...
		download_link.download = "PRQ_OrderList_Export";
...
//////////////////////////////////////////////////////////////////////////fill data in listview
	//filling the grid with the needed data
 	function fillGrid(filledGrid){
 		...
 		exportArrayGrid.push(["PO ID","LAST_MODIFICATION_DATE","SUPPLIER", "TOTAL AMOUNT","TOTAL QTY","WAREHOUSE", "SITE_ID","WAREHOUSE_NAME"]);
 	...
 	$("#export").click(function() {
  	...
  	$("#searchPurchase").click(function() {
 		$("#poModal").modal("show");
 		document.getElementById('searchPurchase').className = 'btn btn-danger';
  	...
  	$("#popUpSubmit").click(function() {
  		$("#poModal").modal("show");
 		$("#ModuleScreenTable").remove();
 		$("#ModuleScreenGridTable").append('<table id="ModuleScreenTable" class="table table-striped table-bordered almgrid-table"><thead>'
 				+'<tr class="header">'
 		...
 		$.ajax({
			...
			url : "${pageContext.request.contextPath}/FilteredPurchaseOrderListView",
			
			data : {
				"startDatee":$("#popupstartdate").val(),
				"endDatee":$("#popupenddate").val(),
				"Supplierr":$("#popupSupplier").val(),
				"warehousee":$("#popupwarehouse").val(),
		},
