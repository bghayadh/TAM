						...
						<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
							...
							<li class="nav-item">
								<class="nav-link inactive" id="custom-tabs-one-home-tab"
									style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">PURCHASE REQUEST LIST
...
<!--------------------------------------------------  END card-header ------------------------------------------>
								...
								<div id="purchaseGridTable" class="table-responsive almgrid-table-div">
									<table id="PurchaseTable" class="table table-striped table-bordered almgrid-table">
										<thead>
											<tr class="header">
...
var PurchaseListData = ${ ListGridTable };
...
$(document).ready(function () {
	...
	if (writeForm == 1 || readForm == 1)
		colLink = [1];
	
	var almgrid = new Almgrid({tableId: "PurchaseTable",
		dataArray: PurchaseListData,  
		columnLinkNb: colLink,
		selectCheckbox: true});
	

//////////////////////////////////////////////////////////////////////////form button
$( "#Fview" ).click(function() {	
	var id =  $('#PurchaseTable tbody tr:eq(0) td:eq(1)').text();
	
	location.href="${pageContext.request.contextPath}/PurchaseReqFormView?ID="+id+"&NavAction=2";
});  

////////////////////////////////////////for link
	$(".almgrid-table").on("click", ".almgrid-link", function (e) {
		var param1 = $(this).attr('id');
		var param = "${pageContext.request.contextPath}/PurchaseReqFormView?ID=" + param1 +"&NavAction=2";
...
$("#deleteButton").click(function () {
	...
	$("#PurchaseTable").find(".table-select-checkbox").each(function () {
		if ($(this).is(":checked")) {
...
//method to export the data into excel sheet
	function exportGrid() {
		...
		download_link.download = "PRQ_List_Export";
...
//////////////////////////////////////////////////////////////////////////fill data in listview
	//filling the grid with the needed data
 	function fillGrid(filledGrid){
 		...
 		exportArrayGrid.push(["PRQ ID", "LAST_MODIFICATION_DATE", "SUPPLIER_NAME", "TOTAL AMOUNT", "TOTAL QTY","WAREHOUSE","SITE_ID", "WAREHOUSE_NAME"]);
 	...
 	$("#export").click(function() {
     	...
     	if(almgrid.filteredArray.length == 0 ){
     		fillGrid(PurchaseListData);	
     		console.log("the purchase list data is",PurchaseListData );
	     }else{
  	...
  	$("#popUpSubmit").click(function() {
 		...
 		$("#purchaseGridTable").append('<table id="PurchaseTable" class="table table-striped table-bordered almgrid-table"><thead>'
 				+'<tr class="header">'
 				+'<th class="table-select-all"></th>'
 				+'<th>Purchase Req ID <li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
 		...
 		$.ajax({
			...
			url : "${pageContext.request.contextPath}/FilteredPurchaseListView",
