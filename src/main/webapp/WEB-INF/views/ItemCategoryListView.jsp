...
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<head>
		<meta charset="utf-8">
		<title>Item Category List View</title>
 ...
 <!--  end of general head page -->
		 		...
		 		<div class="btn-group pull-right">
		 			...
		 			<button type="button" class="btn btn-light" data-toggle="tooltip"
			        	...
			        	<button type="button" id="Lview"  class="btn btn-danger" data-toggle="tooltip"
			        		...
			        		<button type="button" id="searchitemcat" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
	...
	<div class="modal-body">
	 <ul class="nav nav-tabs" id="myTab" role="tablist" style="background-color: #00757C;">
  		...
  		<li class="nav-item">
   			 <a class="nav-link " id="itemcat-tab" style="color: gold;" data-toggle="tab" href="#itemcat" role="tab" aria-controls="itemcat" aria-selected="false">Item Category</a>
  		</li>
...
<div class="tab-pane" id="itemcat" role="tabpanel" aria-labelledby="itemcat-tab">
<p></p>
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-6">
			<div class="form-group">
				<div class="input-group-prepend">
					...
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popupcatname" style="width:675px; height:37px"  />					
				</div>
			</div>
		</div>
		<div class="col-sm-6">
			<div class="form-group">
				<div class="input-group-prepend">
					<span class="input-group-text" >Parent Code</span>
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popuprootcat" style="width:675px; height:37px"  />					
				</div>
			</div>
		</div>
		<div class="col-sm-6">
			<div class="form-group">
				<div class="input-group-prepend">
					...
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popupcatcode" style="width:675px; height:37px"  />					
				</div>
			</div>
		</div>
    </div>
...
<div class="modal-footer">
	...
	<p></p>
		...
		<div class="col-12 col-sm-12 col-lg-12">	
		      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #007b7c; margin-top: 0px;">
		             <li class="nav-item"><class="nav-link inactive" id="custom-tabs-one-home-tab"  style="color: gold;center;line-height:2.75em; padding-left:5px;">ITEM CATEGORY LIST</li>
		            ...
		            <li class="nav-item ml-auto">
						...
						onclick='window.location.href = "${pageContext.request.contextPath}/ItemCategoryFormView?type=addNew"'
						class="btn btn-primary BtnActive">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="itemcatgridtable"  class="table-responsive almgrid-table-div">
	...
	<script>
	var categoryData = ${ ListGridTable };
			...
			$("#Fview").click(function(){
				...
				if(categoryData.length == 1)
					{
						location.href="${pageContext.request.contextPath}/ItemCategoryFormView";
					}
				else if (categoryData.length > 1)
					{
						location.href="${pageContext.request.contextPath}/ItemCategoryFormView?itemCategoryID="+categoryData[1][0];
					}	
...
//console.log("the array of data is ", ReportArrayGlobal);
					...
					$(".almgrid-table").on("click", ".almgrid-link", function (e) {
						...
						location.href="${pageContext.request.contextPath}/ItemCategoryFormView?itemCategoryID="+param1+"&itemcatList=itemcatList";
						e.preventDefault();
...
download_link.download = "ItemCategory_List_Export";
...
exportArrayGrid.push(["Category Name", "Category Code", "Parent","Last Modified Date"]);
...
//location.reload();
	...
	$("#searchitemcat").click(function() {
		$("#poModal").modal("show");
		document.getElementById('searchitemcat').className = 'btn btn-danger';
 	document.getElementById('Lview').className = 'btn btn-light';
 	$("#Lview").css('background', '#FFFFFF');	
		$("#searchitemcat").css('background', '#da6815');	
	...
	$("#popUpSubmit").click(function() {
		$("#poModal").modal("show");
		$("#gridTable").remove();
		$("#itemcatgridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
				+'<tr class="header">'
				+'<th class="table-select-all"></th>'
				+'<th>Category Name	<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
				+'<th>Category Code<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
				+'<th>Parent<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
		...
		url : "${pageContext.request.contextPath}/FilteredItemCategoryListView",
		
		data : {
			"startDate":$("#popupstartdate").val(),
			"endDate":$("#popupenddate").val(),
			"catname":$("#popupcatname").val(),
			"rootcat":$("#popuprootcat").val(),
			"catcode":$("#popupcatcode").val(),
	...
	success : function(data) {
			...
			console.log(data.listitemcat);
			if(data!=null){
				almgrid = new Almgrid({
				tableId: "gridTable",
				dataArray: data.listitemcat,
				columnLinkNb: [1],
				selectCheckbox: true
				});
				//console.log("the  data   is :" , data);
				
				//console.log("the filtered data  is :" , data.listitemcat);
         		...
         		$(".almgrid-table").on("click", ".almgrid-link", function (e) {
         			...
         			var param = "${pageContext.request.contextPath}/ItemCategoryFormView?itemCategoryID=" + param1 +"&NavAction=2";
