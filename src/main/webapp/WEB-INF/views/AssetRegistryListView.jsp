...
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<head>
		<meta charset="utf-8">
		<title>Asset Registry List View</title>
						...
						<div class="btn-group pull-right">
							<div class="glyph">
							  ...
							  <button class="btn btn-secondary" type="button" id="export">Export</button>
								...
								<!-- onclick='window.location.href = "${pageContext.request.contextPath}/AssetRegistryFormView?type=edit"' -->
								...
								<button type="button" id="searchAssetRegistry" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
						...
						<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
							...
							<li class="nav-item">
								<class="nav-link inactive" id="custom-tabs-one-home-tab"
									style="color: gold;center;line-height:2.75em; padding-left:5px;">ASSET REGISTRY LIST
							</li>
							...
							<li class="nav-item ml-auto">
								...
								<button type="button" id="saveButton"
									onclick='window.location.href = "${pageContext.request.contextPath}/AssetRegistryFormView?type=addNew&NavAction=2"'
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="AssetRegistrygridtable"class="table-responsive almgrid-table-div">
			...
			<script>
				...
				$(document).ready(function () {
					...
					/* $.each(arListData, function (i, value) {
						ReportArrayGlobal.push({
							...
							itemName: value[2],
							itemModel: value[3],
							itemPartNumber: value[4],
							lastModifiedDate: value[5],
							itemSerialNumber: value[6],
							itemNameRegister: value[7],
							poId: value[8],
							nodeId: value[9],
							nodeName: value[10],
							siteId: value[11]
				...
				//	console.log("Test2 " + ReportArrayGlobal);
					...
					$("#deleteButton").click(function () {
						...
						$.ajax({
							type: "GET",
							url: "${pageContext.request.contextPath}/AssetRegistryListViewDelete",
							dataType: "json",
							data: {
								"arID": deleteArray
							},
					...
					$("#Fview").click(function () {
						var id = $(".almgrid-table > tbody > tr:first").children("td:nth-child(2)").children("a").attr('id');
						
						//var param = "${pageContext.request.contextPath}/AssetRegistryFormView?arID=" + arListData[0][0]+"&NavAction=2";
						var param = "${pageContext.request.contextPath}/AssetRegistryFormView?arID=" + id+"&NavAction=2";
...
download_link.download = "AssetRegistry_List_Export";
...
exportArrayGrid.push(["AR ID", "Item Code", "Item Name", "Item Model", "Item Part No","Last Modified Date","Item Serial Number", "Item Name Register","PO ID","Node ID","Node Name","Site ID","Site Name"]);
...
//location.reload();
					...
					$(".almgrid-table").on("click", ".almgrid-link", function (e) {
						...
						var param = "${pageContext.request.contextPath}/AssetRegistryFormView?arID=" + param1+"&NavAction=2";
						window.location.href = param;
						e.preventDefault();
				  	...
				  	$("#searchAssetRegistry").click(function() {
				 		$("#poModal").modal("show");
				 		document.getElementById('searchAssetRegistry').className = 'btn btn-danger';
				     	document.getElementById('Lview').className = 'btn btn-light';
				     	$("#Lview").css('background', '#FFFFFF');	
				 		$("#searchAssetRegistry").css('background', '#da6815');	
				  	...
				  	$("#popUpSubmit").click(function() {
				  		$("#poModal").modal("show");
				 		$("#gridTable").remove();
				 		$("#AssetRegistrygridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
				 				+'<tr class="header">'
				 				+'<th class="table-select-all"></th>'
				 				+'<th>AR ID<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				 				...
				 				+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
				 				+'<th>Item Serial Number<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"><i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				 				+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
				 				+'<th>Item Name Register <li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				 				+'<ul class="dropdown-menu dropdown-menu-right filter-dropdown-ul"></ul></li></th>'
				 		...
				 		$.ajax({
							...
							url : "${pageContext.request.contextPath}/FilteredAssetRegistryListView",
						...
						dataType : "json",
						success : function(data) {
								//console.log("filtered success"+data.listAsset);
								if(data!=null){
									
									almgrid = new Almgrid({
									tableId: "gridTable",
									dataArray: data.listAsset,
									columnLinkNb: [1],
									selectCheckbox: true
									});
				             		...
				             		$(".almgrid-table").on("click", ".almgrid-link", function (e) {
				             			var param1 = $(this).attr('id');
				             			var param = "${pageContext.request.contextPath}/AssetRegistryFormView??ID=" + param1 +"&NavAction=2";
				             			window.location.href = param;
				             			e.preventDefault();
				             		});
								}	
				 ...
				 // Resize and drag the popup
					...
					//{$("#popupstartdate").val('');$("#popupenddate").val('');	$("#popupitemcode").val('');$("#popupitemname").val('');$("#popupitemmodel").val('');}

			//$(function() {
			   // Get the form fields and hidden div
			    var circleRangeCheckbox = $("#Circle");
			    var rectangleRangeCheckbox = $("#Rectangle");
			    var hiddenRowRect = $("#row_Rectangle");
