				...
				<div class="btn-group pull-right">
					<div class="glyph">
					   <button class="btn btn-secondary" type="button" id="export">Export</button>
						...
						<button type="button" id="shopSearch" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
...
<div class="modal-footer">
	...
	<button type="button" id="popUpCancel" class ="btn btn-default btn-primary BtnActive" style="color:white;position:relative;">Cancel</button>
			...
			<div class="col-12 col-sm-12 col-lg-12">
				<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
					...
					<li class="nav-item"><class ="nav-link
							inactive" id="custom-tabs-one-home-tab"
							style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">SHOPS
						LIST</li>
			...
			<div class="row">
				<div class="col-sm-12">
					<div class="almgrid-container">
						...
						<div id="ShopsTable" class="table-responsive almgrid-table-div">
							<table id="ShopsgridTable"
		...
		<script>
			var shopsListData = ${ListGridTable};

			$(document).ready(function() {
				
				var almgrid = new Almgrid({
				tableId: "ShopsgridTable",
				dataArray: shopsListData,
				columnLinkNb: [1],
				selectCheckbox: true
				});
			...
			$(".almgrid-table").on("click",".almgrid-link",function(e) {
				...
				var param = "${pageContext.request.contextPath}/ShopsFormView?shopsID="+ param1+"&NavAction=2";
			...
			$("#deleteButton").click(function() {
				...
				var deleteArray = [];
				$("#ShopsgridTable").find(".table-select-checkbox").each(function() {
		...
		$("#Fview").click(function() {
			if (shopsListData.length == 0) {
				var param = "${pageContext.request.contextPath}/ShopsFormView?type=addNew";
				window.location.href = param;
				e.preventDefault();
			 } else {
				var firstCell =  $(".almgrid-link").closest('tr').find('td:first-child input').val();
				var param = "${pageContext.request.contextPath}/ShopsFormView?shopsID=" + firstCell+"&NavAction=2";
				window.location.href = param;
				e.preventDefault();
			  }
...
download_link.download = "Shops_List_Export";
...
exportArrayGrid.push(["Owner", "Shop Name", "Address","Region","Last Modified Date"]);
...
fillGrid(shopsListData);	
console.log("the shops list data is",shopsListData );
}else{
...
//location.reload();
       	...
       	$("#shopSearch").click(function() {
     		$("#poModal").modal("show");
     		document.getElementById('shopSearch').className = 'btn btn-danger';
         	document.getElementById('Lview').className = 'btn btn-light';
         	$("#Lview").css('background', '#FFFFFF');	
     		$("#shopSearch").css('background', '#da6815');	
    	...
    	$("#popUpSubmit").click(function() {
     		...
     		$("#poModal").modal("show");
     		$("#ShopsgridTable").remove();

     		$("#ShopsTable").append('<table id="ShopsgridTable" class="table table-striped table-bordered almgrid-table"><thead>'
    				+'<tr class="header">'
    				+'<th class="table-select-all"></th>'
    				+'<th>Owner<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
    				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
    				+'<th>Shop Name	<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
    				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
     		...
     		$.ajax({
				...
				url : "${pageContext.request.contextPath}/FilteredShopsListView",
