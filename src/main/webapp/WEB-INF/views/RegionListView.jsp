						...
						<div class="btn-group pull-right">
							<div class="glyph">
							 <button class="btn btn-secondary" type="button" id="export">Export</button>
								...
								<button type="button" id="searchregion" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-6">
			...
			<div class="form-group">
				<div class="input-group-prepend">
					<span class="input-group-text">Region Code</span>
					<input type="text" class="ui-widget ui-widget-content ui-corner-all form-control text-input" id="popupRegioncode" style="width:674px; height:37px"  />
				</div>
			</div>
		</div>
	...
	<button type="button" id="popUpCancel" class ="btn btn-default btn-primary BtnActive" style="color:white;position:relative;">Cancel</button>
						...
						<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
							...
							<li class="nav-item">
								<class="nav-link inactive" id="custom-tabs-one-home-tab"
									style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">REGION
									LIST
							</li>
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="regiongridtable" class="table-responsive almgrid-table-div">
			...
			<script>
				...
				$(document).ready(function () {
					...
					$.each(regionListData, function (i, value) {
					...
					$(".almgrid-table").on("click", ".almgrid-link", function (e) {
						...
						var param = "${pageContext.request.contextPath}/RegionFormView?RegionID=" + param1+"&NavAction=2";
						...
						e.preventDefault();
						 ...
						 var param = "${pageContext.request.contextPath}/RegionFormView";
						 window.location.href = param;
						 e.preventDefault();
						 }
					     ...
					     var param = "${pageContext.request.contextPath}/RegionFormView?RegionID=" + firstCell+"&regionList=regionList";
						 window.location.href = param;
						 e.preventDefault();
						 }
...
download_link.download = "Region_List_Export";
...
exportArrayGrid.push(["Region ID", "Region Name", "Region Code","Creation Date","Last Modified Date"]);
...
fillGrid(regionListData);	
console.log("the Region list data is",regionListData );
}else{
...
$("#poModal").modal("show");
document.getElementById('searchregion').className = 'btn btn-danger';
document.getElementById('Lview').className = 'btn btn-light';
$("#Lview").css('background', '#FFFFFF');	
$("#searchregion").css('background', '#da6815');	
...
$("#popupClearFields").on("click", function (e) {
	...
	$("#popUpSubmit").click(function() {
  		$("#poModal").modal("show");
 		$("#gridTable").remove();
 		$("#regiongridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
 				+'<tr class="header">'
 				+'<th class="table-select-all"></th>'
 				+'<th>Region ID	<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
...
url : "${pageContext.request.contextPath}/FilteredRegionListView",
...
"regioncode":$("#popupRegioncode").val(),
