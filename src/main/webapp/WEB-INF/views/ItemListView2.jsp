...
function navigate(name,name2){
	...
	param2=name2;
   if( bassamData != null){
   	var size = new TextEncoder().encode(JSON.stringify(bassamData)).length
...
function setupStorage(browser){
	for(var i =0;i<bassamData.length;i++){
		navigateArray.push(bassamData[i][1]);
		}
	console.log(navigateArray);
	if(browser == setArray){
			localStorage.setItem("itemsList", JSON.stringify(navigateArray));
			var param ="${pageContext.request.contextPath}/ItemFormView?itemcode="+param1;
		...
		var size = new TextEncoder().encode(JSON.stringify(navigateArray)).length
	   	...
	   	if(megaBytes > 2){
	   		var param ="${pageContext.request.contextPath}/ItemFormView?itemcode="+param1+"&itemsList=itemsList";
			window.location.href =param;
