						...
						<div class="btn-group pull-right">
							<div class="glyph">
							 <button class="btn btn-secondary" type="button" id="export">Export</button>
								...
								<button type="button" id="searchArea" class="btn btn-light" data-toggle="tooltip" data-placement="top"
									title="Search">
						...
						<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist"
							...
							<li class="nav-item">
								<class="nav-link inactive" id="custom-tabs-one-home-tab"
									style="color: gold;text-align:center;line-height:2.75em; padding-left:5px;">AREA
									LIST
							</li>
							...
							<li class="nav-item ml-auto">
								...
								<button type="button" id="saveButton"
									onclick='window.location.href = "${pageContext.request.contextPath}/AreaFormView?type=addNew"'
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="almgrid-container">
								...
								<div id="arealistgridtable"class="table-responsive almgrid-table-div">
			...
			<script>
				...
				$(document).ready(function () {
					...
					$("#deleteButton").click(function () {
						...
						$.ajax({
							...
							error: function (error) {
								console.log("The error is " + error.responseText);
							}
						});
				    	//location.replace("${pageContext.request.contextPath}/AreaListView");
					...
					$(".almgrid-table").on("click", ".almgrid-link", function (e) {
						...
						var param = "${pageContext.request.contextPath}/AreaFormView?AreaID="+ param1+ "&areaList=areaList&NavAction=2";
					...
					$('#Fview').click(function(e) {
							...
							if (ReportArrayGlobal.length == 0) {
								...
								var param = "${pageContext.request.contextPath}/AreaFormView?AreaID="+ firstCell+ "&areaList=areaList&NavAction=2";
								window.location.href = param;
								e.preventDefault();
							}
...
download_link.download = "Area_List_Export";
...
exportArrayGrid.push(["Area ID", "Area Name", "Region Name","Creation Date","Last Modified Date"]);
...
console.log("the Area list data is",ReportArrayGlobal );
}else{
...
$("#poModal").modal("show");
document.getElementById('searchArea').className = 'btn btn-danger';
document.getElementById('Lview').className = 'btn btn-light';
$("#Lview").css('background', '#FFFFFF');	
$("#searchArea").css('background', '#da6815');	
...
$("#popupClearFields").on("click", function (e) {
	...
	$("#popUpSubmit").click(function() {
		$("#poModal").modal("show");
		$("#gridTable").remove();
		$("#arealistgridtable").append('<table id="gridTable" class="table table-striped table-bordered almgrid-table"><thead>'
				+'<tr class="header">'
				+'<th class="table-select-all"></th>'
				+'<th>Area ID<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
				+'<th>Area Name<li class="filter-dropdown dropdown"><button class="almgrid-filter" data-toggle="dropdown"> <i class="fa fa-list almgrid-filter-i" aria-hidden="true"></i></button>'
				+'<ul class="dropdown-menu filter-dropdown-ul"></ul></li></th>'
...
url : "${pageContext.request.contextPath}/FilteredAreaListView",
...
var param = "${pageContext.request.contextPath}/AreaFormView?id=" + param1+"&NavAction=2";
window.location.href = param;
e.preventDefault(); 
