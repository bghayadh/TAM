	...
	<p></p>
		...
		<div class="col-12 col-sm-12 col-lg-12">	
		      <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist" style="background-color: #007b7c; margin-top: 0px;">
		             <li class="nav-item"><class="nav-link inactive" id="custom-tabs-one-home-tab"  style="color: gold;center;line-height:2.75em; padding-left:5px;"> REGION LIST</li>
		            ...
		            <li class="nav-item ml-auto">
						...
						<button type="button" id="saveButton" onclick='window.location.href = "${pageContext.request.contextPath}/RegionFormView?type=addNew"'
						class="btn btn-primary BtnActive">
...
$(document).ready(function() {
	...
	"bProcessing": true,
    "aaData": ahmadData,// <-- your array of objects
    "aoColumns":[
    	{
        	"mData": null,
          	"bSortable": false,
	        "mRender" : function ( ahmadData, type, full ) { 
	            	return ''}    		
    	}, 
	...
	$('#multiselect').on('click', function() {
 	...
 	$("#deleteButton").click(  function() {
		...
		$.ajax({
			type : "GET",
			url : "${pageContext.request.contextPath}/RegionListViewDelete",
			dataType : "json",
			data : {
				"RegionId" : slct
			},
...
function navigate(name,name2){
	...
	param2=name2;
   if( ahmadData != null){
   	var size = new TextEncoder().encode(JSON.stringify(ahmadData)).length
...
function setupStorage(browser){
	for(var i =0;i<ahmadData.length;i++){
		navigateArray.push(ahmadData[i][1]);
		}
	console.log(navigateArray);
	if(browser == setArray){
			localStorage.setItem("regionList", JSON.stringify(navigateArray));
			var param ="${pageContext.request.contextPath}/RegionFormView?RegionID="+param1;
		...
		var size = new TextEncoder().encode(JSON.stringify(navigateArray)).length
	   	...
	   	if(megaBytes > 2){
	   		var param ="${pageContext.request.contextPath}/RegionFormView?RegionID="+param1+"&regionList=regionList";
			window.location.href =param;
