...
function drawPanelDiagram(numRowsFromDb, numColumnsFromDb, controllerID,controllerName, dbId, dbName){
	...
	const wrapper = document.createElement("div");
						...
						//	var numRowsFromDb = Number(data.numRows || 1);
							//  var numColumnsFromDb = Number(data.numColumns || 1);
							  ...
							  // Calculate total ports dynamically
							 // var totalPorts = numRowsFromDb * numColumnsFromDb;
							...
							// function to create a simple port (kept)
							function createPort(group, x, y, id) {
							  const portWidth = 20;
							  const portHeight = 15;
							  const notchWidth = 10;
							  const notchHeight = 4;
							...
							// populate ports inside the panel (kept)
							var portSpacingX = 35;
							  var portSpacingY = 35;
							  ...
							  for (let row = 0; row < numRowsFromDb; row++) {
							    for (let col = 0; col < numColumnsFromDb; col++) {
							      ...
							      createPort(panelGroup, x, y, "P" + portCount);
							      portCount++;
							    }
							  }
