...
$("#network_tree li > span").bind("mouseover","li > span",function(e) {
...
for (n = 0; n < lst.length; n++){ 
	...
	str="<li id='"+lst[n][2]+"' class='Site' style='display:none;width:100px;'><span class='folder' onclick='StNdCellCore1("+lst[n][2]+")' ><img src='"+getContext()+"/resources/NetworkImages/site.png'> "+lst[n][1]+"</span>";
str+= "<ul><li id='" +lst[n][2]+"_f' class='NodeFolder' style='display:none'><span class='folder'> <i class='fa fa-folder' style='color: #08526D'></i> Node </span></li></ul></li></ul>";
const div = document.createElement('ul');
div.innerHTML = str;
dFrag.appendChild(div);
} 
...
$("#network_tree").append(str); 
		var dFrag = document.createDocumentFragment();
for (n = 0; n < lst.length; n++){ 
str="<li id='"+lst[n][2]+"' class='Site' style='display:none;width:100px;'><span class='folder' onclick='StNdTypNdCellCore("+lst[n][2]+")'><img src='"+getContext()+"/resources/NetworkImages/site.png'> "+lst[n][1]+"</span>";
 str+= "<ul><li id='" +lst[n][2]+"_f' class='NodeTypeFolder' style='display:none'><span class='folder'> <i class='fa fa-folder' style='color: #08526D'></i> NodeType </span></li></ul></li></ul>";
const div = document.createElement('ul');
div.innerHTML = str;
dFrag.appendChild(div);
} 
...
if(!sitesNtCreated.includes(selectedItem))
 ...
 $.ajax({
	  type: "GET",
  contentType: "application/json; charset=utf-8",
  url: getContext()+'/resources/js/Network/NetworkTree.js/findSiteNdTypeNodes_Cells',
  data: {
                "selectedItem":selectedItem,							                
 },
 dataType: "json",
 success: function (data) {					        	
     if (data != null) {					            		
    	...
    	for(j=0;j<listNodesType.length;j++)	
			{												
var str= "<ul><li class='NodeType' id='" + listNodesType[j][0] +"_"+listNodesType[j][1]+"'  class='folder' style='display:none'>";								  															
str+= "<span ><i class='fa fa-cogs '></i>"+listNodesType[j][0]+"</span></li></ul>";
$("#"+selectedItem+"_f").append(str);
str="<ul><li id='" +listNodesType[j][0] +"_"+listNodesType[j][1]+"_f' class='NodeFolder parent_li' style='display:none'><span class='folder'> <i class='fa fa-folder' style='color: #08526D'></i> Node </span></li></ul>";
 ...
 success: function (data) {														        	
     if (data != null) {
		...
		var NdTypeChildrenLength=$("#" + selectedNodetType+"_"+selectedItem+"_f").find(' > ul > li').length;																	
													if(NdTypeChildrenLength<listNodes.length){
 Create_TreeNode_Cell(listNodes,"findSiteNdTypeNodes_Cells",NdTypeChildrenLength,true,true,4,"Sup",6,"null",selectedItem);					            												    
				}
     	}
 },
 error: function(result) {
...
var str="<ul><li class='Cell' id='" + listCells[k][0] + "' style='display:none'><span class='tree-span' ><i class='fas fa-vector-square fa-2x'></i>"+listCells[k][1]+"</span></li></ul>";
