	 ...
	 var start2 = performance.now();
		...
		if(lst!=null){
			...
			for(i=0;i<lst.length;i++){
				...
				create_Marker_Click2(lst[i][2],siteName,lst[i][4],lst[i][3],markersSites,markerClusterSites,arrayParam,parsingdate);
			}
		}
		var end2 = performance.now();
}
function create_Marker_Click2(Id,Name,Long,Lat,markers,marker_Cluster,arrayParam,parsingdate) {
...
//	 if(!markers[Id]){	
		...
		google.maps.event.addListener(marker, "click", function (e) {
			console.log("---clicked---9999");
			var IdSelected = this.ID;
			if(InfoArray.hasOwnProperty(IdSelected)){	
				data=InfoArray[IdSelected]
				markerClicking(marker,IdSelected,data)
						
	        }
			else {
				CreateMarkersInfo(IdSelected,arrayParam,parsingdate,marker);
		...
		google.maps.event.addListener(marker, "click", function (e) {
	     ...
	     //Supplier pages	        
	        ...
	        liElement.closest('.NodeType').find('.SingleSupplier').find('> .folder > svg').removeClass('fa fa-folder').addClass('fa-folder-open');	
	        liElement.closest('.NodeType').find('.SingleSupplier').find(' > ul > li').show();
...
function markerClicking(marker,IdSelected,data) {
...
function CreateMarkersInfo(selectedItem,arrayParam,parsingdate,marker){
	...
	if(arrayParam[0]==1){
		...
		$.ajax({
			...
			url: getContext()+'/findMarkersInfo',
			data: {
			...
			if (data != null) {
				...
				var siteName="<b style='font-size:13px;'><u>Site: </u></b>  "+siteInfo[1];
				var data;
				if (siteInfo[2].includes("WARE")) {
				    //console.log("Found 'WARE'");
				    var listNodesCount="<b style='font-size:13px;'> Number of Nodes: </b>"+siteInfo[5];
					var listCellsCount=siteInfo[6]+siteInfo[7]+siteInfo[8];
					listCellsCount="<b style='font-size:13px;'>Number of Cells: </b>"+listCellsCount;
					 data="<div>"+siteName+"</br>"+listNodesCount+"</br>"+listCellsCount+"</div>";
				 }else if (siteInfo[2].includes("NODE")) {
				   // console.log("Found 'NODE'");
				    var nodeType= "<b style='font-size:13px;'><u>Node Type: </u></b>  "+siteInfo[11];	
				    var nodeName= "<b style='font-size:13px;'><u>Node Name: </u></b>  "+siteInfo[10];	
					var listCellsCount=siteInfo[6]+siteInfo[7]+siteInfo[8];
					listCellsCount="<b style='font-size:13px;'>Number of Cells: </b>"+listCellsCount;
				...
				markerClicking(marker,selectedItem,data)
