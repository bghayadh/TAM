        ...
        success: function(data) {
            ...
            if (distributionBoardList != null) {
                ...
                for (i = 0;i < distributionBoardList.length;i++) {
                    ...
                    else if (distributionBoardList[i][8] == "metro") {
                        create_Marker_Click(distributionBoardList[i][0] + "_showDB", distributionBoardList[i][3], distributionBoardList[i][1], distributionBoardList[i][2], markersDistBoard, markerClusterMetroDistBoard, "", distributionBoardList[i][7]);
                        markerClusterMetroDistBoard.addMarker(markersDistBoard["" + distributionBoardList[i][0]]);
    ...
    if (!markers[Id]) {
        ...
        if (markerType == "DistributionBoard") {
            ...
            google.maps.event.addListener(Mapmarker, "click", function(e) {
                ...
                if (selected.closest("#initial_ul_CurrentPhysicalLayer").length) {
                    rootNode = $("#initial_ul_CurrentPhysicalLayer");
                    console.log("The selected is belonging to current physical layer");

                } else if (selected.closest("#initial_ul_Projects").length) {
                    rootNode = $("#initial_ul_Projects");
                    console.log("the selected element is belonging to the projects");
                }
                ...
                selected.parents("li").each(function() {
                    ...
                    // show direct ul children
                    li.children("ul").show();

                    // show li inside those ul
                    li.children("ul").children("li").show();


                    li.find("> .Parentfolder svg, > .folder svg")
                        .removeClass("fa-folder")
                        .addClass("fa-folder-open");

                    if (li.attr("id") == rootNode.attr("id")) {
                        return false;
                    }
                });
            ...
            google.maps.event.addListener(Mapmarker, "click", function(e) {
                console.log("Hello, this is the event for click on marker");
                console.log("the ID of clicked marker is ", this.ID);
                ...
                /*					
                                    ...
                                    console.log("parents ID of selected eq0: ", $("#"+IdSelected).parents().eq(0).attr("id"));
                                    console.log("parents html of selected eq0: ", $("#"+IdSelected).parents().eq(0).html());
                                    console.log("parents ID of selected eq1: ", $("#"+IdSelected).parents().eq(1).attr("id"));
                                    console.log("parents html of selected eq1: ", $("#"+IdSelected).parents().eq(1).html());
                                    console.log("parents ID of selected eq2: ", $("#"+IdSelected).parents().eq(2).attr("id"));
                                    console.log("parents html of selected eq2: ", $("#"+IdSelected).parents().eq(2).html());
                                    console.log("parents ID of selected eq3: ", $("#"+IdSelected).parents().eq(3).attr("id"));
                                	
                                	
                                    console.log("parents eq0 full html: ", $("#"+IdSelected).parents().eq(0).prop("outerHTML"));
                
                                    console.log("parents eq1 full html: ", $("#"+IdSelected).parents().eq(1).prop("outerHTML"));
                
                                    console.log("parents eq2 full html: ", $("#"+IdSelected).parents().eq(2).prop("outerHTML"));
                
                                    console.log("parents eq3 full html: ", $("#"+IdSelected).parents().eq(3).prop("outerHTML"));					
                                	
                                	
                                    nodeFileId = $("#"+IdSelected).parents().eq(3).attr("id").split("initial_ul_")[1];
                                    console.log("nodeFileId is " +nodeFileId);
                                    var projectInitial=$("#initial_ul_Projects").find('>ul > #'+nodeFileId);
                                    console.log("projectInitial is " +$("#initial_ul_Projects").find('>ul > #'+nodeFileId).prop("outerHTML"));
                                    var projectRel=$("#"+nodeFileId+"").find('>ul > #initial_ul_'+nodeFileId);
                                    console.log("projectRel is " +$("#"+nodeFileId+"").find('>ul > #initial_ul_'+nodeFileId).prop("outerHTML"));
                                    var childrenInitial=$("#initial_ul_"+nodeFileId+"").find(' > ul > li');
                                    var children = $("#"+markerType+"_f_"+nodeFileId+"").find(' > ul > li');
...
// Mirrors CreateFiberPath's fiberAuxiliary_Data naming logic exactly
function buildAuxPointName(row) {
    if (row.WARE_ID == "") {
        return "";
    }
    else if (row.WARE_ID != "null" && row.WARE_ID != null) {
        return row.WARE_ID + ":" + row.AUXILIARY_POINT_NAME + ":" + row.AUXILIARY_POINT_ID;
    }
    else {
        if (row.AUXILIARY_POINT_ID &&
            (row.AUXILIARY_POINT_ID.split("_")[0] == "MH"
                || row.AUXILIARY_POINT_ID.split("_")[0] == "HH"
                || row.AUXILIARY_POINT_ID.split("_")[0] == "DB")) {
            return row.AUXILIARY_POINT_ID + ":" + row.AUXILIARY_POINT_NAME;
        }
        else if (row.AUXILIARY_POINT_NAME && row.AUXILIARY_POINT_NAME.includes("Auxiliary_Point") == true) {
            return row.AUXILIARY_ID + ":" + row.AUXILIARY_POINT_NAME;
        }
        else {
            return row.AUXILIARY_POINT_NAME;
        }
    }
}
...
function buildPath(Id, path, pathArray, allPathArray, folder, strokeColor, strokeOpacity, strokeWeight, fontColor, IdNb) {
    ...
    google.maps.event.addListener(flightPath, 'rightclick', function(e) {
        ...
        //if (deleting == true) { deleteAuxPath(e); } // I beleive this need to be commented to avoid having delete icon when right click
        /*if(typeof listenerPathDelete !== 'undefined'){
            google.maps.event.removeListener(listenerPathDelete); 									
         }*/
...
function AllManholeCheckFilter() {
    $('.AllManholes').bind("change", function() {
        ...
        console.log("Welcome to allmanholecheckfilter");
...
function AllCoreCheckFilter() {
    $('.AllCore').bind("change", function() {
        markerClusterCore.clearMarkers();

        if ($(this).is(':checked')) {
            $(this).parent().find('.Core').each(function() {
                $(this).prop('checked', true);
            });
            ...
            if ($("#Core_f_CurrentPhysicalLayer > .AllCore").is(":checked")) {
...
function AllRanCheckFilter() {
    $('.AllRan').bind("change", function() {
        markerClusterRan.clearMarkers();

        if ($(this).is(':checked')) {
            $(this).parent().find('.Ran').each(function() {
                $(this).prop('checked', true);
            });
            ...
            if ($("#Ran_f_CurrentPhysicalLayer > .AllRan").is(":checked")) {
...
function TransmissionCheckFilter(Id) {
    $("#" + Id).children('input').on("change", function() {
        ...
        if ($("#Transmission_f_CurrentPhysicalLayer").find(".Transmission:checked").length == 0) {
            $("#transmissionCheckAllBoq").prop("checked", false);
...
function CoreCheckFilter(Id) {
    $("#" + Id).children('input').on("change", function() {
        var folderID = $(this).parents().eq(4).attr('id');

        var CoreID = $(this).parent().attr('id');
        ...
        if ($(this).is(':checked')) {
            ...
            markerClusterCore.removeMarker(markersCore[CoreID]);
        ...
        if ($(this).parents().eq(2).find('.Core:checked').length == $(this).parents().eq(2).find('.Core').length) {
            $(this).parents().eq(2).children('input').prop('checked', true);
        }
        else {
            $(this).parents().eq(2).children('input').prop('checked', false);
        ...
        if ($("#Core_f_CurrentPhysicalLayer").find(".Core:checked").length == 0) {
            $("#coreCheckAllBoq").prop("checked", false);
...
function RanCheckFilter(Id) {
    $("#" + Id).children('input').on("change", function() {
        var folderID = $(this).parents().eq(4).attr('id');

        var RanID = $(this).parent().attr('id');
        ...
        if ($(this).is(':checked')) {
            ...
            markerClusterRan.removeMarker(markersRan[RanID]);
        ...
        if ($(this).parents().eq(2).find('.Ran:checked').length == $(this).parents().eq(2).find('.Ran').length) {
            $(this).parents().eq(2).children('input').prop('checked', true);
        }
        else {
            $(this).parents().eq(2).children('input').prop('checked', false);
        ...
        if ($("#Ran_f_CurrentPhysicalLayer").find(".Ran:checked").length == 0) {
            $("#ranCheckAllBoq").prop("checked", false);
...
// This function to build check and uncheck event for nodes.
...
function DistributionBoardCheckFilter(Id, folder, clusterName) {
    console.log("Welcome to DistributionBoardCheckFilter");
    markersDistBoard[Id].setMap(null);
    $("#" + Id).children('input').bind("change", function() {
        console.log("In the DB, the Id is " + Id);
        var folderID = $(this).parents().eq(4).attr('id');
        ...
        if ($("#DistributionBoard_f_CurrentPhysicalLayer").find(".DistBoard:checked").length == $("#DistributionBoard_f_CurrentPhysicalLayer").find(".DistBoard").length && $("#DistributionBoard_f_CurrentPhysicalLayer").find(".DistBoard").length > 0) {
...
/*
function computeDistributionBoardState() {

    const $root = $("#network_tree");

    // -----------------------------
    // DB states per layer
    // -----------------------------
    const $backboneDBs = $root.find(".BackboneDB .DistBoard");
    const $metroDBs = $root.find(".MetroDB .DistBoard");
    const $accessDBs = $root.find(".AccessDB .DistBoard");

    console.log("backboneDBs.length is " +$backboneDBs.length);
	
    const backboneChecked =
        $backboneDBs.length > 0 &&
        $backboneDBs.length === $backboneDBs.filter(":checked").length;

    const metroChecked =
        $metroDBs.length > 0 &&
        $metroDBs.length === $metroDBs.filter(":checked").length;

    const accessChecked =
        $accessDBs.length > 0 &&
        $accessDBs.length === $accessDBs.filter(":checked").length;

    // -----------------------------
    // Layer UI checkboxes
    // -----------------------------
    $("#backboneLayerCheckbox").prop("checked", backboneChecked);
    $("#metroLayerCheckbox").prop("checked", metroChecked);
    $("#accessLayerCheckbox").prop("checked", accessChecked);

    // -----------------------------
    // Root Distribution Board folder
    // -----------------------------
    const allDBs = $root.find(".DistBoard");

    const rootChecked =
        allDBs.length > 0 &&
        allDBs.length === allDBs.filter(":checked").length;

    $("#DistributionBoard_f_CurrentPhysicalLayer > .AllDistBoards")
        .prop("checked", rootChecked);

    // -----------------------------
    // BOQ global checkbox
    // -----------------------------
    $("#distBoardCheckAllBoq")
        .prop("checked", allDBs.filter(":checked").length > 0);
}
...
function AllDistributionBoardCheckFilter(Id,clssName) {
    ...
    $("#"+Id).children('input').bind("change",function() {
        ...
        console.log("clssName is " +clssName);
...
function AllDistributionBoardCheckFilter(Id, clssName) {
    ...
    $("#" + Id).children('input')
        ...
        .on("change", function() {
            ...
            // --------------------------------------------------
            // 1. Detect impacted layers (from subtree ONLY)
            // --------------------------------------------------
            const layers = [];

            if ($root.find('.BackboneDB').length) layers.push("backbone");
            if ($root.find('.MetroDB').length) layers.push("metro");
            if ($root.find('.AccessDB').length) layers.push("access");

            // --------------------------------------------------
            // 2. Controller detection (from subtree ONLY)
            // --------------------------------------------------
            const hasController =
                $root.find(
            ...
            // --------------------------------------------------
            // 3. Propagate checkbox state inside subtree
            // --------------------------------------------------
            $root.find('input:checkbox').prop('checked', isChecked);

            // --------------------------------------------------
            // 4. Clear ONLY impacted clusters (global)
            // --------------------------------------------------
            ...
            // --------------------------------------------------
            // 5. REBUILD FROM FULL DOM (IMPORTANT CHANGE)
            // --------------------------------------------------
            rebuildAllLayerClustersFromDOM(layers);

            // --------------------------------------------------
            // 6. Controller (ONLY if subtree contains controller)
            // --------------------------------------------------
            if (hasController && typeof markerClusterController !== "undefined") {
                markerClusterController.clearMarkers();
                rebuildControllerFromDOM();
            }

            // --------------------------------------------------
            // 7. UI sync
            // --------------------------------------------------
            syncDistributionBoardGroupState(Id);
            syncGlobalBoqState();
        });
}
...
function getCluster(layer) {
    if (layer === "backbone") return markerClusterBackboneDistBoard;
    if (layer === "metro") return markerClusterMetroDistBoard;
    if (layer === "access") return markerClusterAccessDistBoard;
}




function rebuildControllerFromDOM() {

    markerClusterController.clearMarkers();

    $("#network_tree")
        .find('.backboneControllerDBFolder, .metroControllerDBFolder, .accessControllerDBFolder')
        .each(function() {
...
function rebuildAllLayerClustersFromDOM(layers) {

    const $all = $("#network_tree");
    ...
    layers.forEach(layer => {
        ...
        if (layer === "backbone") selector = ".backboneDBFolder";
        else if (layer === "metro") selector = ".metroDBFolder";
        else if (layer === "access") selector = ".accessDBFolder";

        $all
            .find(`${selector} .DistributionBoard input:checked`)
            .each(function() {
...
function syncGlobalBoqState() {

    const anyChecked =
        $("#network_tree")
            .find(".DistBoard:checked, .BackboneDB:checked, .MetroDB:checked, .AccessDB:checked")
            .length > 0;

    $("#distBoardCheckAllBoq").prop("checked", anyChecked);
}


function syncDistributionBoardGroupState(Id) {

    if (Id.includes("showDB")) return;

    let selector = "";

    if (Id.includes("CurrentPhysicalLayer")) {
        selector = "#DistributionBoard_f_CurrentPhysicalLayer";
    }
    else if (Id.includes("PROJECT")) {

        const startIndex = Id.indexOf('PROJECT');
        const projectId = Id.substring(startIndex);

        selector = "#DistributionBoard_f_" + projectId;
    }
    ...
    const backboneChecked =
        $scope.find('.BackboneDB').length > 0 &&
        $scope.find('.BackboneDB').length === $scope.find('.BackboneDB:checked').length;

    const metroChecked =
        $scope.find('.MetroDB').length > 0 &&
        $scope.find('.MetroDB').length === $scope.find('.MetroDB:checked').length;

    const accessChecked =
        $scope.find('.AccessDB').length > 0 &&
        $scope.find('.AccessDB').length === $scope.find('.AccessDB:checked').length;

    $scope.find('.AllDistBoards').prop(
        'checked',
        backboneChecked && metroChecked && accessChecked
    );
}
...
function pathCheckFilter(Target, type, Id, drawingTypeIndex, pathArray, allPathArray, drivingPathArray, pathRouteDisplay, chldrnClass, chldrnPathArray, chldrnDrivPathArray, chldrnAllPathArray) {
    ...
    if (type == "parentFolderCheck" || type == "parentFolderCheck__showPath") {
        ...
        if (Target == "initial_ul") { // Case of main Trench Folder, Fiber Path Folder , Backbone / Metro / Access Folder
            ...
            mainPathCheckFilterEvent(Id, type);
        }
...
function mainPathCheckFilter(selector) {
    $(selector).parent().find('input:checkbox').each(function() {
        $(this).prop('checked', true);
        if ($(this).parent().hasClass('FIBER')) {
            singleFiberCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('TUBE')) {
            singleTubeCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('STRAND')) {
            singleStrandCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('Trench')) {
            singleTrenchCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('Duct')) {
            singleDuctCheckFilter($(this));
        }
    });
}

function mainPathUnCheckFilter(selector) {

    console.log("mainPathUnCheckFilter and selector is " + selector);

    $(selector).parent().find('input:checkbox').each(function() {
        $(this).prop('checked', false);
        if ($(this).parent().hasClass('FIBER')) {
            singleFiberUnCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('TUBE')) {
            singleTubeUnCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('STRAND')) {
            singleStrandUnCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('Trench')) {
            singleTrenchUnCheckFilter($(this));
        }
        else if ($(this).parent().hasClass('Duct')) {
            singleDuctUnCheckFilter($(this));
        }
    });
...
function singleFiberCheckFilter(selector) {

    var fiberId = $(selector).parent().attr('id');
...
function singleStrandCheckFilter(selector) {

    var strandId = $(selector).parent().attr('id');
...
function singleTrenchCheckFilter(selector) {
...
function singleTubeUnCheckFilter(selector) {
...
function singleStrandUnCheckFilter(selector) {
...
function calculateDistanceSourceDestination(sLat,sLng,dLat,dLng,tId){
        ...
        var countrowAuxiliaryFO =$('#'+tId+' >tbody >tr').length;
                    ...
                    $('#'+tId+' >tbody >tr').each(function(){
                            ...
                            console.log("latAux is " , latAux + " lngAux is " , lngAux);
                    ...
                    if(countrowAuxiliaryFO>0){
                        ...
                        console.log("sourceFirstAux is " , sourceFirstAux);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            */
            ...
            console.log("********Fiber Ducts is ", data.fiberDucts);
            loadFiberDuctTable(data.fiberDucts);
...
function showHideManHandHolesWithJct(pathID) {
    ...
    if (window["mapPointsNames_" + pathID] != undefined) {
        ...
        for (var x = 0;x < showManhHandJctArray.length;x++) {
            ...
            if (showManhHandJctArray[x].startsWith("MH_") == true && readManhole === '1') {
            ...
            else if (showManhHandJctArray[x].startsWith("HH_") == true && readHandhole === '1') {
                    ...
                    if (markersSite[wareID]) {
                        ...
                        //Show Seq is unchecked
                            ...
                            if (allcheckedLabels.length > 0 && allcheckedLabels.includes("sitesMapCheck_Labels") == true) {
                                ...
                                //if(markersSite[wareID].getLabel()!="undefined") {
...
function showHideAllSequence(pathID, action) {
    if (action == "Show") {
        for (var x = 0;x < window["mapPointsNames_" + pathID].length;x++) {
            ...
            else if (window["mapPointsNames_" + pathID][x].startsWith("MH_") == true && readManhole === '1') {
            ...
            else if (window["mapPointsNames_" + pathID][x].startsWith("HH_") == true && readHandhole === '1') {
        ...
        success: function(data) {
            ...
            if (layer == "Manhole" || layer == "AllManholes") {
                ...
                for (var k = 0;k < allSelectedLayer.length;k++) {
                    ...
                    if (window["" + allSelectedLayer[k]][8] == "backbone") {
                        markerClusterBackboneDistBoard.removeMarker(markersDistBoard[allSelectedLayer[k]]);
                    }
                    else if (window["" + allSelectedLayer[k]][8] == "metro") {
                        markerClusterMetroDistBoard.removeMarker(markersDistBoard[allSelectedLayer[k]]);
                    }
                    else if (window["" + allSelectedLayer[k]][8] == "access") {
                        markerClusterAccessDistBoard.removeMarker(markersDistBoard[allSelectedLayer[k]]);
                    }
                ...
                for (var k = 0;k < allSelectedLayer.length;k++) {
                    ...
                    if (allDBProject.length > 0) {
                        for (var D = 0;D < allDBProject.length;D++) {
                            ...
                            if (window["" + allDBProject[D]][8] == "backbone") {
                                markerClusterBackboneDistBoard.removeMarker(markersDistBoard[allDBProject[D]]);
                            }
                            else if (window["" + allDBProject[D]][8] == "metro") {
                                markerClusterMetroDistBoard.removeMarker(markersDistBoard[allDBProject[D]]);
                            }
                            else if (window["" + allDBProject[D]][8] == "access") {
                                markerClusterAccessDistBoard.removeMarker(markersDistBoard[allDBProject[D]]);
                            }
                        }
                    }
...
function changeColor() {
    ...
    $('table tr input[name="showFibersOwner"]').on('change', function() {// for checkbox of the owner to show related fibers
        ...
        var ownerElements = $('table tr input[name="showFibersOwner"]:checked');
        //console.log("ownerElements length is " +ownerElements.length);
        var ownerNameArr = []
        for (var k = 0;k < ownerElements.length;k++) {
            ownerNameArr[k] = $(ownerElements[k]).attr("id");
        }
        //console.log("ownerNameArr length is " +ownerNameArr.length);
        //console.log("ownerNameArr values is ", ownerNameArr);


        //var ownerName = $(this).attr("id");
        for (var f = 0;f < fiberIdList.length;f++) {
            //console.log("fiberIdList[f] is " +fiberIdList[f] , " f is " +f);
            if (ownerNameArr.includes(window['' + fiberIdList[f]][22])) {
...
// show path for manhole/handhole/DB
function showPath(url, dataSel, tr) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#path_table").append(tr);

            var str = "<ul style='list-style-type:none;padding:0px;cursor: pointer;'><li id='FiberPath_f_showPath' class='FiberPath_f_Cable'><input type='checkbox' checked class='AllFiberCables__showPath checkFilter' class='filter'></input> <span id='FiberPath_spanFolder'  class='Parentfolder'><i class='fa fa-folder' style='color: #08526D'></i></span><span id='FiberPath_span' class='TreeSpan' style='color:black;margin-left:5px;'>Fiber Path </span></li></ul>";
            $("#path_table").append(str);
            pathCheckFilter("initial_ul", "parentFolderCheck__showPath", "FiberPath_f_showPath", "", "", "", "", "", "", "", "", "");
            ...
            if (data.BackboneCableData.length > 0) {
                ...
                for (i = 0;i < data.BackboneCableData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.BackboneCableData[i][0] + "__showPath' class='FIBER' style='display:none; width:1000px;'><input type='checkbox' class='FiberCable checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'><img src='" + getContext() + "/resources/NetworkImages/fiber.png'>  " + data.BackboneCableData[i][1] + "/" + data.BackboneCableData[i][0] + "  </span></li></ul>";
                    $("#FiberPath_backbone_cable__showPath_f").append(str);

                    if (fiberArray[data.BackboneCableData[i][0]]) {
                        fiberArray[data.BackboneCableData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.BackboneCableData[i][0], window["mapPoints_" + data.BackboneCableData[i][0]], fiberArray, allFiberCables, "FiberPath_f_", window['FiberColor_' + data.BackboneCableData[i][3]], 0.7, 4.5, 'blue', 13);
                        fiberArray[data.BackboneCableData[i][0]].setMap(map);
                    }
                    Create_FiberPath(data.BackboneCableData[i][0] + "__showPath");
                    pathCheckFilter(TargetFiber, "showPath", data.BackboneCableData[i][0] + "__showPath", "20", fiberArray, allFiberCables, directionHashmap, routeDisplay, "", "", "", "");
                }
            }
            ...
            if (data.BackboneTubeData.length > 0) {
                ...
                for (i = 0;i < data.BackboneTubeData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.BackboneTubeData[i][0] + "__showPath' class='TUBE' style='display:none; width:1000px;'><input type='checkbox' class='FiberTube checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block; color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px;font-size:12px; font-weight:bold; color:" + data.BackboneTubeData[i][5] + "'>" + data.BackboneTubeData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/core.png' style='margin-bottom:-10px;'> " + data.BackboneTubeData[i][1] + "/" + data.BackboneTubeData[i][0] + "<b> : </b>" + data.BackboneTubeData[i][3] + "/" + data.BackboneTubeData[i][2] + "  </span></li></ul>";
                    $("#FiberPath_backbone_tube__showPath_f").append(str);

                    if (tubeArray[data.BackboneTubeData[i][0]]) {
                        tubeArray[data.BackboneTubeData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.BackboneTubeData[i][0], window["mapPoints_" + data.BackboneTubeData[i][0]], tubeArray, allTubes, "Tube", 'green', 0.7, 3.3, 'green', 0);
                        tubeArray[data.BackboneTubeData[i][0]].setMap(map);
                    }
                    Create_FiberTubeClick_Event(data.BackboneTubeData[i][0] + "__showPath");
                    pathCheckFilter(TargetTube, "showPath", data.BackboneTubeData[i][0] + "__showPath", "14", tubeArray, allTubes, directionHashmapTube, routeDisplayTube, "", "", "", "");
                }
            }
            ...
            if (data.BackboneStrandData.length > 0) {
                ...
                for (i = 0;i < data.BackboneStrandData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.BackboneStrandData[i][0] + "__showPath' class='STRAND' style='display:none; width:1000px;'><input type='checkbox' class='FiberStrand checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px; font-size:12px; font-weight:bold; color:" + data.BackboneStrandData[i][5] + "'>" + data.BackboneStrandData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/strand.png' style='margin-bottom:-10px;'> " + data.BackboneStrandData[i][1] + "/" + data.BackboneStrandData[i][0] + "<b> : </b>" + data.BackboneStrandData[i][3] + "/" + data.BackboneStrandData[i][2] + "<b> : </b>" + data.BackboneStrandData[i][7] + "/" + data.BackboneStrandData[i][8] + " </span></li></ul>";
                    $("#FiberPath_backbone_strand__showPath_f").append(str);

                    if (strandArray[data.BackboneStrandData[i][0]]) {
                        strandArray[data.BackboneStrandData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.BackboneStrandData[i][0], window["mapPoints_" + data.BackboneStrandData[i][0]], strandArray, allStrands, "Strand", 'purple', 0.7, 2.8, 'purple', 0);
                        strandArray[data.BackboneStrandData[i][0]].setMap(map);
                    }
                    Create_FiberStrandClick_Event(data.BackboneStrandData[i][0] + "__showPath");
                    pathCheckFilter(TargetStrand, "showPath", data.BackboneStrandData[i][0] + "__showPath", "14", strandArray, allStrands, directionHashmapStrand, routeDisplayStrand, "", "", "", "");
                }
            }
            ...
            if (data.MetroCableData.length > 0) {
                ...
                for (i = 0;i < data.MetroCableData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.MetroCableData[i][0] + "__showPath' class='FIBER' style='display:none; width:1000px;'><input type='checkbox' class='FiberCable checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'> <img class='image' src='" + getContext() + "/resources/NetworkImages/fiber.png'>" + data.MetroCableData[i][1] + "/" + data.MetroCableData[i][0] + " </span></li></ul>";
                    $("#FiberPath_metro_cable__showPath_f").append(str);

                    if (fiberArray[data.MetroCableData[i][0]]) {
                        fiberArray[data.MetroCableData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.MetroCableData[i][0], window["mapPoints_" + data.MetroCableData[i][0]], fiberArray, allFiberCables, "FiberPath_f_", window['FiberColor_' + data.MetroCableData[i][3]], 0.7, 4.5, 'blue', 13);
                        fiberArray[data.MetroCableData[i][0]].setMap(map);
                    }
                    Create_FiberPath(data.MetroCableData[i][0] + "__showPath");
                    pathCheckFilter(TargetFiber, "showPath", data.MetroCableData[i][0] + "__showPath", "20", fiberArray, allFiberCables, directionHashmap, routeDisplay, "", "", "", "");
                }
            }
            ...
            if (data.MetroTubeData.length > 0) {
                ...
                for (i = 0;i < data.MetroTubeData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.MetroTubeData[i][0] + "__showPath'  class='TUBE' style='display:none; width:1000px;'><input type='checkbox' class='FiberTube checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px; font-size:12px; font-weight:bold; color:" + data.MetroTubeData[i][5] + "'>" + data.MetroTubeData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/core.png' style='margin-bottom:-10px;'> " + data.MetroTubeData[i][1] + "/" + data.MetroTubeData[i][0] + "<b> : </b>" + data.MetroTubeData[i][3] + "/" + data.MetroTubeData[i][2] + "  </span></li></ul>";
                    $("#FiberPath_metro_tube__showPath_f").append(str);

                    if (tubeArray[data.MetroTubeData[i][0]]) {
                        tubeArray[data.MetroTubeData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.MetroTubeData[i][0], window["mapPoints_" + data.MetroTubeData[i][0]], tubeArray, allTubes, "Tube", 'green', 0.7, 3.3, 'green', 0);
                        tubeArray[data.MetroTubeData[i][0]].setMap(map);
                    }
                    Create_FiberTubeClick_Event(data.MetroTubeData[i][0] + "__showPath");
                    pathCheckFilter(TargetTube, "showPath", data.MetroTubeData[i][0] + "__showPath", "14", tubeArray, allTubes, directionHashmapTube, routeDisplayTube, "", "", "", "");
                }
            }
            ...
            if (data.MetroStrandData.length > 0) {
                ...
                for (i = 0;i < data.MetroStrandData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.MetroStrandData[i][0] + "__showPath' class='STRAND' style='display:none; width:1000px;'><input type='checkbox' class='FiberStrand checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block; color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px; font-size:12px; font-weight:bold; color:" + data.MetroStrandData[i][5] + "'>" + data.MetroStrandData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/strand.png' style='margin-bottom:-10px;'> " + data.MetroStrandData[i][1] + "/" + data.MetroStrandData[i][0] + "<b>:</b>" + data.MetroStrandData[i][3] + "/" + data.MetroStrandData[i][2] + "<b>:</b>" + data.MetroStrandData[i][7] + "/" + data.MetroStrandData[i][8] + " </span></li></ul>";
                    $("#FiberPath_metro_strand__showPath_f").append(str);

                    if (strandArray[data.MetroStrandData[i][0]]) {
                        strandArray[data.MetroStrandData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.MetroStrandData[i][0], window["mapPoints_" + data.MetroStrandData[i][0]], strandArray, allStrands, "Strand", 'purple', 0.7, 2.8, 'purple', 0);
                        strandArray[data.MetroStrandData[i][0]].setMap(map);
                    }
                    Create_FiberStrandClick_Event(data.MetroStrandData[i][0] + "__showPath");
                    pathCheckFilter(TargetStrand, "showPath", data.MetroStrandData[i][0] + "__showPath", "14", strandArray, allStrands, directionHashmapStrand, routeDisplayStrand, "", "", "", "");
                }
            }
            ...
            if (data.DistributionCableData.length > 0) {
                ...
                for (i = 0;i < data.DistributionCableData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.DistributionCableData[i][0] + "__showPath' class='FIBER' style='display:none; width:1000px;'><input type='checkbox' class='FiberCable checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'><img class='image' src='" + getContext() + "/resources/NetworkImages/fiber.png'>" + data.DistributionCableData[i][1] + "/" + data.DistributionCableData[i][0] + " </span></li></ul>";
                    $("#FiberPath_distribution_cable__showPath_f").append(str);

                    if (fiberArray[data.DistributionCableData[i][0]]) {
                        fiberArray[data.DistributionCableData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.DistributionCableData[i][0], window["mapPoints_" + data.DistributionCableData[i][0]], fiberArray, allFiberCables, "FiberPath_f_", window['FiberColor_' + data.DistributionCableData[i][3]], 0.7, 4.5, 'blue', 13);
                        fiberArray[data.DistributionCableData[i][0]].setMap(map);
                    }
                    Create_FiberPath(data.DistributionCableData[i][0] + "__showPath");
                    pathCheckFilter(TargetFiber, "showPath", data.DistributionCableData[i][0] + "__showPath", "20", fiberArray, allFiberCables, directionHashmap, routeDisplay, "", "", "", "");
                }
            }
            ...
            if (data.DistributionTubeData.length > 0) {
                ...
                for (i = 0;i < data.DistributionTubeData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.DistributionTubeData[i][0] + "__showPath'  class='TUBE' style='display:none; width:1000px;'><input type='checkbox' class='FiberTube checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block;color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px; font-size:12px; font-weight:bold; color:" + data.DistributionTubeData[i][5] + "'>" + data.DistributionTubeData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/core.png' style='margin-bottom:-10px;'> " + data.DistributionTubeData[i][1] + "/" + data.DistributionTubeData[i][0] + "<b> : </b>" + data.DistributionTubeData[i][3] + "/" + data.DistributionTubeData[i][2] + "  </span></li></ul>";
                    $("#FiberPath_distribution_tube__showPath_f").append(str);

                    if (tubeArray[data.DistributionTubeData[i][0]]) {
                        tubeArray[data.DistributionTubeData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.DistributionTubeData[i][0], window["mapPoints_" + data.DistributionTubeData[i][0]], tubeArray, allTubes, "Tube", 'green', 0.7, 3.3, 'green', 0);
                        tubeArray[data.DistributionTubeData[i][0]].setMap(map);
                    }
                    Create_FiberTubeClick_Event(data.DistributionTubeData[i][0] + "__showPath");
                    pathCheckFilter(TargetTube, "showPath", data.DistributionTubeData[i][0] + "__showPath", "14", tubeArray, allTubes, directionHashmapTube, routeDisplayTube, "", "", "", "");
                }
            }
            ...
            if (data.DistributionStrandData.length > 0) {
                ...
                for (i = 0;i < data.DistributionStrandData.length;i++) {
                    var str = "<ul style='list-style-type:none;cursor: pointer;'><li id='" + data.DistributionStrandData[i][0] + "__showPath' class='STRAND' style='display:none; width:1000px;'><input type='checkbox' class='FiberStrand checkFilter' checked class='filter'></input> <span class='TreeSpan' style='display: inline-block; color:black; margin-inline:10px;'> <div style='margin-bottom:-17px;margin-left:6px; font-size:12px; font-weight:bold; color:" + data.DistributionStrandData[i][5] + "'>" + data.DistributionStrandData[i][6] + "</div> <img src='" + getContext() + "/resources/NetworkImages/strand.png' style='margin-bottom:-10px;'> " + data.DistributionStrandData[i][1] + "/" + data.DistributionStrandData[i][0] + "<b>:</b>" + data.DistributionStrandData[i][3] + "/" + data.DistributionStrandData[i][2] + "<b>:</b>" + data.DistributionStrandData[i][7] + "/" + data.DistributionStrandData[i][8] + " </span></li></ul>";
                    $("#FiberPath_distribution_strand__showPath_f").append(str);

                    if (strandArray[data.DistributionStrandData[i][0]]) {
                        strandArray[data.DistributionStrandData[i][0]].setMap(map);
                    }
                    else {
                        buildPath(data.DistributionStrandData[i][0], window["mapPoints_" + data.DistributionStrandData[i][0]], strandArray, allStrands, "Strand", 'purple', 0.7, 2.8, 'purple', 0);
                        strandArray[data.DistributionStrandData[i][0]].setMap(map);
                    }
