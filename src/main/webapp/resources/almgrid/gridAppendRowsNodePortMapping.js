  ...
  showRows(rows, start, end) {
      ...
      for (let i = start; i <= end; i++) {
          ...
          for (var j = 0; j < this.ArrayKeys.length; j++) {
        	  if (j == 0) {
				itemRow += "<td><button type='button' class='viewPortMapping' id='viewPortMapping"+i+ "'  onClick='viewMapping(\"" + rows[i][this.ArrayKeys[j]] + "\",\"" +i+ "\")' >View Port Mapping Diagram</button></td>"
              } 
