...
public class ClusterController {
	...
	public String ClusterListView(Locale locale, Model model, HttpServletRequest request,HttpServletResponse response) {
		...
		query = session.createQuery("select 1,t.id, t.name,TO_CHAR(t.creationDate, 'YYYY-MM-DD HH24:MI:SS'),TO_CHAR(t.lastModifieddate, 'YYYY-MM-DD HH24:MI:SS') from Cluster t");
	...
	public String ClusterFormView(Locale locale, Model model, HttpServletRequest request,HttpServletResponse response) throws JsonProcessingException {
		...
		if(cluster!=null) {
			if (cluster.getCreationDate() == null) {
	       	...
	       	if (cluster.getLastModifieddate() == null) {
	    		...
	    		model.addAttribute("lastModifiedDate", formatter.format(cluster.getLastModifieddate()).toString());
		...
		// add data in table WorkOrderSource
		    ...
		    query = session.createQuery("select  TO_CHAR(t.startDate,'YYYY-MM-DD') as startDate , TO_CHAR(t.endDate,'YYYY-MM-DD') as endDate,t.number2gSites as number2gSites, t.number2g3gSites as number2g3gSites , t.number2g3g4gSites as number2g3g4gSites, t.pl2g as pl2g, t.pl2g3g as pl2g3g, t.pl2g3g4g as pl2g3g4g, t.totalSites as totalSites, t.sumProfitLoss as sumProfitLoss, t.id as id from ClusterFinance t where t.clusterID like :param1");
		    query.setParameter("param1", request.getParameter("clusterID"));
		    @SuppressWarnings("unchecked")
			List<ClusterFinanceBOQ> listClusterFin = (List<ClusterFinanceBOQ>) query.setResultTransformer(Transformers.aliasToBean(ClusterFinanceBOQ.class)).list();
	...
	public Map<String, Object> ClusterFormSave(Locale locale, Model model, HttpServletRequest request,
		...
		String clusterID=""; String areaId="";  String areaName="",Area;
...
System.out.println("all email parameters  = "+email+" "+emailTo+" "+password+" "+message+" "+subject+" "+ccmail);
...
///////////////////////////////////////////// END OF SEND EMAIL BUTTON  //////////////////////////////////////////////////////////
		...
		{	
			synchronized (this) {						
				//clusterID= "CLUSTER_"+calendar.get(Calendar.YEAR)+"_" +appConfig.getSequenceNbr(21);	
				clusterID= "CLUSTER_"+calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(session.createSQLQuery("SELECT CLUSTERS FROM SEQ_TABLE").uniqueResult().toString());	
				query = session.createSQLQuery("UPDATE SEQ_TABLE SET CLUSTERS = CLUSTERS + 1 ");
				query.executeUpdate();
		...
		for ( int i = 0; i <itemParameters.getDictParameter().size(); i++) {
			ClusterFinance clusterFin=new ClusterFinance();
			 ...
			 if (StringUtils.equalsIgnoreCase(itemParameters.getDictParameter().get(i).get(ID), "0")) {
					...
					synchronized (this) {						
						//clusterIdFinance= "CLUSTER_FIN_"+calendar.get(Calendar.YEAR)+"_" +appConfig.getSequenceNbr(22);
						clusterIdFinance= "CLUSTER_FIN_"+calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(session.createSQLQuery("SELECT CLUSTER_FINANCE FROM SEQ_TABLE").uniqueResult().toString());	
						query = session.createSQLQuery("UPDATE SEQ_TABLE SET CLUSTER_FINANCE = CLUSTER_FINANCE + 1 ");
						query.executeUpdate();
						session.createSQLQuery("commit").executeUpdate();
						}
			 ...
			 if((itemParameters.getDictParameter().get(i).get(START_DATE)) !="") {
				 ...
				 clusterFin.setStartDate(new Timestamp((formatter1.parse(itemParameters.getDictParameter().get(i).get(START_DATE))).getTime()));			 
			
			 }
			 if((itemParameters.getDictParameter().get(i).get(END_DATE))!="") {	
				 
				 clusterFin.setEndDate(new Timestamp((formatter1.parse(itemParameters.getDictParameter().get(i).get(END_DATE))).getTime()));
			
			 }

			clusterFin.setId(clusterIdFinance);
			clusterFin.setNumber2gSites(Float.parseFloat(itemParameters.getDictParameter().get(i).get(SITES2G)));
			clusterFin.setPl2g(Float.parseFloat(itemParameters.getDictParameter().get(i).get(PROFIT2G)));
			clusterFin.setNumber2g3gSites(Float.parseFloat(itemParameters.getDictParameter().get(i).get(SITES2G3G)));
			clusterFin.setPl2g3g(Float.parseFloat(itemParameters.getDictParameter().get(i).get(PROFIT2G3G)));
			clusterFin.setNumber2g3g4gSites(Float.parseFloat(itemParameters.getDictParameter().get(i).get(SITES2G3G4G)));
			clusterFin.setPl2g3g4g(Float.parseFloat(itemParameters.getDictParameter().get(i).get(PROFIT2G3G4G)));
			clusterFin.setTotalSites(Float.parseFloat(itemParameters.getDictParameter().get(i).get(TOTALSITES)));
			clusterFin.setSumProfitLoss((Float.parseFloat(itemParameters.getDictParameter().get(i).get(TOTALSUM))));
	...
	try {
		...
		query = session.createQuery("select t.id, t.name from Cluster t where t.id like :param1 or t.name like :param1 ORDER BY lastModifieddate DESC");
		query.setString("param1", cluster);
