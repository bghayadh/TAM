...
public class CellController {
	...
	public String CellListView(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response) {
			...
			try {
				...
				query = session.createNativeQuery(
						"SELECT  ng.Gcell_Id as gcellPk, ng.Gcell_Id as Pk, ng.cellid as cellid,   ng.cellname as cellname , '2G' as Technology, na.unique_node_id as uniqueid, na.site_id as siteid, "
								...
								+ " FROM  NODE_4GCELL nl LEFT JOIN " + " NODE_ACTIVE na ON nl.Node_PK = na.Node_PK");
	...
	public String CellFormView(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response) {
			...
			try {
				...
				if (CellPK != null) {
					...
					if (CellPK.contains("GCELL")) {
						...
						result = (Object[]) query.uniqueResult();
						query = session
								.createNativeQuery("SELECT ID, DATE_ON_AIR, SITE_SUB_TYPE, ANTENNA_GAIN, BEAM_WIDTH, "
										+ "AZIMUTH, ANTENNA_HEIGHT_AGL, ELECTRICAL_TILT, MECHANICAL_TILT, RET, FEEDER_SIZE, "
										+ "APPROXIMATE_FEEDER_LENGTH, TMA_MHA, REMARKS, SECTOR_STATUS, SECTOR_LOCKED_DATE, "
										+ "LOCKED_REASON, AT, MASTER_SECTOR_ID, FLAG, STATUS, NEP_SYNCH, CIRCLE_ID "
						...
						query = session.createNativeQuery(
								"SELECT  nl.Lcell_Id ,  nL.cellid ,   nL.cellname  , na.unique_node_id , na.site_id , "
										...
										+ " FROM  NODE_4GCELL nl LEFT JOIN "
										+ " NODE_ACTIVE na ON nl.Node_PK = na.Node_PK where nl.Lcell_Id=:param1");
						query.setParameter("param1", CellPK);
						result = (Object[]) query.uniqueResult();
						query = session.createNativeQuery(
								"SELECT ID, LTE_CELL_ID, DATE_ON_AIR, CELLNAME, SITE_SUB_TYPE, ANTENNA_SHARED_WITH_2G,\r\n"
			...
			try {
				...
				if (cellPk.contains("GCELL")) {
					...
					if (request.getParameter("passivePK") == "") {
						passivePK = "Passive_" + calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(session
								.createNativeQuery("SELECT GCELL_PASSIVE FROM SEQ_TABLE").uniqueResult().toString());
					} else {
					...
					if (request.getParameter("passivePK") == "") {
						passivePK = "Passive_" + calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(session
								.createNativeQuery("SELECT LCELL_PASSIVE FROM SEQ_TABLE").uniqueResult().toString());
					} else {
						passivePK = request.getParameter("passivePk");
					}
					...
					if (request.getParameter("passivePK") == "") {
						passivePK = "Passive_" + calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(session
								.createNativeQuery("SELECT UCELL_PASSIVE FROM SEQ_TABLE").uniqueResult().toString());
					} else {
						passivePK = request.getParameter("passivePk");
					}
