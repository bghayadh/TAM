...
public class FixedAssetRegistryController {
	...
	public String FixedAssetRegistryListView(Locale locale, Model model, HttpServletRequest request,HttpServletResponse response) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				/*String str = "select FAR_ID as farID, FAR_ID as fixedassetID, ITEM_CODE as itemCode, ITEM_NAME as itemName, "
				...
				listFAR =  ((SQLQuery) query)
						.addScalar("farID").addScalar("fixedassetID").addScalar("itemCode").addScalar("itemName")
		       			...
		       			.addScalar("itemNameRegister").addScalar("poID").addScalar("nodeID").addScalar("nodeName").addScalar("siteID").addScalar("siteName")
	...
	public Map<String, Object> FilteredFixedAssetListView(Locale locale, Model model, HttpServletRequest request,
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				/*String str1 = "select 1 as chkBox, FAR_ID as farID, ITEM_CODE as itemCode, ITEM_NAME as itemName," 
												+" TO_CHAR(LAST_MODIFIED_DATE,'YYYY-MM-DD HH24:MI:SS') as lastModifiedDate, ITEM_SN as itemSN, ITEM_NAME_REGISTER as itemNameRegister, PO_ID as poID," 
											    + "COALESCE(NODE_ID, ' ') as nodeID, COALESCE(NODE_NAME, ' ') as nodeName  " 
				...
				if (( radius != null && !radius.equalsIgnoreCase("") && Double.parseDouble(radius) > 0) && (longitude != null && !longitude.equalsIgnoreCase("")) && (latitude != null
					...
					for (int i = 0; i < listFixed.size(); i++) {
						...
						distance = haversine(Double.parseDouble(longitude), Double.parseDouble(latitude),
								Double.valueOf(listFixedAssetTemp.get(i)[12].toString()),Double.valueOf(listFixedAssetTemp.get(i)[13].toString()));
	...
	public String FixedAssetRegistryFormView(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response) {
		if(LoginServices.checkSession(request, response).equals("redirect:/")) {
			return LoginServices.checkSession(request, response);
		} 
		...
		String farID = request.getParameter("farID"); String navAction = request.getParameter("NavAction"); String itemCategoryDetails = "";
		int SelectedIndex = 0;
		
		FixedAssetRegistry fassetreg; Item itm; List<PurchaseOrderItem> listPurchaseOrderItem;
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				result = form.NavigationNP(session,"FIXED_ASSET_REGISTRY","FAR_ID",farID,"LAST_MODIFIED_DATE",navAction);		
			...
			// get Finance Tab
				...
				model.addAttribute("farcreatedDate", formatterTime.format(fassetreg.getFarcreatedDate()).toString());
		 	    model.addAttribute("farlastModifiedDate", formatterTime.format(fassetreg.getFarlastModifiedDate()).toString());
	...
	@ResponseBody
	public Map<String, Object> FixedAssetRegistryFormSave(Locale locale, Model model, HttpServletRequest request, @ModelAttribute ItemParameters itemParameters, HttpServletResponse response) throws Exception {
...
///////////////////////////////////////////// END OF SEND EMAIL BUTTON  //////////////////////////////////////////////////////////
		...
		query.executeUpdate();
		//appConfig.deleteRowsByQueryParamList(FarPartNumber.class, querydeletePNum, "param1", slctDelModPart);
		}
		...
		if (itemParameters.getDictParameter() != null) {
			...
			for (int i = 0; i < itemParameters.getDictParameter().size(); i++) {
				...
				if(itemParameters.getDictParameter().get(i).get("qtyPartNum")!="") {
					 ...
					 qtyPartNum=Float.parseFloat(itemParameters.getDictParameter().get(i).get("qtyPartNum"));
		...
		query.executeUpdate();
		//appConfig.deleteRowsByQueryParamList(FarNode.class, querydelete, "param1", slctDelNode);
	    }
		...
		if( slctDelSite != null) {
			...
			query.executeUpdate();
		//appConfig.deleteRowsByQueryParamList(FarSite.class, querySitedelete, "param1", slctDelSite);
		}
		...
		// Saving Serial Number BOQ 
			...
			query.executeUpdate();
			//appConfig.deleteRowsByQueryParamList(FarSerialNumber.class, querydeleteSNum, "param1", slctDelSerial);
            }
			
			String serialId;
			if (itemParameters.getDictParameterFixedAssetRegistrySerialNumber() != null) {
				
				for (int i = 0; i < itemParameters.getDictParameterFixedAssetRegistrySerialNumber().size(); i++) {
                ...
                farSerialNumber.setSerialId(serialId);
						...
						farSerialNumber.setSerialId(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("farSerialID"));
					}

				farSerialNumber.setInputSerialNb(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNserialNumber"));
				farSerialNumber.setInputModel(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNmodel"));
				farSerialNumber.setInputpartNumber(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNpartNumber"));
				farSerialNumber.setInputNodeID(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNnodeID"));
				farSerialNumber.setInputNodeName(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNnodeName"));
				farSerialNumber.setInputsite(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNsite"));
				farSerialNumber.setInputPosition(itemParameters.getDictParameterFixedAssetRegistrySerialNumber().get(i).get("SNposition"));
		...
		rtn.put("farlastModifiedDate", formatterTime.format(new Timestamp(System.currentTimeMillis())).toString());
	...
	@ResponseBody
	public Map<String, Object> FixedAssetRegistryFormViewDelete(Locale locale, Model model, HttpServletRequest request,HttpServletResponse response) {
		Map<String, Object> rtn = new LinkedHashMap<>();
		...
		session = almsessions.getSession();
					...
					if (session != null && session.isOpen()) {
			            ...
			            try {
			                            ...
			                            System.out.println("Error in creating session with the FixedAssetRegistryFormViewDelete method " + e.getMessage());
	...
	@ResponseBody
	public Map<String, Object> FixedAssetRegistryListViewDelete(Locale locale, Model model, HttpServletRequest request,HttpServletResponse response) {
		...
		if (session != null && session.isOpen()) {
            ...
            try {
                ...
                System.out.println("Error in creating session with the FixedAssetRegistryListViewDelete method " + e.getMessage());
	...
	@ResponseBody
	public Map<String, Object> GetAllFarNodeID(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response)
	...
	return rtn;
			...
			try {
					...
					logger.info(
							"Error while getting FAR NODE ID using autocomplete with error message: "+ e);
					e.printStackTrace();
...
@ResponseBody
public Map<String, Object> GetAllFarNodeName(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response)
throws JsonGenerationException, JsonMappingException, IOException {
...
try {
		...
		logger.info(
				"Error while getting FAR NODE NAME using autocomplete with error message: "+ e);
		e.printStackTrace();
...
//getting All FAR using autocomplete in navigation
	...
	public Map<String, Object> GetAllFAR(Locale locale, Model model, HttpServletRequest request, 
		...
		try {
				...
				logger.info(
						"Error while getting FAR using autocomplete with error message: "+ e);
				e.printStackTrace();
