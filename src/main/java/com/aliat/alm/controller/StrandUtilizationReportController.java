			...
			try {
				...
				if (session != null && session.isOpen()) {
					...
					String str ="SELECT * FROM ( SELECT DISTINCT A.STRAND_NB_SIDE_A AS strandNo,A.TUBE_NB_SIDE_A AS tubeNo,A.LOCATION_TYPE_SIDE_A AS locationType,A.LOCATION_ID_SIDE_A AS locationID,A.LOCATION_NAME_SIDE_A AS locationName,"
							...
							+ " 'Junction' AS elementType,A.JCT_ID  as elementID, 'Side A' as frontBackPort, CAST(A.SEQUENCE_NUMBER AS VARCHAR2(10)) as portIndex,'' as portRow,'' as portColumn,'' AS status,'' as equipmentType,'' AS equipmentID,'' as equipmentName,"
							...
							+ " 'Junction' AS elementType,A.JCT_ID  as elementID,'Side B' as frontBackPort,CAST(A.SEQUENCE_NUMBER AS VARCHAR2(10)) as portIndex,'' as portRow,'' as portColumn,'' AS status,'' as equipmentType,'' AS equipmentID,'' as equipmentName,"
							...
							+ " 'DB' AS elementType,A.DB_ID  as elementID,  'Back' as frontBackPort, CAST(A.ROW_COL_INDEX AS VARCHAR2(10)) as portIndex, CAST(A.ROW_NUMBER AS VARCHAR2(10)) as portRow, CAST(A.COLUMN_NUMBER AS VARCHAR2(10)) as columnRow,A.BP_STATUS as status,A.BP_EQUIPMENT as equipmentType,A.BP_EQUIPMENT_ID AS equipmentID,A.BP_EQUIPMENT_NAME as equipmentName,"
					...
					listStrandsUtilization = ((NativeQuery<StrandUtilizationReport>) query).addScalar("strandNo").addScalar("tubeNo").addScalar("locationType").addScalar("locationID").addScalar("locationName").addScalar("elementType").addScalar("elementID").addScalar("longitude").addScalar("latitude").addScalar("frontBackPort").addScalar("portIndex").addScalar("portRow").addScalar("portColumn").addScalar("status").addScalar("equipmentType").addScalar("equipmentID").addScalar("equipmentName")
