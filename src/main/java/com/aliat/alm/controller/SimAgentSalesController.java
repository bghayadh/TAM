...
public class SimAgentSalesController {
	...
	private static final Logger logger = Logger.getLogger(SimAgentSalesController.class.getName());
		...
		if (LoginServices.checkSession(request, response).equals("redirect:/")) {
			...
			if (sessionALM != null && sessionALM.isOpen()) {
				...
				try {
					...
					// Google maps query
					model.addAttribute("agentSalesCountList", mapper.writeValueAsString(sessionALM.createNativeQuery(
					...
					// Grid Query
					List<SimAgentSales> agentReportResults = (List<SimAgentSales>) ((SQLQuery) sessionALM
							.createNativeQuery(
									"SELECT agent_id as \"agentId\" ,full_name as \"fullName\" ,msisdn as \"msisdn\",createdDate as \"startDate\",endDate as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\",coalesce(successCount,0) as \"successCount\" ,coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.createdDate as createdDate,A.endDate as endDate,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount FROM( SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as msisdn ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate ,to_char(b.created_date, 'YYYY-MM-DD') as endDate,a.region_name as region_name ,count(*) as totalCount from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= trunc(sysdate) AND TRUNC(b.created_date) < (trunc(sysdate)) + 1 and b.status !='DEACTIVATED'  AND b.status !='CANCELLED' group by to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )A LEFT JOIN (  SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as secondMSISDN ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate ,to_char(b.created_date, 'YYYY-MM-DD') as endDate,a.region_name as region_name ,count(*) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= trunc(sysdate) AND TRUNC(b.created_date) < (trunc(sysdate)) + 1 and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn  )B ON A.createdDate = B.createdDate and a.msisdn = b.secondMSISDN  )D LEFT JOIN ( SELECT a.agent_id as agent_id1,a.full_name as full_name1  ,a.msisdn as secondMSISDN1 ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate1 ,to_char(b.created_date, 'YYYY-MM-DD') as endDate1,a.region_name as region_name1 ,count(*) as failedCount from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= trunc(sysdate) AND TRUNC(b.created_date) < (trunc(sysdate)) + 1  and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )C ON D.createdDate = C.createdDate1 and D.msisdn = C.secondMSISDN1 )"))
											.addScalar("agentId").addScalar("fullName").addScalar("msisdn")
											.addScalar("startDate").addScalar("endDate").addScalar("regionName")
											.addScalar("totalCount").addScalar("successCount").addScalar("failedCount")
					...
					agentsCountChartQuery = sessionALM.createNativeQuery(
							"select a.full_name,count(*) from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >=  trunc(sysdate) AND b.created_date < (trunc(sysdate)) + 1 and b.status !='DEACTIVATED' and b.status !='CANCELLED'  GROUP BY a.full_name order by a.full_name");

					// 2nd Chart and 3rd chart :x-axis time

					periodicDataQuery = sessionALM.createNativeQuery(
							"select TO_CHAR(b.created_date, 'YYYY/MM/DD') as created_date,  count(*) as totalSimSales from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >=  trunc(sysdate) AND b.created_date < (trunc(sysdate)) + 1 and b.status !='DEACTIVATED' and b.status !='CANCELLED' group by TO_CHAR(created_date, 'YYYY/MM/DD') order by created_date desc");

					// 3-pie chart
					registrationStatusQuery = sessionALM.createNativeQuery(
							"SELECT b.ussd_status, COUNT(b.ussd_status) from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >=  trunc(sysdate) AND b.created_date < (trunc(sysdate)) + 1  group by b.ussd_status");
					...
					chartArrayRegistrationStatus = new ArrayList<Float>(
							PieStatusPerClient(registrationStatusQuery.list()));
	...
	@ResponseBody
	public Map<String, Object> GetSimCardSalesPerAgent(Locale locale, Model model, HttpServletRequest request,
				...
				if (session != null && session.isOpen()) {
					...
					if (finalAgentsNameList != "") {
						...
						for (int i = 0; i < agentsNameList.length; i++) {
							...
							agentSalesCountQuery = session.createNativeQuery(
									"SELECT (SELECT coalesce(count(a.agent_id),0) FROM agent a inner join clients b on a.msisdn = b.agent_number "
											+ "where b.created_date >= :param1 and b.created_date <= :param2 and a.full_name ="
											+ agentName + " and a.region_name LIKE '%" + region
											+ "%' AND b.REGISTRATION_STATUS ='Success' )total_count, "
											+ " agent_id,full_name,latitude,longitude,MSISDN FROM agent where full_name ="
											+ agentName + " ");
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")
							...
							agentSalesCountQuery = session.createNativeQuery(
									"select count(*),a.agent_id,a.full_name,a.latitude,a.longitude,a.msisdn from agent a inner join "
											+ "clients b on a.msisdn = b.agent_number where b.created_date >= :param1 and b.created_date <= :param2 and a.region_name LIKE '%"
											+ region + "%' AND b.REGISTRATION_STATUS ='Success' "
	...
	@ResponseBody
	public Map<String, Object> GenerateAgentSalesReport(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		// logger.info("Welcome home! The client locale is {}.", locale);
					...
					if (finalAgentsNameList == "") {
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								// agentSalesCount ="select a.agent_id as \"agentId\" ,a.full_name as
								// \"fullName\" ,a.msisdn as \"msisdn\" ,to_char(b.created_date, 'YYYY-MM-DD')
								// as \"startDate\" ,to_char(b.created_date, 'YYYY-MM-DD') as \"endDate\"
								// ,a.region_name as \"regionName\" ,count(*) as \"totalCount\",(SELECT
								// successCount FROM(SELECT count(*) as successCount,a.agent_id from agent a
								// inner join clients b on a.msisdn = b.agent_number where TRUNC(
								// b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2 and
								// a.REGION_NAME LIKE '%"+region+"%' and b.status !='DEACTIVATED' and b.status
								// !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' GROUP BY a.agent_id )) as
								// \"successCount\",(SELECT failedCount FROM(SELECT count(*) as
								// failedCount,a.agent_id from agent a inner join clients b on a.msisdn =
								// b.agent_number where TRUNC(b.created_date) >= :param1 AND
								// TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' and b.status
								// !='CANCELLED' and a.REGION_NAME LIKE '%"+region+"%' AND b.REGISTRATION_STATUS
								// ='Failed' GROUP BY a.agent_id)) as \"failedCount\" from agent a inner join
								// clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >=
								// :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND
								// b.status !='CANCELLED' "+
								// " and a.REGION_NAME LIKE '%"+region+"%' group by to_char(b.created_date,
								// 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn order by
								// to_char(b.created_date, 'YYYY-MM-DD') DESC";
								...
								agentSalesCount = " SELECT agent_id as \"agentId\" ,full_name as \"fullName\" ,msisdn as \"msisdn\",createdDate as \"startDate\",endDate as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\",coalesce(successCount,0) as \"successCount\" ,coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.createdDate as createdDate,A.endDate as endDate,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount FROM( "
										...
										+ region
										+ "%'  and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )C ON D.createdDate = C.createdDate1 and D.msisdn = C.secondMSISDN1 ) ORDER BY full_name ASC,createdDate DESC";
							...
							// Daily-Max condition
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\" ,msisdn as \"msisdn\",createdDate as \"startDate\",endDate as \"endDate\" ,region_name as \"regionName\" ,totalCount as \"totalCount\",coalesce(successCount,0) as \"successCount\" ,coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.createdDate as createdDate,A.endDate as endDate,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount FROM ( SELECT agent_id as agent_id , FULL_NAME as full_name ,MSISDN  as msisdn ,created_date  as createdDate ,created_date as endDate ,region_name as region_name ,dailyCount as totalCount "
										...
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' "
										+ " GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn)B ON A.createdDate = B.createdDate and a.msisdn = b.secondMSISDN )D LEFT JOIN (  SELECT a.agent_id as agent_id1,a.full_name as full_name1  ,a.msisdn as secondMSISDN1 ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate1 ,to_char(b.created_date, 'YYYY-MM-DD') as endDate1,a.region_name as region_name1 ,count(*) as failedCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )C ON D.createdDate = C.createdDate1 and D.msisdn = C.secondMSISDN1 ) ORDER BY full_name ASC,createdDate DESC";
							...
							// Daily-Min condition
							else if (StringUtils.equalsIgnoreCase(min, "Min")
							...
							agentSalesCountQuery = session.createNativeQuery(agentSalesCount);
							agentSalesCountQuery.setParameter("param1", startDate);
							agentSalesCountQuery.setParameter("param2", endDate);
						...
						if (StringUtils.equalsIgnoreCase(period, "Accu")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								// agentSalesCount ="select a.agent_id as \"agentId\" ,a.full_name as
								// \"fullName\" ,a.msisdn as \"msisdn\" ,'"+startDate+"' as \"startDate\"
								// ,'"+endDate+"' as \"endDate\" ,a.region_name as \"regionName\" ,count(*) as
								// \"totalCount\" from agent a inner join clients b on a.msisdn = b.agent_number
								// where TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2
								// and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "+
								// " and a.REGION_NAME LIKE '%"+region+"%' group by
								// a.agent_id,a.full_name,a.region_name,a.msisdn order by \"fullName\",
								// \"startDate\" DESC";

								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
										+ startDate + "' as \"startDate\", '" + endDate
										+ "' as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\" ,coalesce(successCount,0) as \"successCount\",coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount "
										...
										+ region + "%' group by a.agent_id,a.full_name,a.region_name,a.msisdn ) A "
										+ " LEFT JOIN ( SELECT a.MSISDN as secondMSISDN,coalesce(count(*),0) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' GROUP BY a.MSISDN) B ON A.msisdn = B.secondMSISDN )D LEFT JOIN( "
										+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN ) ORDER BY \"fullName\" DESC ";
							...
							// Accumulated-Max condition
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
										...
										+ " FROM( SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as msisdn ,a.region_name as region_name ,count(*) as totalCount  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region
										+ "%' group by a.agent_id,a.full_name,a.region_name,a.msisdn HAVING COUNT(a.msisdn)=(SELECT MAX(salesCount) FROM (SELECT a.agent_id,a.full_name,a.msisdn,a.region_name, COUNT(a.msisdn) as salesCount"
										+ " FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' GROUP BY a.agent_id,a.full_name,a.msisdn,a.region_name))  ) A "
										+ " LEFT JOIN ( SELECT a.MSISDN as secondMSISDN,coalesce(count(*),0) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' GROUP BY a.MSISDN) B ON A.msisdn = B.secondMSISDN )D LEFT JOIN( "
										+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
							...
							// Accumulated-Min condition
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
										...
										+ " FROM( SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as msisdn ,a.region_name as region_name ,count(*) as totalCount  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region
										+ "%' group by a.agent_id,a.full_name,a.region_name,a.msisdn HAVING COUNT(a.msisdn)=(SELECT MIN(salesCount) FROM (SELECT a.agent_id,a.full_name,a.msisdn,a.region_name, COUNT(a.msisdn) as salesCount"
										+ " FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' GROUP BY a.agent_id,a.full_name,a.msisdn,a.region_name))  ) A "
										+ " LEFT JOIN ( SELECT a.MSISDN as secondMSISDN,coalesce(count(*),0) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
							...
							// Accumulated-Max-Min condition
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
										...
										+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN )"
										+ " UNION ( "
										+ " SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
										+ startDate + "' as \"startDate\", '" + endDate
										+ "' as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\" ,coalesce(successCount,0) as \"successCount\", coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount "
										+ " FROM( SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as msisdn ,a.region_name as region_name ,count(*) as totalCount  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							for (int i = 0; i < diff; i++) {
								...
								if (StringUtils.equalsIgnoreCase(max, null)
									...
									// agentSalesCount ="select a.agent_id as \"agentId\" ,a.full_name as
									// \"fullName\" ,a.msisdn as \"msisdn\" , '"+startDate+"' as \"startDate\"
									// ,'"+weeklyEndDate+"' as \"endDate\" ,a.region_name as \"regionName\"
									// ,count(*) as \"totalCount\" from agent a inner join clients b on a.msisdn =
									// b.agent_number where TRUNC( b.created_date) >= :param1 AND
									// TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND b.status
									// !='CANCELLED' "+
									// " and a.REGION_NAME LIKE '%"+region+"%' group by
									// a.agent_id,a.full_name,a.region_name,a.msisdn order by \"fullName\" ASC,
									// \"startDate\" DESC";
								}
								...
								if (StringUtils.equalsIgnoreCase(max, "Max")
									...
									agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
											...
											+ " FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region
											+ "%' GROUP BY a.agent_id,a.full_name,a.msisdn,a.region_name))  ) A "
											+ " LEFT JOIN ( SELECT a.MSISDN as secondMSISDN,coalesce(count(*),0) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
											+ region
											+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' GROUP BY a.MSISDN) B ON A.msisdn = B.secondMSISDN )D LEFT JOIN( "
											+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
						...
						if (StringUtils.equalsIgnoreCase(period, "Monthly")) {
							...
							for (int i = 0; i <= diff; i++) {
								Timestamp monthofDate = new Timestamp(
										formatter.parse(formatter.format(month)).getTime());
								...
								Timestamp endOfMonth = new Timestamp(
										formatter.parse(formatter.format(lastDayOfMonth)).getTime());
								...
								if (StringUtils.equalsIgnoreCase(max, null)
									...
									/*
									 * agentSalesCount
									 * ="select a.agent_id as \"agentId\" ,a.full_name as \"fullName\" ,a.msisdn as \"msisdn\" ,'"
									 * +monthofDate+"' as \"startDate\" ,'"
									 * +endOfMonth+"' as \"endDate\" ,a.region_name as \"regionName\" ,count(*) as \"totalCount\" from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
									 * + " and a.REGION_NAME LIKE '%"
									 * +region+"%' group by a.agent_id,a.full_name,a.region_name,a.msisdn order by \"fullName\", \"startDate\" DESC"
									 * ;
									 */
									agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\",'"
											...
											+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
											+ region
											+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN )order by \"fullName\", \"startDate\" DESC ";
					...
					// if AGENT is selected
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								/*
								 * agentSalesCount
								 * ="select a.agent_id as \"agentId\" ,a.full_name as \"fullName\" ,a.msisdn as \"msisdn\" ,to_char(b.created_date, 'YYYY-MM-DD') as \"startDate\" ,to_char(b.created_date, 'YYYY-MM-DD') as \"endDate\" ,a.region_name as \"regionName\" ,count(*) as \"totalCount\" from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
								 * + " and a.full_name IN ("+finalAgentsNameList+")  and a.REGION_NAME LIKE '%"
								 * +region+"%' group by to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn order by to_char(b.created_date, 'YYYY-MM-DD') DESC"
								 * ;
								 */
								agentSalesCount = " SELECT agent_id as \"agentId\" ,full_name as \"fullName\" ,msisdn as \"msisdn\",createdDate as \"startDate\",endDate as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\",coalesce(successCount,0) as \"successCount\" ,coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.createdDate as createdDate,A.endDate as endDate,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount FROM( "
										+ "  SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as msisdn ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate ,to_char(b.created_date, 'YYYY-MM-DD') as endDate,a.region_name as region_name ,count(*) as totalCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED'  AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' and a.full_name IN (" + finalAgentsNameList
										+ ")   group by to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn "
										+ " )A LEFT JOIN (  SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as secondMSISDN ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate ,to_char(b.created_date, 'YYYY-MM-DD') as endDate,a.region_name as region_name ,count(*) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success'  and a.full_name IN ("
										+ finalAgentsNameList
										+ ")  GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn "
										+ " )B ON A.createdDate = B.createdDate and a.msisdn = b.secondMSISDN  )D LEFT JOIN ( SELECT a.agent_id as agent_id1,a.full_name as full_name1  ,a.msisdn as secondMSISDN1 ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate1 ,to_char(b.created_date, 'YYYY-MM-DD') as endDate1,a.region_name as region_name1 ,count(*) as failedCount from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%'  and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed'  and a.full_name IN ("
										+ finalAgentsNameList
										+ ")  GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )C ON D.createdDate = C.createdDate1 and D.msisdn = C.secondMSISDN1 ) ";
							...
							// Daily-Max condition
								...
								agentSalesCount = "SELECT agent_id as \"agentId\" ,full_name as \"fullName\" ,msisdn as \"msisdn\" ,createdDate as \"startDate\" ,endDate as \"endDate\" ,region_name as \"regionName\" ,totalCount \"totalCount\" ,coalesce(successCount,0) as \"successCount\" ,coalesce(failedCount,0) as \"failedCount\" FROM ( "
										...
										+ " WHERE  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' and a.full_name IN (" + finalAgentsNameList
										+ ") GROUP BY to_char(b.created_date, 'YYYY-MM-DD'), a.agent_id,a.full_name,a.msisdn,a.region_name  ORDER BY FULL_NAME ASC,TO_CHAR(b.created_date, 'YYYY-MM-DD') ,Rank) WHERE RANK=1 )A LEFT JOIN( SELECT a.agent_id as agent_id,a.full_name as full_name  ,a.msisdn as secondMSISDN ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate ,to_char(b.created_date, 'YYYY-MM-DD') as endDate,a.region_name as region_name ,count(*) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' "
										+ " and a.FULL_NAME IN (" + finalAgentsNameList
										+ ") GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn )B ON A.createdDate = B.createdDate and a.msisdn = b.secondMSISDN )D LEFT JOIN (  SELECT a.agent_id as agent_id1,a.full_name as full_name1  ,a.msisdn as secondMSISDN1 ,to_char(b.created_date, 'YYYY-MM-DD') as createdDate1 ,to_char(b.created_date, 'YYYY-MM-DD') as endDate1,a.region_name as region_name1 ,count(*) as failedCount from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.FULL_NAME IN ("
										+ finalAgentsNameList
										+ ")  GROUP BY to_char(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.full_name,a.region_name,a.msisdn  )C ON D.createdDate = C.createdDate1 and D.msisdn = C.secondMSISDN1 ) order by  \"startDate\" DESC ";
						...
						if (StringUtils.equalsIgnoreCase(period, "Accu")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								 ...
								 * +endDate+"' as \"endDate\" ,a.region_name as \"regionName\" ,count(*) as \"totalCount\" from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
								 * + " and a.full_name IN ("+finalAgentsNameList+") and a.REGION_NAME LIKE '%"
								 * +region+"%' group by a.agent_id,a.full_name,a.region_name,a.msisdn order by \"fullName\", \"startDate\" DESC"
								 * ;
								 */
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
										...
										+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region
										+ "%' group by a.agent_id,a.full_name,a.region_name,a.msisdn ) A "
										+ " LEFT JOIN ( SELECT a.MSISDN as secondMSISDN,coalesce(count(*),0) as successCount from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Success' and a.full_name IN ("
										+ finalAgentsNameList
										+ ") GROUP BY a.MSISDN ) B ON A.msisdn = B.secondMSISDN )D LEFT JOIN( "
										+ " SELECT coalesce(count(*),0) as failedCount,a.MSISDN as secondMSISDN from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and a.REGION_NAME LIKE '%"
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.full_name IN ("
										+ finalAgentsNameList
										+ ") GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN ) ORDER BY \"fullName\", \"startDate\" DESC ";
							...
							if (StringUtils.equalsIgnoreCase(max, "Max") && StringUtils.equalsIgnoreCase(min, null)) {
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
										...
										+ "FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.FULL_NAME IN ("
										+ finalAgentsNameList + ")  and a.REGION_NAME LIKE '%" + region + "%' "
										...
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.full_name IN ("
										+ finalAgentsNameList
										+ ") GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN ) ORDER BY \"fullName\"  ASC ";
							...
							if (StringUtils.equalsIgnoreCase(max, "Max") && StringUtils.equalsIgnoreCase(min, "Min")) {
								...
								agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
										...
										+ region
										+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.full_name IN ("
										+ finalAgentsNameList + ") GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN )"
										+ " UNION ( "
										+ "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
										+ startDate + "' as \"startDate\", '" + endDate
										+ "' as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\" ,coalesce(successCount,0) as \"successCount\",coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount "
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							for (int i = 0; i < diff; i++) {
								...
								if (StringUtils.equalsIgnoreCase(max, null)
									 ...
									 * ="select a.agent_id as \"agentId\" ,a.full_name as \"fullName\" ,a.msisdn as \"msisdn\" , '"
									 * +startDate+"' as \"startDate\" ,'"
									 * +weeklyEndDate+"' as \"endDate\" ,a.region_name as \"regionName\" ,count(*) as \"totalCount\" from agent a inner join clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
									 * + " and a.full_name IN ("+finalAgentsNameList+") and a.REGION_NAME LIKE '%"
									 * +region+"%' group by a.agent_id,a.full_name,a.region_name,a.msisdn order by \"fullName\" ASC, \"startDate\" DESC"
									 * ;
									 */
									agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
											+ startDate + "' as \"startDate\", '" + weeklyEndDate
								...
								// Weekly-Selected Agent-Minimum-Maximum
									...
									agentSalesCount = "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
											...
											+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.full_name IN ("
											+ finalAgentsNameList
											+ ") GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN )" + " UNION ( "
											+ "SELECT agent_id as \"agentId\",full_name as \"fullName\",msisdn as \"msisdn\", '"
											+ startDate + "' as \"startDate\", '" + weeklyEndDate
											+ "' as \"endDate\",region_name as \"regionName\",totalCount as \"totalCount\" ,coalesce(successCount,0) as \"successCount\",coalesce(failedCount,0) as \"failedCount\" FROM ( (SELECT A.agent_id as agent_id , A.full_name as full_name ,A.msisdn as msisdn ,A.region_name as region_name,A.totalCount as totalCount, coalesce(B.successCount,0) as successCount "
											+ " FROM( SELECT t.agent_id as agent_id, t.FULL_NAME as full_name, t.MSISDN as msisdn, t.region_name as region_name, t.totalSalesCount as totalCount FROM( "
											...
											+ "%' and b.status !='DEACTIVATED' and b.status !='CANCELLED' AND b.REGISTRATION_STATUS ='Failed' and a.full_name IN ("
											+ finalAgentsNameList
											+ ") GROUP BY a.MSISDN )C ON D.msisdn = C.secondMSISDN )"
											+ ") ORDER BY \"fullName\" DESC , \"totalCount\" DESC";
	...
	@ResponseBody
	public Map<String, Object> GenerateCountCharts(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		// logger.info("Welcome home! The client locale is {}.", locale);
		...
		String periodicDataChart = "", agentSalesCountChart = "", registrationStatus = "",
				maxStackedLineChartCount = "", minStackedLineChartCount = "";
		Query agentSalesCountQuery, periodicDataQuery, registrationStatusQuery, maxStackedLineChartQuery,
				minStackedLineChartQuery;
				...
				if (session != null && session.isOpen()) {
					...
					if (finalAgentsNameList == "") {
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")) {
							...
							// Null Max- Null Min
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								agentSalesCountChart = "select a.full_name,count(*) from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' group by a.full_name order by a.full_name ";

								periodicDataChart = "select * from(select TO_CHAR(b.created_date, 'YYYY/MM/DD') as created_date,  count(*) as totalSimSales from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED'  and a.REGION_NAME LIKE '%"
										+ region
										+ "%' group by TO_CHAR(created_date, 'YYYY/MM/DD')) order by created_date DESC";
								...
								agentSalesCountQuery = session.createNativeQuery(agentSalesCountChart);
								agentSalesCountQuery.setParameter("param1", startDate);
								agentSalesCountQuery.setParameter("param2", endDate);

								periodicDataQuery = session.createNativeQuery(periodicDataChart);
								periodicDataQuery.setParameter("param1", startDate);
								periodicDataQuery.setParameter("param2", endDate);

								registrationStatusQuery = session.createNativeQuery(registrationStatus);
							...
							// MAX OR MIN ARE CHECKED
								...
								if (StringUtils.equalsIgnoreCase(max, "Max")
									 ...
									 * ;
									 */
									maxStackedLineChartQuery = session.createNativeQuery(maxStackedLineChartCount);
									maxStackedLineChartQuery.setParameter("param1", startDate);
									maxStackedLineChartQuery.setParameter("param2", endDate);

									minStackedLineChartQuery = session.createNativeQuery(minStackedLineChartCount);
									...
									if (maxStackedLineChartQuery.list().size() > 0) {
										...
										rtn.put("maxStackedandLineCount",
												mapper.writeValueAsString(maxdefPeriodicData));
									} else {
									...
									if (minStackedLineChartQuery.list().size() > 0) {
										...
										rtn.put("minStackedandLineCount",
												mapper.writeValueAsString(mindefPeriodicData));
									} else {
								...
								// Daily-Max OR Daily-Min condition
									...
									if (StringUtils.equalsIgnoreCase(max, "Max")
										...
										// periodicDataChart = "select created_date as created_date,MAX(dailySalesCount)
										// AS maxCount from(SELECT DISTINCT a.agent_id as agent_id ,a.FULL_NAME as
										// FULL_NAME,a.MSISDN as MSISDN, TO_CHAR(b.created_date, 'YYYY-MM-DD') as
										// created_date, COUNT(a.msisdn) as dailySalesCount FROM agent a INNER JOIN
										// CLIENTS b ON a.msisdn =b.agent_number WHERE TRUNC( b.created_date) >= :param1
										// AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND
										// b.status !='CANCELLED' " +
										// " GROUP BY TO_CHAR(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.FULL_NAME
										// ,a.MSISDN order by TO_CHAR(b.created_date, 'YYYY-MM-DD') ASC ) GROUP BY
									...
									// Daily-Min condition
										...
										// periodicDataChart = "select created_date,MIN(dailySalesCount) from(SELECT
										// DISTINCT a.agent_id as agent_id ,a.FULL_NAME as FULL_NAME,a.MSISDN as MSISDN,
										// TO_CHAR(b.created_date, 'YYYY-MM-DD') as created_date, COUNT(a.msisdn) as
										// dailySalesCount FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number
										// WHERE TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2
										// and b.status !='DEACTIVATED' AND b.status !='CANCELLED' " +
										// " GROUP BY TO_CHAR(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.FULL_NAME
										// ,a.MSISDN order by TO_CHAR(b.created_date, 'YYYY-MM-DD') ASC ) GROUP BY
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								periodicDataChart = "select t.totalCount as \"totalCount\",'" + date
										+ "' as \"Date\" from ( SELECT count(a.msisdn) as totalCount from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
										+ region + "%' )t ";
								...
								for (int i = 0; i < diff; i++) {
									...
									salesCountLineCharts = (List<SimAgentCountChartsReport>) ((SQLQuery) periodicDataQuery)
											.addScalar("totalCount").addScalar("Date").setResultTransformer(
													Transformers.aliasToBean(SimAgentCountChartsReport.class))
											.list();
							...
							// Weekly with MAX-MIN
								...
								if (StringUtils.equalsIgnoreCase(max, "Max")
									...
									maxStackedLineChartCount = "select '" + date
											+ "' as \"Date\" ,coalesce(sum(t.salesCount),0)  as \"totalCount\" from ( "
											...
											+ "FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region + "%'  " + "GROUP BY a.full_name order by salesCount desc)"
											+ ") t where RANK = 1 order by '" + date + "' desc";

									minStackedLineChartCount = "select '" + date
											+ "' as \"Date\" ,coalesce(sum(t.salesCount),0)  as \"totalCount\" from ( "
											+ " select salesCount,full_name, RANK() over(order by salesCount asc) as RANK from "
											+ "(SELECT DISTINCT a.FULL_NAME as full_name, COUNT(a.msisdn) as salesCount  "
											+ "FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region + "%'  " + "GROUP BY a.full_name order by salesCount asc)"
											+ ") t where RANK = 1 order by '" + date + "' desc";
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								for (int i = 0; i <= diff; i++) {
									...
									date = (monthofDate.getYear() + 1900) + "/" + (monthofDate.getMonth() + 1) + "/"
											+ (monthofDate.getDate()) + "-" + (monthofDate.getYear() + 1900) + "/"
					...
					// if AGENT is selected
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								agentSalesCountChart = "select a.full_name,count(*) from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.full_name IN ("
										+ finalAgentsNameList + " )  and a.REGION_NAME LIKE '%" + region
										+ "%' group by a.full_name order by a.full_name ";

								periodicDataChart = " select TO_CHAR(b.created_date, 'YYYY/MM/DD') as created_date,  count(*) as totalSimSales from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED' and a.full_name IN ("
										+ finalAgentsNameList + " ) and a.REGION_NAME LIKE '%" + region
										+ "%' group by TO_CHAR(created_date, 'YYYY/MM/DD') order by created_date desc";

								registrationStatus = "SELECT b.ussd_status, COUNT(b.ussd_status) from agent a inner join clients b on a.msisdn=b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and a.full_name IN ("
										+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region
										+ "%'  group by ussd_status";
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								periodicDataChart = "select t.totalCount as \"totalCount\",'" + date
										+ "' as \"Date\" from ( SELECT count(a.msisdn) as totalCount from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED' and a.FULL_NAME IN ("
										+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region + "%' )t ";
							...
							// Weekly-Selected Agent-Maximum OR Minimum
								...
								for (int i = 0; i < diff; i++) {
									...
									if (StringUtils.equalsIgnoreCase(max, "Max")
										...
										// periodicDataChart = "select t.totalCount as \"totalCount\",'"+date+"' as
										// \"Date\" from ( SELECT count(a.msisdn) as totalCount from agent a inner join
										// clients b on a.msisdn = b.agent_number where TRUNC( b.created_date) >=
										// :param1 AND TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' and
										// b.status !='CANCELLED' and a.FULL_NAME ="+agentFullName+" and a.REGION_NAME
										// LIKE '%"+region+"%' )t ";
	...
	@ResponseBody
	public Map<String, Object> GetAllAgentIDAutocomplete(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		// logger.info("Welcome home! The client locale is {}.", locale);
			...
			try {
				...
				if (session != null && session.isOpen()) {
					...
					if (areaID == "" && regionID == "") {
						...
						agentDataQuery = session.createNativeQuery(
								"SELECT  distinct (agent_id),full_name,msisdn FROM agent where UPPER(agent_id) like UPPER(:param1) or UPPER(full_name) like UPPER(:param1) ");

					} else if (areaID != "" && regionID == "") {

						agentDataQuery = session.createNativeQuery(
								"SELECT * from ( SELECT distinct a.agent_id as agent_id,a.full_name as full_name,a.msisdn as msisdn, b.AREA_ID as AREA_ID FROM agent a inner join AGENT_AREAS b on a.agent_id = b.agent_id where UPPER(a.agent_id) like UPPER(:param1) or UPPER(a.full_name) like UPPER(:param1) ) WHERE AREA_ID ='"
										+ areaID + "' ");

					} else if (regionID != "" && areaID == "") {
						agentDataQuery = session.createNativeQuery(
								"SELECT * from ( SELECT distinct a.agent_id as agent_id,a.full_name as full_name,a.msisdn as msisdn, b.REGION_ID as REGION_ID FROM agent a inner join AGENT_REGIONS b on a.agent_id = b.agent_id where UPPER(a.agent_id) like UPPER(:param1) or UPPER(a.full_name) like UPPER(:param1) ) WHERE REGION_ID ='"
										+ regionID + "' ");

					} else if (areaID != "" && regionID != "") {
						agentDataQuery = session.createNativeQuery(
								"SELECT * from ( SELECT distinct a.agent_id as agent_id,a.full_name as full_name,a.msisdn as msisdn, b.REGION_ID as REGION_ID,c.AREA_ID as AREA_ID FROM agent a inner join AGENT_REGIONS b on a.agent_id = b.agent_id inner join AGENT_AREAS c on  b.agent_id = c.agent_id where UPPER(a.agent_id) like UPPER(:param1) or UPPER(a.full_name) like UPPER(:param1) ) WHERE REGION_ID ='"
										+ regionID + "' and AREA_ID ='" + areaID + "'  ");
	...
	@ResponseBody
	public Map<String, Object> FilteringMethodSIM(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		...
		if (StringUtils.equalsIgnoreCase(filterID, "lineChartApplyFilter")
				|| StringUtils.equalsIgnoreCase(filterID, "stackedChartApplyFilter")) {
			...
			if (minVal == null && maxVal == null) {
				...
				stackedLineChartCaseCondition = "(CASE WHEN total_count >= '" + maxVal
						+ "' THEN total_count ELSE 0 END) total_count ";
			} else if (minVal != null && ((StringUtils.equalsIgnoreCase(maxVal, "null")) || maxVal == null)) {

				stackedLineChartCaseCondition = "(CASE WHEN total_count >= '" + minVal
						+ "' THEN total_count ELSE 0 END) total_count ";
			}

			else if (minVal != null && maxVal != null) {

				stackedLineChartCaseCondition = "(CASE WHEN total_count >= '" + minVal + "' AND total_count <= '"
						+ maxVal + "' THEN total_count ELSE 0 END) total_count ";
		...
		if (selectedValue != null) {
			minMaxDecodeCondition = " and " + countOption + " >= DECODE(" + minVal + ",null,0," + minVal + ") AND "
					+ countOption + " <= DECODE('" + maxVal
					+ "','null',(select count(a.agent_id) from agent a inner join clients b on a.msisdn= b.agent_number),"
					+ maxVal + ")";
		...
		if (LoginServices.checkSession(request, response).equals("redirect:/")) {
			...
			try {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								case "countChartApplyFilter":
									applyFilterCount = "SELECT * FROM (select a.full_name,count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
											+ " and a.REGION_NAME LIKE '%" + region + "%' group by a.full_name)"
											+ decodeCondition + "  group by full_name,total_count order by full_name ";
								...
								case "stackedChartApplyFilter":
									...
									applyFilterCount = "SELECT * FROM (select TO_CHAR(b.created_date, 'YYYY/MM/DD') as created_date, count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED'\r\n"
											+ " and a.REGION_NAME LIKE '%" + region
											+ "%' group by TO_CHAR(b.created_date, 'YYYY/MM/DD'))" + decodeCondition
								...
								case "countPieApplyFilter":
									applyFilterCount = "SELECT * FROM (select a.full_name,count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region + "%' group by a.full_name ) group by full_name,total_count ";

									break;

								case "regStatPieApplyFilter":

									applyFilterCount = "SELECT b.ussd_status, COUNT(b.ussd_status) from agent a inner join clients b on a.msisdn=b.agent_number  where  b.created_date >= :param1 AND b.created_date <= :param2 "
											+ regStatusWhereCondition + "group by ussd_status";
								...
								case "countChartApplyFilter":
									...
									applyFilterQuery = session.createNativeQuery(applyFilterCount);
								...
								// if maximum OR minimum is checked
									...
									if (StringUtils.equalsIgnoreCase(max, "Max")
										...
										case "countPieApplyFilter":
											...
											applyFilterCount = "SELECT * FROM (select a.full_name,count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' "
													+ "  and a.REGION_NAME LIKE '%" + region
													+ "%' group by a.full_name)" + decodeCondition
													+ "  group by full_name,total_count order by full_name ";
									...
									if (StringUtils.equalsIgnoreCase(max, null)
										...
										case "lineChartApplyFilter":
											...
											// periodicSalesCount= "select created_date,MIN(dailySalesCount) from(SELECT
											// DISTINCT a.agent_id as agent_id ,a.FULL_NAME as FULL_NAME,a.MSISDN as
											// MSISDN, TO_CHAR(b.created_date, 'YYYY-MM-DD') as created_date,
											// COUNT(a.msisdn) as dailySalesCount FROM agent a INNER JOIN CLIENTS b ON
											// a.msisdn =b.agent_number WHERE TRUNC( b.created_date) >= :param1 AND
											// TRUNC(b.created_date) <= :param2 and b.status !='DEACTIVATED' AND
											// b.status !='CANCELLED' " +
											// " GROUP BY TO_CHAR(b.created_date, 'YYYY-MM-DD'),a.agent_id,a.FULL_NAME
											// ,a.MSISDN order by TO_CHAR(b.created_date, 'YYYY-MM-DD') ASC )
											// "+dailyDecodeCondition+" GROUP BY created_date ORDER BY created_date DESC
											// " ;
											applyFilterCount = "SELECT created_date,sum(salesCount) from( select t.created_date AS created_date ,t.salesCount AS salesCount from ( "
													+ "select salesCount,created_date,full_name, RANK() over(PARTITION BY created_date order by salesCount asc) as RANK from "
													+ "(SELECT DISTINCT a.FULL_NAME as full_name, TO_CHAR(b.created_date, 'YYYY-MM-DD') as created_date, COUNT(a.msisdn) as salesCount  "
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								case "lineChartApplyFilter":
									...
									applyFilterCount = "select t.total_count as \"totalCount\",'" + date
											+ "' as \"Date\" from (select " + stackedLineChartCaseCondition
											+ " from(SELECT coalesce(count(a.msisdn),0) as total_count from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region + "%'))t ";
								...
								case "countPieApplyFilter":
									...
									applyFilterCount = "select a.full_name,count(*) from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.REGION_NAME LIKE '%"
											+ region + "%' group by a.full_name order by a.full_name ";
								...
								if (StringUtils.equalsIgnoreCase(max, "Max")
									...
									case "lineChartApplyFilter":
										...
										for (i = 0; i < diff; i++) {
											...
											maxStackedLineChartQuery = session
													.createNativeQuery(maxStackedLineChartCount);
											maxStackedLineChartQuery.setParameter("param1", weeklyStartDate);
											maxStackedLineChartQuery.setParameter("param2", weeklyEndDate);

											minStackedLineChartQuery = session
													.createNativeQuery(minStackedLineChartCount);
											minStackedLineChartQuery.setParameter("param1", weeklyStartDate);
											minStackedLineChartQuery.setParameter("param2", weeklyEndDate);
								...
								if (StringUtils.equalsIgnoreCase(max, "Max")
									...
									case "lineChartApplyFilter":
										...
										for (i = 0; i <= diff; i++) {
											...
											date = (monthofDate.getYear() + 1900) + "/" + (monthofDate.getMonth() + 1)
													+ "/" + (monthofDate.getDate()) + "-"
													+ (monthofDate.getYear() + 1900) + "/" + (endOfMonth.getMonth() + 1)
													+ "/" + (endOfMonth.getDate());
					...
					// if agent is selected
						...
						if (StringUtils.equalsIgnoreCase(period, "Daily")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								case "countPieApplyFilter":
									applyFilterCount = "SELECT * FROM (select a.agent_id,count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.full_name IN ("
											+ finalAgentsNameList + ")   and a.REGION_NAME LIKE '%" + region
											+ "%' group by a.agent_id ) group by agent_id,total_count ";

									break;

								case "regStatPieApplyFilter":

									applyFilterCount = "SELECT b.ussd_status, COUNT(b.ussd_status) from agent a inner join clients b on a.msisdn=b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 AND b.status !='CANCELLED' and a.full_name IN ("
											+ finalAgentsNameList + ")  and a.REGION_NAME LIKE '%" + region + "%' "
											+ regStatusWhereCondition + "group by ussd_status";
						...
						if (StringUtils.equalsIgnoreCase(period, "Accu")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								case "countPieApplyFilter":
									applyFilterCount = "SELECT * FROM (select a.full_name,count(*) as total_count  from agent a inner join clients b on a.msisdn = b.agent_number where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.full_name IN ("
											+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region
											+ "%' group by a.full_name ) group by full_name,total_count ";

									break;

								case "regStatPieApplyFilter":

									applyFilterCount = "SELECT b.ussd_status, COUNT(b.ussd_status) from agent a inner join clients b on a.msisdn=b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and a.full_name IN ("
											+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region + "%' "
											+ regStatusWhereCondition + "group by ussd_status";
						...
						if (StringUtils.equalsIgnoreCase(period, "Weekly")) {
							...
							if (StringUtils.equalsIgnoreCase(max, null) && StringUtils.equalsIgnoreCase(min, null)) {
								...
								case "lineChartApplyFilter":
									...
									applyFilterCount = "select t.total_count as \"totalCount\",'" + date
											+ "' as \"Date\" from (select " + stackedLineChartCaseCondition
											+ " from(SELECT coalesce(count(a.msisdn),0) as total_count from agent a inner join clients b on a.msisdn = b.agent_number  where b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' and b.status !='CANCELLED' and a.full_name IN ("
											+ finalAgentsNameList + ")  and a.REGION_NAME LIKE '%" + region + "%'))t ";
								...
								case "countPieApplyFilter":
									...
									applyFilterCount = "select a.full_name,count(*) from agent a inner join clients b on a.msisdn = b.agent_number where  b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED' and a.full_name IN ("
											+ finalAgentsNameList + ") and a.REGION_NAME LIKE '%" + region
											+ "%' group by a.full_name order by a.full_name ";
								...
								case "lineChartApplyFilter":
									...
									for (i = 0; i <= diff; i++) {
										...
										date = (monthofDate.getYear() + 1900) + "/" + (monthofDate.getMonth() + 1) + "/"
												...
												+ (endOfMonth.getMonth() + 1) + "/" + (monthofDate.getDate());
				...
				logger.finest("Error in GetAllAgentIDAutocomplete due to \n " + exceptionAsString);
	...
	@ResponseBody
	public Map<String, Object> FilteringMaxMinChart(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		// logger.info("Welcome home! The client locale is {}.", locale);
			...
			try {
				...
				if (session != null && session.isOpen()) {
					...
					if (StringUtils.equalsIgnoreCase(period, "Daily")) {
						...
						case "maxStackedChartApplyFilter":
						case "maxLineChartApplyFilter":
						...
						case "minStackedChartApplyFilter":
						case "minLineChartApplyFilter":
						...
						case "maxStackedChartApplyFilter":
						case "maxLineChartApplyFilter":
							periodicSalesCountQuery = session.createNativeQuery(periodicSalesCount);
					...
					// Weekly period
						...
						case "maxLineChartApplyFilter":
							...
							periodicSalesCount = "select '" + date
									+ "' as \"Date\" ,coalesce(sum(t.salesCount),0)  as \"totalCount\" from ( "
									+ " select salesCount,full_name, RANK() over(order by salesCount desc) as RANK from "
									+ "(SELECT DISTINCT a.FULL_NAME as full_name, COUNT(a.msisdn) as salesCount  "
									...
									+ "GROUP BY a.full_name order by salesCount desc)" + ") t where RANK = 1 order by '"
									+ date + "' desc";

							// periodicSalesCount = "select coalesce(MAX(dailySalesCount),0) as
							// \"totalCount\", '"+date+"' as \"Date\" from(SELECT DISTINCT a.agent_id as
							// agent_id ,a.FULL_NAME as FULL_NAME,a.MSISDN as MSISDN, COUNT(a.msisdn) as
							// dailySalesCount FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number
							// WHERE TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2
							// and b.status !='DEACTIVATED' AND b.status !='CANCELLED' " +
						...
						case "minLineChartApplyFilter":
							...
							periodicSalesCount = "select '" + date
									...
									+ "(SELECT DISTINCT a.FULL_NAME as full_name, COUNT(a.msisdn) as salesCount  "
									+ "FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number WHERE b.created_date >= :param1 AND b.created_date <= :param2 and b.status !='DEACTIVATED' AND b.status !='CANCELLED'   "
									+ "GROUP BY a.full_name order by salesCount asc)" + ") t where RANK = 1 order by '"
									+ date + "' desc";

							// periodicSalesCount = "select coalesce(MIN(dailySalesCount),0) as
							// \"totalCount\", '"+date+"' as \"Date\" from(SELECT DISTINCT a.agent_id as
							// agent_id ,a.FULL_NAME as FULL_NAME,a.MSISDN as MSISDN, COUNT(a.msisdn) as
							// dailySalesCount FROM agent a INNER JOIN CLIENTS b ON a.msisdn =b.agent_number
							// WHERE TRUNC( b.created_date) >= :param1 AND TRUNC(b.created_date) <= :param2
						...
						case "maxLineChartApplyFilter":
							...
							for (int i = 0; i < diff; i++) {
								...
								salesCountLineCharts = (List<SimAgentCountChartsReport>) ((SQLQuery) periodicSalesCountQuery)
										.addScalar("totalCount").addScalar("Date")
										.setResultTransformer(Transformers.aliasToBean(SimAgentCountChartsReport.class))
										.list();
					...
					else if (StringUtils.equalsIgnoreCase(period, "null")
							|| StringUtils.equalsIgnoreCase(period, "Accu")) {
				...
				e.printStackTrace(new PrintWriter(sw));
				exceptionAsString = sw.toString();
				logger.finest("Error in FilteringMaxMinChart due to \n " + exceptionAsString);
	...
	@ResponseBody
	public Map<String, Object> GetAgentAreaAutocomplete(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
			...
			try {
				...
				if (session != null && session.isOpen()) {
					...
					if (agentID == "" && regionID == "") {
						...
						areaListQuery = session.createNativeQuery(
								"SELECT  distinct AREA_ID,AREA_NAME FROM area where UPPER(AREA_NAME)like UPPER(:param1) and REGION_ID = '"
										+ regionID + "' ");
					}

					else if (agentID != "" && regionID == "") {
						areaListQuery = session.createNativeQuery(
								"SELECT distinct AREA_ID,AREA_NAME FROM AGENT_AREAS where UPPER(AREA_NAME)like UPPER(:param1) and AGENT_ID = '"
										+ agentID + "'  ");
					}
					...
					// agentID and regionID are both selected
						...
						areaListQuery = session.createNativeQuery(
								"SELECT * FROM (SELECT distinct a.AREA_ID as AREA_ID,a.AREA_NAME as AREA_NAME,b.REGION_ID as REGION_ID,c.AGENT_ID as AGENT_ID FROM AGENT_AREAS a inner join AGENT_REGIONS b on a.agent_id = b.agent_id inner join AGENT C  on b.agent_id = c.agent_id where UPPER(a.AREA_NAME)like UPPER(:param1)) WHERE REGION_ID = '"
										+ regionID + "' AND AGENT_ID = '" + agentID + "' ");
	...
	@ResponseBody
	public Map<String, Object> GetAgentRegionAutocomplete(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
				...
				if (session != null && session.isOpen()) {
					...
					if (agentID == "" && areaID == "") {
						regionListQuery = session.createNativeQuery(
								"SELECT distinct REGION_ID,REGION_NAME,REGION_CODE FROM REGION where UPPER(REGION_NAME)like UPPER(:param1) ");
					}

					else if (agentID == "" && areaID != "") {
						regionListQuery = session.createNativeQuery(
								"SELECT distinct a.REGION_ID,a.REGION_NAME,a.REGION_CODE FROM region a inner join area b on a.region_id = b.region_id  where UPPER(a.REGION_NAME)like UPPER(:param1) and b.AREA_ID = '"
										+ areaID + "' ");
					}
						...
						regionListQuery = session.createNativeQuery(
								"SELECT distinct REGION_ID,REGION_NAME,REGION_CODE FROM AGENT_REGIONS where UPPER(REGION_NAME)like UPPER(:param1) and AGENT_ID = '"
										+ agentID + "' ");
					}
					// agentID and areaID are both selected
						...
						regionListQuery = session.createNativeQuery(
								"SELECT * FROM (SELECT distinct a.REGION_ID as REGION_ID,a.REGION_NAME as REGION_NAME,a.REGION_CODE as REGION_CODE,b.AREA_ID as AREA_ID,c.AGENT_ID as AGENT_ID FROM AGENT_AREAS b inner join AGENT_REGIONS a on b.agent_id = a.agent_id inner join AGENT C  on a.agent_id = c.agent_id where UPPER(a.REGION_NAME)like UPPER(:param1)) WHERE AREA_ID = '"
										+ areaID + "' AND AGENT_ID = '" + agentID + "' ");
	...
	private JSONArray getPeriodicStackedChartData(List<SimAgentCountChartsReport> countChartList)
			throws JsonProcessingException { // List<Object[]>

		JSONArray totalCountArrFnl = new JSONArray();
		JSONArray obj = new JSONArray(), DateArrFnl = new JSONArray();
		JSONObject json;
	...
	private JSONArray getStackedChartData(List<Object[]> perDataChrtLst) {
		JSONObject json;
		JSONArray totalSimSales = new JSONArray();
