...
*/
	...
	Query q = session.createSQLQuery("SELECT CELL_ID, CELL_NAME FROM (SELECT GCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_2G  UNION (SELECT LCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_3G) UNION (SELECT UCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_4G)) WHERE CELL_ID like :param1");
	q.setString("param1", requestName);
	q.setFirstResult(0);
	q.setMaxResults(40);
	listrequest = q.list();
			   ...
			   //if(!nodePkList.isEmpty() || nodePkList != null) {
			       ...
			       //params.remove(nodePk);
            		 ...
            		 if (StringUtils.equalsIgnoreCase(transactionTableOption, "Ce") || StringUtils.equalsIgnoreCase(transactionTableOption, "CeSN")){
            			 ...
            			 ///cell 
        				 cellStringQuery ="SELECT TO_CHAR(CREATION_DATE, 'DD-MM-YYYY') as CREATION_DATE, TRANS_SOURCE, TRANS_TYPE, TRANS_ID ,CELL_ID, CELL_NAME,  NODE_ID, NODE_NAME, SITE_ID, WARE_NAME FROM (SELECT  NODE_2G.CREATION_DATE AS CREATION_DATE,  NODE_2G.TRANS_SOURCE AS  TRANS_SOURCE, NODE_2G.TRANS_ID AS TRANS_ID , NODE_2G.GCELL_ID AS CELL_ID, NODE_2G.CELLNAME AS CELL_NAME, NODE_2G.TRANS_TYPE AS TRANS_TYPE ,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_2G , NODE_ACTIVE where NODE_2G.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_2G.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3 UNION (SELECT  NODE_3G.CREATION_DATE AS CREATION_DATE,  NODE_3G.TRANS_SOURCE AS  TRANS_SOURCE, NODE_3G.TRANS_ID AS TRANS_ID , NODE_3G.LCELL_ID AS CELL_ID, NODE_3G.CELLNAME AS CELL_NAME, NODE_3G.TRANS_TYPE AS TRANS_TYPE ,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_3G , NODE_ACTIVE where NODE_3G.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_3G.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3) UNION (SELECT  NODE_4G.CREATION_DATE AS CREATION_DATE,  NODE_4G.TRANS_SOURCE AS  TRANS_SOURCE, NODE_4G.TRANS_ID AS TRANS_ID , NODE_4G.UCELL_ID AS CELL_ID, NODE_4G.CELLNAME AS CELL_NAME, NODE_4G.TRANS_TYPE AS TRANS_TYPE,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_4G, NODE_ACTIVE where NODE_4G.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_4G.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3))";
