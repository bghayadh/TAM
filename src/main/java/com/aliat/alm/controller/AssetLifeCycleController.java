...
*/
	...
	Query q = session.createSQLQuery("SELECT CELL_ID, CELL_NAME FROM (SELECT GCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_2GCELL  UNION (SELECT LCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_4GCELL) UNION (SELECT UCELL_ID AS CELL_ID, CELLNAME AS CELL_NAME FROM NODE_3GCELL)) WHERE CELL_ID like :param1");
	q.setString("param1", requestName);
	q.setFirstResult(0);
	q.setMaxResults(40);
	listrequest = q.list();
			   ...
			   //if(!nodePkList.isEmpty() || nodePkList != null) {
			       ...
			       //params.remove(nodePk);
            		 ...
            		 if (StringUtils.equalsIgnoreCase(transactionTableOption, "Ce") || StringUtils.equalsIgnoreCase(transactionTableOption, "CeSN")){
            			 ...
            			 ///cell 
        				 cellStringQuery ="SELECT TO_CHAR(CREATION_DATE, 'DD-MM-YYYY') as CREATION_DATE, TRANS_SOURCE, TRANS_TYPE, TRANS_ID ,CELL_ID, CELL_NAME,  NODE_ID, NODE_NAME, SITE_ID, WARE_NAME FROM (SELECT  NODE_2GCELL.CREATION_DATE AS CREATION_DATE,  NODE_2GCELL.TRANS_SOURCE AS  TRANS_SOURCE, NODE_2GCELL.TRANS_ID AS TRANS_ID , NODE_2GCELL.GCELL_ID AS CELL_ID, NODE_2GCELL.CELLNAME AS CELL_NAME, NODE_2GCELL.TRANS_TYPE AS TRANS_TYPE ,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_2GCELL , NODE_ACTIVE where NODE_2GCELL.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_2GCELL.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3 UNION (SELECT  NODE_4GCELL.CREATION_DATE AS CREATION_DATE,  NODE_4GCELL.TRANS_SOURCE AS  TRANS_SOURCE, NODE_4GCELL.TRANS_ID AS TRANS_ID , NODE_4GCELL.LCELL_ID AS CELL_ID, NODE_4GCELL.CELLNAME AS CELL_NAME, NODE_4GCELL.TRANS_TYPE AS TRANS_TYPE ,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_4GCELL , NODE_ACTIVE where NODE_4GCELL.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_4GCELL.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3) UNION (SELECT  NODE_3GCELL.CREATION_DATE AS CREATION_DATE,  NODE_3GCELL.TRANS_SOURCE AS  TRANS_SOURCE, NODE_3GCELL.TRANS_ID AS TRANS_ID , NODE_3GCELL.UCELL_ID AS CELL_ID, NODE_3GCELL.CELLNAME AS CELL_NAME, NODE_3GCELL.TRANS_TYPE AS TRANS_TYPE,NODE_ACTIVE.NODE_ID AS NODE_ID , NODE_ACTIVE.NODE_NAME AS NODE_NAME, NODE_ACTIVE.SITE_ID AS SITE_ID, NODE_ACTIVE.WARE_NAME AS WARE_NAME FROM NODE_3GCELL, NODE_ACTIVE where NODE_3GCELL.NODE_PK = NODE_ACTIVE.NODE_PK AND NODE_3GCELL.ACTIVE_RECORD = '1' AND NODE_ACTIVE.CREATION_DATE >= :param_1 AND NODE_ACTIVE.UPDATE_DATE <= :param_2 AND NODE_ACTIVE.NODE_PK LIKE :param_3))";
