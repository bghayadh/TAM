			...
			if (session != null && session.isOpen()) {
				...
				try {
					...
					String str = "select AGENT_ID as agentId, FIRST_NAME as agfirstName, LAST_NAME as aglastName, ADDRESS as  "
							...
							+ " from AGENT order by LAST_MODIFIED_DATE DESC";
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (areaid != null && !areaid.equalsIgnoreCase("")
						|| areaname != null && !areaname.equalsIgnoreCase("")) {

					str = str + " and AGENT_ID in (select AGENT_ID from AGENT_AREAS where (AREA_ID)  LIKE upper('%"
							+ areaid + "%')  and upper(AREA_NAME) LIKE upper('%" + areaname + "%'))";
				}
				if (regionid != null && !regionid.equalsIgnoreCase("")
						|| regionname != null && !regionname.equalsIgnoreCase("")) {

					str = str + " and AGENT_ID in (select AGENT_ID from AGENT_REGIONS where (REGION_ID)  LIKE upper('%"
							+ regionid + "%')  and upper(REGION_NAME) LIKE upper('%" + regionname + "%'))";
				}

				if (shopid != null && !shopid.equalsIgnoreCase("")
						|| shopname != null && !shopname.equalsIgnoreCase("")) {

					str = str + " and AGENT_ID in (select AGENT_ID from AGENT_SHOPS where (SHOPS_ID)  LIKE upper('%"
							+ shopid + "%')  and upper(SHOPS_NAME) LIKE upper('%" + shopname + "%'))";
				}
				...
				rtn.put("listAgent", session.createNativeQuery(str).list());
	...
	public String AgentDelete(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (agentID != null) {
					query = session.createNativeQuery("delete AGENT_AREAS  where AGENT_ID ='" + agentID + "'");
					query.executeUpdate();
					query = session.createNativeQuery("delete AGENT_SHOPS  where AGENT_ID='" + agentID + "'");
					query.executeUpdate();
					query = session.createNativeQuery("delete AGENT_REGIONS  where AGENT_ID= '" + agentID + "'");
					query.executeUpdate();
					query = session.createNativeQuery("Delete AGENT where AGENT_ID='" + agentID + "'");
					query.executeUpdate();
					tx.commit();
					rtn = "success";
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (agentId == null) {
					...
					result = form.NavigationNP(session, "AGENT", "AGENT_ID", agentId, "LAST_MODIFIED_DATE", navAction);
					...
					if (StringUtils.equalsIgnoreCase(Agent.getFrontsideStatus(), "1")) {
						if (StringUtils.equalsIgnoreCase(Agent.getAgentFrontId(), "")
								|| Agent.getAgentFrontId() == null) {
					...
					if (StringUtils.equalsIgnoreCase(Agent.getBacksideStatus(), "1")) {
						if (StringUtils.equalsIgnoreCase(Agent.getAgentBackId(), "")
								|| Agent.getAgentBackId() == null) {
							model.addAttribute("AGENT_BACK_ID", "addNew");
						} else {
					...
					query = session.createNativeQuery(
							"select SPEEDCOVERAGEID,TO_NUMBER(COVERAGE_SIGNAL),TO_NUMBER(SPEEDCOVERAGE_LAT),TO_NUMBER(SPEEDCOVERAGE_LNG),TECHNOLOGY,AGENT_NAME,AGENT_NUMBER,CID from SPEED_COVERAGE_TEST "
									...
									+ "and not(SPEEDCOVERAGE_LAT is null or SPEEDCOVERAGE_LAT='null') "
									+ "and not(SPEEDCOVERAGE_LNG is null or SPEEDCOVERAGE_LNG='null')"
									+ "and not(CID is null or CID='null') " + " and agent_number='" + Agent.getMSISDN()
									+ "'" + " and  SPEEDCOVERAGE_DATE between SYSDATE - INTERVAL '1' DAY and SYSDATE"
									+ " ORDER BY SPEEDCOVERAGE_DATE ASC");
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (StringUtils.equalsIgnoreCase(agentId, "")) {
					...
					query = session.createNativeQuery("UPDATE AGENT SET " + "FIRST_NAME='"
							+ request.getParameter("fname") + "'," + "LAST_NAME='" + request.getParameter("lname")
							+ "'," + "DISPLAY_NAME='" + request.getParameter("dName") + "'," + "ADDRESS='"
							+ request.getParameter("address") + "'," + "EMAIL='" + request.getParameter("emaill") + "',"
							+ "MSISDN='" + request.getParameter("MSISDN") + "'," + "LAST_MODIFIED_DATE=SYSDATE,"
							+ "STATUS='" + request.getParameter("status") + "'," + "PIN_CODE='"
							+ request.getParameter("PIN") + "'," + "LONGITUDE='" + request.getParameter("agentLng")
							+ "'," + "LATITUDE='" + request.getParameter("agentLat") + "'," + "FULL_NAME='"
							+ request.getParameter("fullName") + "'," + "SUPERAGENT='"
							+ request.getParameter("superAgentPermission") + "'," + "CAPTURE_SPEED='"
							+ request.getParameter("captureSpeed").charAt(0) + "'," + "SITE_ENGINEER='"
							+ request.getParameter("siteEngineer").charAt(0) + "'," + "RUNNING_INTERVAL='"
							+ request.getParameter("runningInterval") + "'," + "CAPTURE_COVERAGE='"
							+ request.getParameter("captureCoverage").charAt(0) + "'," + "COVERAGE_RUNNING_INTERVAL='"
							+ request.getParameter("coverageRunningInterval") + "'" + " WHERE AGENT_ID='" + agentId
							+ "'");
				...
				if (request.getParameterValues("slctDelShops[]") != null) {
					...
					for (int i = 0; i < request.getParameterValues("slctDelShops[]").length; i++) {
						...
						String shopID = session
								.createNativeQuery("SELECT SHOPS_ID from AGENT_SHOPS where AGENT_SHOPS_ID='"
										+ (request.getParameterValues("slctDelShops[]"))[i] + "'")
								.uniqueResult().toString();

						System.out.println(shopID);
						query = session.createNativeQuery("UPDATE SHOPS SET"
				...
				if (itemParameters.getDictParameter() != null) {
					...
					for (int i = 0; i < itemParameters.getDictParameter().size(); i++) {
						...
						if (StringUtils.equalsIgnoreCase(agentShopsId, "")) {
							...
							synchronized (this) {
								agentShopsId = "AG_SHOPS_" + year + "_"
										+ Integer
												.parseInt(session.createNativeQuery("SELECT AGENT_SHOPS FROM SEQ_TABLE")
														.uniqueResult().toString());
								query = session
										.createNativeQuery("UPDATE SEQ_TABLE SET AGENT_SHOPS = AGENT_SHOPS + 1 ");
								query.executeUpdate();
								session.createNativeQuery("commit").executeUpdate();
							}
						...
						query = session.createNativeQuery("UPDATE SHOPS SET " + " AGENT_ID='" + agentId + "',"
				...
				if (itemParameters.getDictParameterArea() != null) {
					...
					for (int i = 0; i < itemParameters.getDictParameterArea().size(); i++) {
						...
						if (StringUtils.equalsIgnoreCase(agentAreaId, "")) {
							synchronized (this) {
								agentAreaId = "AG_Area_" + year + "_"
										+ Integer
												.parseInt(session.createNativeQuery("SELECT AGENT_AREAS FROM SEQ_TABLE")
														.uniqueResult().toString());
								query = session
										.createNativeQuery("UPDATE SEQ_TABLE SET AGENT_AREAS = AGENT_AREAS + 1 ");
								query.executeUpdate();
								session.createNativeQuery("commit").executeUpdate();
							}
						}
				...
				if (itemParameters.getDictParameterRegion() != null) {
					...
					for (int i = 0; i < itemParameters.getDictParameterRegion().size(); i++) {
						...
						if (StringUtils.equalsIgnoreCase(agentRegionId, "")) {
							synchronized (this) {
								agentRegionId = "AG_REGION_" + year + "_"
										+ Integer.parseInt(
												session.createNativeQuery("SELECT AGENT_REGIONS FROM SEQ_TABLE")
														.uniqueResult().toString());
								query = session
										.createNativeQuery("UPDATE SEQ_TABLE SET AGENT_REGIONS = AGENT_REGIONS + 1 ");
								query.executeUpdate();
								session.createNativeQuery("commit").executeUpdate();
							}
						}
			...
			try {
				...
				query = session.createNativeQuery(
						"select TO_CHAR(SPEEDCOVERAGE_DATE,'YYYY-MM-DD HH24:MI:SS') as startDate ,TO_CHAR(SPEEDCOVERAGE_DATE,'YYYY-MM-DD HH24:MI:SS') as endDate,SPEEDCOVERAGE_LAT,SPEEDCOVERAGE_LNG,COVERAGE_SIGNAL,SPEED_DOWNLOAD,SPEED_UPLOAD"
								...
								+ " and agent_number='" + request.getParameter("agentNumber")
								+ "' and SPEEDCOVERAGE_DATE between TO_DATE('" + StartDate
								+ "','MM/DD/YYYY HH24:MI:SS') and TO_DATE('" + EndDate + "','MM/DD/YYYY HH24:MI:SS')"
								+ " ORDER BY SPEEDCOVERAGE_DATE ASC");
			...
			try {
				...
				query = session.createNativeQuery(
						"SELECT SHOPS_ID,SHOP_NAME,LONGTITUDE,LATITUDE from SHOPS where (UPPER(SHOPS_ID) LIKE UPPER('"
								+ shopID + "')" + " or UPPER(SHOP_NAME) LIKE UPPER('" + shopID + "'))"
								+ " and (SHOPS_ID IN (SELECT SHOPS_ID FROM SHOPS WHERE (AGENT_ID IS NULL) OR (AGENT_ID='"
		...
		if (session != null && session.isOpen()) {
			...
			try {
				for (int i = 0; i < idList.length; i++) {
					...
					query = session.createNativeQuery("delete AGENT_SHOPS  where AGENT_ID='" + idForm + "'");
					query.executeUpdate();

					query = session.createNativeQuery("delete AGENT_REGIONS  where AGENT_ID= '" + idForm + "'");
					query.executeUpdate();
