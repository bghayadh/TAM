...
public class UserController {
	...
	public String userListView(Locale locale, Model model, HttpServletResponse httpResponse,
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				str = "select t.User_Name as Name, t.User_Name as userName,  t.user_email_address as emailAddress "
						+ "from Users_table t order by t.MODIFIED_DATE DESC";
	...
	@ResponseBody
	public Map<String, Object> UserFormSave(Locale locale, Model model, HttpServletRequest request,
			...
			try {
				...
				try {
					...
					if (array != null && array.length != 0) {
						...
						Query getRole = session
								.createSQLQuery("select ROLE_NAME from USER_ROLE where USER_NAME ='" + un + "'");

						List<String> getroleList = getRole.list();
						if (getroleList != null) {
							System.out.println("delete user");
							Query deleteRole = session
									.createSQLQuery("DELETE FROM USER_ROLE WHERE USER_NAME='" + un + "'");
							deleteRole.executeUpdate();
						}
						UserRole userRole = new UserRole();
						System.out.println("the array is " + array[0]);
						for (String role : array) { // insert into user_role (USER_ROLE_ID,ROLE_NAME,USER_NAME) values
													// (USER_ROLE_ID.nextval,'hi','bye')
							Query insertRole = session.createSQLQuery(
									"insert into USER_ROLE (USER_ROLE_ID,ROLE_NAME,USER_NAME) values (USER_ROLE_ID.nextval,'"
											+ role.trim() + "','" + un + "')");
...
//			session.saveOrUpdate(userRole);
	...
	@ResponseBody
	public Map<String, Object> ItemFormDelete(Locale locale, Model model, HttpServletRequest request,
	...
	@RequestMapping(value = "/UserListViewDelete", method = RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> UserListViewDelete(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				UserList = request.getParameterValues("userName[]");
				if (UserList != null) {
					for (int i = 0; i < UserList.length; i++) {
						String username = UserList[i];
						query = session.createQuery("delete User t  where t.userName='" + username + "'");
						query.executeUpdate();
					}
					tx.commit();
				}
			...
			System.out.println("could not connect to DB in UserListViewDelete method");
		}
		return rtn;
	}
	...
	@ResponseBody
	public Map<String, Object> getUserRole(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		// logger.info("Welcome home! The client locale is {}.", locale);
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (globalUserName != null) {
					...
					Query getRole = session.createSQLQuery(
							"select ROLE_NAME from USER_ROLE where USER_NAME ='" + globalUserName + "'");
	...
	public Map<String, Object> GetAllRegionss(Locale locale, Model model, HttpServletRequest request,
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				listSalesManager = session
						.createQuery("SELECT  t.userName, t.fname, t.mname, t.lname from User t where userJobTitle ='"
								+ SalesManager + "'")
						.list();

				String RegionManager = "Region Manager";
				listRegionManager = session
						.createQuery("SELECT  t.userName, t.fname, t.mname, t.lname from User t where userJobTitle ='"
								+ RegionManager + "'")
						.list();

				query = session.createQuery(
						"SELECT  userName, fname, mname, lname from User where  UPPER(userName) like UPPER(:param1) OR UPPER(fname)like UPPER(:param1) OR UPPER(mname)like UPPER(:param1) OR UPPER(lname)like UPPER(:param1)");
				...
				System.out
						.println("/*/*listItSalesManager autocomplete " + mapper.writeValueAsString(listSalesManager));
				System.out
						.println("/*/*listRegionManager autocomplete " + mapper.writeValueAsString(listRegionManager));
	...
	public Map<String, Object> GetAllUser(Locale locale, Model model, HttpServletRequest request,
		...
		if (session != null && session.isOpen()) {
			...
			try {
				query = session
						.createQuery("SELECT t1.userName, t1.fname, t1.mname, t1.lname, t1.emailAddress FROM User t1"
								+ " WHERE LOWER(t1.userName) LIKE LOWER(:User) OR LOWER(t1.fname) "
								+ "LIKE LOWER(:User) OR LOWER(t1.mname) LIKE LOWER(:User) OR LOWER(t1.lname) LIKE LOWER(:User) "
								+ "OR LOWER(t1.emailAddress) LIKE LOWER(:User)");
