...
public class RegionController {
	...
	public String RegionFormView(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				result = form.NavigationNP(session, "Region", "REGION_ID", regionID, "LAST_MODIFICATION_DATE",
						navAction);
				...
				if (region != null) {
					query = session
							.createQuery("from RegionBorder where regionId like :param1 ORDER BY sequence + 0 ASC");
	...
	@ResponseBody
	public Map<String, Object> RegionDelete(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
			...
			try {
				...
				if (idList != null) {
					...
					for (int i = 0; i < idList.length; i++) {
						...
						query = session.createNativeQuery("delete REGION_BORDER where REGION_ID='" + idList[i] + "'");
						query.executeUpdate();

						query = session.createNativeQuery("delete REGION where REGION_ID='" + idList[i] + "'");
						query.executeUpdate();
					}
					tx.commit();
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (StringUtils.equalsIgnoreCase(request.getParameter("type"), "addNew")) {
					synchronized (this) {
						// regionId= "REGION_"+calendar.get(Calendar.YEAR)+"_"
						// +appConfig.getSequenceNbr(33);
						regionId = "REGION_" + calendar.get(Calendar.YEAR) + "_" + Integer.parseInt(
								session.createNativeQuery("SELECT REGION FROM SEQ_TABLE").uniqueResult().toString());
						query = session.createNativeQuery("UPDATE SEQ_TABLE SET REGION = REGION + 1 ");
						query.executeUpdate();
				...
				for (int i = 0; i < itemParameters.getDictParameterRegion().size(); i++) {
					...
					if (StringUtils.equalsIgnoreCase(
							itemParameters.getDictParameterRegion().get(i).get("RegionBorderID"), null)) {
						synchronized (this) {
							// regionBorderID= "REGION_BORDER_ID_"+calendar.get(Calendar.YEAR)+"_"
							// +appConfig.getSequenceNbr(68);
							regionBorderID = "REGION_BORDER_ID_" + calendar.get(Calendar.YEAR) + "_"
									+ Integer.parseInt(session.createNativeQuery("SELECT REGION_BORDER FROM SEQ_TABLE")
											.uniqueResult().toString());
							query = session.createNativeQuery("UPDATE SEQ_TABLE SET REGION_BORDER = REGION_BORDER + 1 ");
							query.executeUpdate();
							session.createNativeQuery("commit").executeUpdate();
						}
					} else {
	...
	public Map<String, Object> GetAllRegions(Locale locale, Model model, HttpServletRequest request) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				query.setParameter("param1", Region);
