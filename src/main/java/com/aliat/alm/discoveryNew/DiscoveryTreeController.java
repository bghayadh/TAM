...
@Controller
public class DiscoveryTreeController {
	@Autowired
	        ...
	        notifications.headerNotifications(session, model);
                 ...
                 String getApproval= request.getParameter("getApproval");
	        		    query = session.createNativeQuery(
	        		    	...
	        		    	if ((toNodes != null && !toNodes.equals("[]")) || (fromNodes != null && !fromNodes.equals("[]"))) {
	        		    	System.out.println(toNodes);
	        		    	query = session.createNativeQuery(
		 	        		    	    "DELETE FROM DISCOVERY_NEW_ITEM_NODE WHERE DNI_ID=:param1"
		 	        		    	);

		 	        		    	query.setParameter("param1", DniID).executeUpdate();
	        		    	
	        		    	toNodes = toNodes.replaceAll("[\\[\\]\"]", ""); // remove brackets and quotes
	        		    	String[] rows = toNodes.split("\\s*,\\s*(?=\\d)"); // split at commas before a digit

	        		    	String[][] nodesArray = new String[rows.length][];
	        		    	for (int i = 0; i < rows.length; i++) {
	        		    	    nodesArray[i] = rows[i].split("\\s*,\\s*");
	        				...
	        				dniNode.setToNodeId(nodesArray[i][0]);
	        				dniNode.setFromNodeName("");
	        				dniNode.setToNodeName(nodesArray[i][1]);
	        				dniNode.setFromNodeType("");
	        				dniNode.setToNodeType(nodesArray[i][2]);
	        				dniNode.setDniId(DniID);
	        				session.saveOrUpdate(dniNode);
	        		    	}
	        		    
	        		    	fromNodes = fromNodes.replaceAll("[\\[\\]\"]", "");
	        		    	String[] frows = fromNodes.split("\\s*,\\s*(?=\\d)");

	        		    	String[][] fromNodesArray = new String[frows.length][];
	        		    	for (int i = 0; i < frows.length; i++) {
	        		    	    fromNodesArray[i] = frows[i].split("\\s*,\\s*");
	        		    	    DiscoveryNewItemNode dniNode = new DiscoveryNewItemNode();
		        				String dnNodeID;
								synchronized (this) {
		        					dnNodeID = "DNI_NODE_" + year + "_" + Integer.parseInt(session.createNativeQuery("SELECT DNI_NODE FROM SEQ_TABLE").uniqueResult().toString());	
		        				...
		        				dniNode.setCreationDate(new Timestamp(System.currentTimeMillis()));
		        				dniNode.setLastModifiedDate(new Timestamp(System.currentTimeMillis()));
		        				dniNode.setFromNodeId(fromNodesArray[i][0]);
		        				dniNode.setToNodeId("");
		        				dniNode.setFromNodeName(fromNodesArray[i][1]);
		        				dniNode.setToNodeName("");
		        				dniNode.setFromNodeType(fromNodesArray[i][2]);
