...
query.executeUpdate();
				...
				if(StringUtils.equalsIgnoreCase(trans_Type, "New Node on New Hardware") || StringUtils.equalsIgnoreCase(trans_Type, "New Hardware on Existed Node") || StringUtils. 
					...
					Manager.ApprovalProjectandAsset(trans_Type, getApproval, dnStatus,AssetRegID, ArCode, PurchaseOrId,itmcode,itmname,WorkOrder,DniID,toSiteID,supplierID,supplierName,towareID,towareName,serialnb,dnRate,itemModel,itemPartNb,toSite,toSerialNumber,toSlot,Site,fromSlot,SiteID, MacAddress,session,tx);
						if(transId != null) {
							Manager.transactionUpdate(transId,trans_Type,getApproval,session,tx);
					}
					}
				}
				...
				if(StringUtils.equalsIgnoreCase(trans_Type, "New Node on New Hardware") || StringUtils.equalsIgnoreCase(trans_Type, "New Hardware on Existed Node") || StringUtils. 
					...
					Manager.ApprovalFinance(trans_Type, getApproval, dnStatus,AssetRegID, ArCode, PurchaseOrId,itmcode,itmname,WorkOrder,DniID,toSiteID,supplierID,supplierName,towareID,towareName,serialnb,dnRate,itemModel,itemPartNb,toSite,toSerialNumber,toSlot,Site,fromSlot,SiteID,MacAddress,session,tx);
						if(transId != null) {
							Manager.transactionUpdate(transId,trans_Type,getApproval,session,tx);
						}
					}
				...
				if(StringUtils.equalsIgnoreCase(trans_Type, "New Node on Existed Hardware") || StringUtils.equalsIgnoreCase(trans_Type, "Transfer from Slot to Slot") || StringUtils.equalsIgnoreCase(trans_Type, "Transfer from Node to Node") 
					...
					Manager.ApprovalOperational(trans_Type, getApproval, dnStatus,AssetRegID, ArCode, PurchaseOrId,itmcode,itmname,WorkOrder,DniID,toSiteID,supplierID,supplierName,towareID,towareName,serialnb,dnRate,itemModel,itemPartNb,toSite,toSerialNumber,toSlot,Site,fromSlot,SiteID, FAR, MacAddress,session,tx);
							if(transId != null) {
								Manager.transactionUpdate(transId,trans_Type,getApproval,session,tx);
							}
						}
...
//zeina
	public  <T> void getMigrationQuery(Session session ,String queryString, List<Object> params, Transaction tx) {
