		...
		if (session != null && session.isOpen()) {
			...
			try {
				List<Object[]> DistBoardDetails = session.createNativeQuery(
						"SELECT DISTINCT A.DB_NAME,A.SITE,A.MAX_CAPACITY,A.DB_LONGITUDE,A.DB_LATITUDE,A.NUM_ROWS,A.NUM_COLUMNS,(SELECT COUNT(B.FP_STATUS) FROM DISTRIBUTION_BOARD_MAPPING B WHERE B.FP_STATUS='Connected' AND B.DB_ID='"
								+ selectedDistBoardContext
								+ "'),(SELECT COUNT(B.BP_STATUS) FROM DISTRIBUTION_BOARD_MAPPING B WHERE B.BP_STATUS='Connected' AND B.DB_ID='"
								+ selectedDistBoardContext
								+ "'),A.CITY, A.SITE_NAME,A.WAREHOUSE,TO_CHAR(A.CREATION_DATE, 'MM/dd/YYYY HH:MI AM'),TO_CHAR(A.LAST_MODIFIED_DATE, 'MM/dd/YYYY HH:MI AM'),DB_INSTALLER ,DB_ENGINEER_NAME ,DB_DEPLOYMENT_TYPE ,DB_ADAPTOR_PANEL_TYPE,DB_TYPE,SERIAL_NUMB,CONTROLLER_ID,CONTROLLER_NAME,ROW_COUNTING, TOTAL_NUM_MODULE, ROW_PER_MODULE, DB_SUBTYPE, IS_SPLITTER, SPLITTER_RATIO, SPLITTER_INPUT_PORT_COUNT FROM DISTRIBUTION_BOARD A WHERE A.DB_ID='"
								+ selectedDistBoardContext + "' ")
						.getResultList();
...
//////////////////////////////77777777777777
	...
	@ResponseBody
	public List<List<Object>> SearchForAuxName(@ModelAttribute ItemParameters itemParameters)
			throws JsonProcessingException {

		List<List<Object>> auxWithName = new ArrayList<>();
			...
			try {
				if (itemParameters.getDictParameter().size() > 0) {
					...
					for (int i = 0; i < itemParameters.getDictParameter().size(); i++) {
						...
						if ("null".equals(AuxName) || AuxName == null) {
							// always add null results
							auxWithName.add(Arrays.asList(longitude, latitude, "", "null", "", ""));
		...
		System.out.println("auxName is " + mapper.writeValueAsString(auxWithName));
		return auxWithName;
	}
				...
				for (Object[] auxData : auxList) {
					...
					if (nearestManhole != null) {
						...
						if (!addedManholeNames.contains(manholeId)) {
							resultManhole.put(auxId, new ArrayList<>(
									Arrays.asList(manholeLng, manholeLat, "", "", manholeId, manholeName)));
					...
					System.out
							.println("After manhole searching, sortedMap is: " + mapper.writeValueAsString(sortedMap));
				}
...
// To be deleted	
	 ...
	 * 
	 * @ResponseBody public Map<String, Object> fiberPathSave(Locale locale, Model
	 * model, @ModelAttribute ItemParameters itemParameters, HttpServletRequest
	 * request, HttpServletResponse response) {
	 * 
	 * Map<String, Object> rtn = new LinkedHashMap<>(); int seq, counter = 0;
	 ...
	 * 
	 * System.out.println("In fiberPathSave New Method"); if (session != null &&
	 * session.isOpen()) { tx = session.beginTransaction();
	 * 
	 * Calendar calendar = new GregorianCalendar(); calendar.setTime(new Date());
	 * int year = calendar.get(Calendar.YEAR);
	 * 
	 * String fiberpathID = request.getParameter("fiberpathID"); FiberCable
	 * fibercable; FiberAuxPoints fiberAuxPoints; FiberTubes fiberTubes;
	 * TubeAuxPoints fiberAuxtubes; FiberStrands fiberStrand; StrandAuxPoints
	 * fiberAuxstrands; String ipAddress = RequestUtils.getIpAddress(request);
	 * String updateModfUser = request.getParameter("updateModfUser");
	 * PhysicalLayerActivity PhyAct = new PhysicalLayerActivity();
	 * 
	 * try {
	 * 
	 * String PhyActID = "PHY_ACT_" + year + "_" + Integer.parseInt(
	 * session.createNativeQuery("SELECT PHY_ACT_ID FROM SEQ_TABLE").uniqueResult().
	 * toString()); query =
	 * session.createNativeQuery("UPDATE SEQ_TABLE SET PHY_ACT_ID = PHY_ACT_ID + 1 "
	 * ); query.executeUpdate();
	 * session.createNativeQuery("commit").executeUpdate();
	 * 
	 * PhyAct.setPhyActID(PhyActID); PhyAct.setScreenName("Fiber Cable");
	 * PhyAct.setUsername(updateModfUser); PhyAct.setUserIP(ipAddress);
	 * PhyAct.setActivityDate(new Timestamp(System.currentTimeMillis())); fibercable
	 * = new FiberCable(); if (StringUtils.equalsIgnoreCase(fiberpathID, "")) {
	 * synchronized (this) { // fiberpathID = "FIBER" + year + "_" +
	 * appConfig.getSeqNbr(51,session); fiberpathID = "FIBER" + year + "_" +
	 * Integer.parseInt(session
	 * .createNativeQuery("SELECT FIBER_CABLE FROM SEQ_TABLE").uniqueResult().
	 * toString()); query = session.
	 * createNativeQuery("UPDATE SEQ_TABLE SET FIBER_CABLE = FIBER_CABLE + 1 ");
	 * query.executeUpdate(); session.createNativeQuery("commit").executeUpdate();
	 * session.flush(); session.clear();
	 * PhyAct.setActivityDescription("Add New Element"); } } else {
	 * PhyAct.setActivityDescription("Edit Existing Element"); }
	 * 
	 * String ItemCodeId = request.getParameter("ItemCodeId"); Float FiberLength =
	 * (float) 0; Float totalDrivingDistance = (float) 0; float totalGeoDistance =
	 * 0; int NumTubes = 0; int NumStrands = 0; String selectedFiberMode = ""; float
	 * lastAuxToDestDistance = 0; float lastAuxToDestDrivDistance = 0; int
	 * auxArraySize = 0;
	 * 
	 * if (request.getParameter("FiberLength") != "") { FiberLength =
	 * Float.parseFloat(request.getParameter("FiberLength")); }
	 * 
	 * if (request.getParameter("totalDrivingDistance") != "") {
	 * totalDrivingDistance =
	 * Float.parseFloat(request.getParameter("totalDrivingDistance")); }
	 * 
	 * if (request.getParameter("totalGeoDistance") != "") { totalGeoDistance =
	 * Float.parseFloat(request.getParameter("totalGeoDistance")); }
	 * 
	 * if (request.getParameter("NumStrands") != "") { NumStrands =
	 * Integer.parseInt(request.getParameter("NumStrands")); }
	 * 
	 * if (request.getParameter("NumTubes") != "") { NumTubes =
	 * Integer.parseInt(request.getParameter("NumTubes")); }
	 * 
	 * if (request.getParameter("selectedFiberMode") != "") { selectedFiberMode =
	 * request.getParameter("selectedFiberMode"); }
	 * 
	 * if (request.getParameter("lastAuxToDestDistance") != "") {
	 * lastAuxToDestDistance =
	 * Float.parseFloat(request.getParameter("lastAuxToDestDistance")); } if
	 * (request.getParameter("lastAuxToDestDrivDistance") != "") {
	 * lastAuxToDestDrivDistance =
	 * Float.parseFloat(request.getParameter("lastAuxToDestDrivDistance")); }
	 * 
	 * String fiberName = request.getParameter("fiberName"); String sourceLng =
	 * request.getParameter("sourceLng"); String sourceLat =
	 * request.getParameter("sourceLat"); String destinationLng =
	 * request.getParameter("destinationLng"); String destinationLat =
	 * request.getParameter("destinationLat"); String srcCity =
	 * request.getParameter("sourceCity"); String dstCity =
	 * request.getParameter("destinationCity"); String source =
	 * request.getParameter("Source"); String destination =
	 * request.getParameter("Destination"); String ProjectId =
	 * request.getParameter("ProjectId"); String Condiut_Id =
	 * request.getParameter("Condiut_Id"); String Condiut_Name =
	 * request.getParameter("Condiut_Name"); String fibertype =
	 * request.getParameter("fibertype"); String fiberdeployment =
	 * request.getParameter("fiberdeployment"); String fibernetlevel =
	 * request.getParameter("fibernetlevel"); String fiberowner =
	 * request.getParameter("fiberowner"); String fiberCableCreatedByUser =
	 * request.getParameter("fiberCableCreatedByUser"); String
	 * fiberCableModifiedByUser = request.getParameter("fiberCableModifiedByUser");
	 * String drawingtype = request.getParameter("drawingType"); String
	 * fiberCableSize = request.getParameter("fiberCableSize"); String
	 * fiberEngineerName = request.getParameter("fiberEngineerName"); String
	 * fiberInstaller = request.getParameter("fiberInstaller"); // related cable
	 * String relatedStrandNb = request.getParameter("relatedStrandNb"); String
	 * relatedStrandColor = request.getParameter("relatedStrandColor"); String
	 * relatedStrandID = request.getParameter("relatedStrandID"); String
	 * relatedStrandName = request.getParameter("relatedStrandName"); String
	 * relatedTubeNb = request.getParameter("relatedTubeNb"); String
	 * relatedTubeColor = request.getParameter("relatedTubeColor"); String
	 * relatedTubeID = request.getParameter("relatedTubeID"); String relatedTubeName
	 * = request.getParameter("relatedTubeName"); String relatedCableID =
	 * request.getParameter("relatedCableID"); String relatedCableName =
	 * request.getParameter("relatedCableName"); String otherLastMileID =
	 * request.getParameter("otherLastMileID"); String otherLastMileName =
	 * request.getParameter("otherLastMileName"); String otherSideLocationID =
	 * request.getParameter("otherSideLocationID"); String otherSideLocationName =
	 * request.getParameter("otherSideLocationName"); String otherSideLocationCity =
	 * request.getParameter("otherSideLocationCity"); String otherSideLocationType =
	 * request.getParameter("otherSideLocationType");
	 * 
	 * // Timestamp lastModifiedDate = new Timestamp(new
	 * Timestamp(System.currentTimeMillis()).getTime());
	 * 
	 * String cableCreatedDate = request.getParameter("cableCreatedDate"); Timestamp
	 * cableCreationDate; if (StringUtils.equalsIgnoreCase(cableCreatedDate, "")) {
	 ...
	 * 
	 * fibercable.setFibercableID(fiberpathID); if (source.contains("WARE") == true)
	 * { fibercable.setSourceWareID(source.split(":")[0]);
	 * fibercable.setSourceID(source.split(":")[2]);
	 * fibercable.setSourceName(source.split(":")[1]); } else if
	 * (source.contains("CUST") == true) { fibercable.setSourceWareID("null");
	 ...
	 * source.contains("DB") == true) { fibercable.setSourceWareID("null");
	 * fibercable.setSourceID(source.split(":")[0]);
	 * fibercable.setSourceName(source.split(":")[1]);
	 * 
	 * } else { fibercable.setSourceWareID("null"); fibercable.setSourceID("null");
	 ...
	 * fibercable.setDestinationName(destination.split(":")[1]); } else if
	 * (destination.contains("CUST") == true) {
	 ...
	 * 
	 * else if (destination.contains("MH") == true || destination.contains("HH") ==
	 * true || destination.contains("DB") == true) {
	 * fibercable.setDestinationWareID("null");
	 * fibercable.setDestinationID(destination.split(":")[0]);
	 * fibercable.setDestinationName(destination.split(":")[1]); } else {
	 * fibercable.setDestinationWareID("null"); fibercable.setDestinationID("null");
	 * fibercable.setDestinationName(destination); }
	 * 
	 * fibercable.setItemcode(ItemCodeId); fibercable.setNbofStrands(NumStrands);
	 * fibercable.setNbofTubes(NumTubes); fibercable.setFiberlength(FiberLength);
	 * fibercable.setConduitID(Condiut_Id); fibercable.setConduitName(Condiut_Name);
	 * fibercable.setSrcLNG(sourceLng); fibercable.setSrcLAT(sourceLat);
	 * fibercable.setDestLNG(destinationLng); fibercable.setDestLAT(destinationLat);
	 * fibercable.setCableMode(selectedFiberMode);
	 * fibercable.setFibercableName(fiberName); fibercable.setSrcCity(srcCity);
	 * fibercable.setDestCity(dstCity); fibercable.setProjectID(ProjectId);
	 ...
	 * 
	 * session.saveOrUpdate(fibercable); session.flush(); session.clear();
	 * PhyAct.setElementID(fiberpathID); session.saveOrUpdate(PhyAct);
	 * 
	 * Query updateMappingJctSideAQuery = session
	 * .createNativeQuery("UPDATE JUNCTION_MAPPING SET FIBER_NAME_SIDE_A = '" +
	 * fiberName + "' WHERE FIBER_ID_SIDE_A = '" + fiberpathID + "' ");
	 * updateMappingJctSideAQuery.executeUpdate();
	 * 
	 * Query updateMappingJctSideBQuery = session
	 * .createNativeQuery("UPDATE JUNCTION_MAPPING SET FIBER_NAME_SIDE_B = '" +
	 * fiberName + "' WHERE FIBER_ID_SIDE_B = '" + fiberpathID + "' ");
	 * updateMappingJctSideBQuery.executeUpdate();
	 * 
	 * session.flush(); session.clear();
	 * 
	 * String fiberAuxFlag = request.getParameter("fiberAuxFlag"); if
	 * (StringUtils.equalsIgnoreCase(fiberAuxFlag, "opened") ||
	 * StringUtils.equalsIgnoreCase(fiberAuxFlag, "new cable")) {
	 * 
	 * System.out.println("Inside " + fiberAuxFlag); query =
	 * session.createNativeQuery(
	 * "delete from FIBER_AUXILIARY_POINTS where FIBER_CABLE_ID='" + fiberpathID +
	 * "'"); query.executeUpdate();
	 * 
	 * if (itemParameters.getDictParameter().size() > 0) { if
	 * (itemParameters.getDictParameter().size() > 1500) { auxArraySize = 1500; }
	 * else { auxArraySize = itemParameters.getDictParameter().size(); }
	 * 
	 * for (int i = 0; i < auxArraySize; i++) { String aux_ID; synchronized (this) {
	 * // String aux_ID = "AUXILIARY_PT_" + year + "_" + //
	 * appConfig.getSeqNbr(53,session); aux_ID = "AUXILIARY_PT_" + year + "_" +
	 * Integer.parseInt(
	 * session.createNativeQuery("SELECT FIBER_CABLE_AUX FROM SEQ_TABLE")
	 * .uniqueResult().toString()); query = session.createNativeQuery(
	 ...
	 * 
	 * driving_distance =
	 * itemParameters.getDictParameter().get(i).get("driving_distance") == "" ? 0 :
	 * itemParameters.getDictParameter().get(i).get("driving_distance") == null ? 0
	 * : StringUtils.equalsIgnoreCase(
	 * itemParameters.getDictParameter().get(i).get("driving_distance"), "null") ? 0
	 * : Double.parseDouble(itemParameters.getDictParameter()
	 * .get(i).get("driving_distance")); geo_distance =
	 * itemParameters.getDictParameter().get(i).get("geo_distance") == "" ? 0 :
	 ...
	 * 
	 * fiberAuxPoints.setAuxID(aux_ID); fiberAuxPoints.setFibercableID(fiberpathID);
	 * fiberAuxPoints.setLong(
	 * Double.parseDouble(itemParameters.getDictParameter().get(i).get(
	 * "aux_Longitude"))); fiberAuxPoints.setLat(
	 * Double.parseDouble(itemParameters.getDictParameter().get(i).get(
	 * "aux_Latitude"))); fiberAuxPoints.setDistancefromsource(Double
	 * .parseDouble(itemParameters.getDictParameter().get(i).get(
	 * "distance_From_Source"))); // fiberAuxPoints.setAuxName( //
	 * itemParameters.getDictParameter().get(i).get("aux_Name"));
	 * 
	 * if (itemParameters.getDictParameter().get(i).get("aux_Name").contains("WARE")
	 * == true) { fiberAuxPoints.setWareID(
	 ...
	 * .contains("Auxiliary_Point") == true) { fiberAuxPoints.setWareID("null");
	 * fiberAuxPoints.setAuxPointID("null"); if
	 * (itemParameters.getDictParameter().get(i).get("aux_Name")
	 * .contains("AUXILIARY_PT") == true) { fiberAuxPoints.setAuxPointName(
	 * itemParameters.getDictParameter().get(i).get("aux_Name").split(":")[1]); }
	 * else { fiberAuxPoints
	 * .setAuxPointName(itemParameters.getDictParameter().get(i).get("aux_Name")); }
	 * } else if
	 * (itemParameters.getDictParameter().get(i).get("aux_Name").contains("MH") ==
	 * true ||
	 * itemParameters.getDictParameter().get(i).get("aux_Name").contains("HH") ==
	 * true || itemParameters.getDictParameter().get(i).get("aux_Name")
	 * .contains("DB") == true) { fiberAuxPoints.setWareID("null");
	 * fiberAuxPoints.setAuxPointID(
	 * itemParameters.getDictParameter().get(i).get("aux_Name").split(":")[0]);
	 * fiberAuxPoints.setAuxPointName(
	 * itemParameters.getDictParameter().get(i).get("aux_Name").split(":")[1]);
	 * 
	 * } else { fiberAuxPoints.setWareID("null");
	 * fiberAuxPoints.setAuxPointID("null"); String AuxName = (String)
	 * itemParameters.getDictParameter().get(i).get("aux_Name");
	 * fiberAuxPoints.setAuxPointName(AuxName);
	 * 
	 * } fiberAuxPoints.setCreationDate(cableCreationDate);
	 * fiberAuxPoints.setLastModifieddate(lastModifiedDate); fiberAuxPoints
	 ...
	 * 
	 * counter++; if (counter % 50 == 0) { session.flush(); session.clear(); } } } }
	 * 
	 ...
	 * 
	 * query = session
	 * .createNativeQuery("delete from FIBER_DUCT where FIBER_PATH_ID='" +
	 * fiberpathID + "'"); query.executeUpdate();
	 * 
	 * counter = 0; for (Map<String, String> ductMap :
	 * itemParameters.getDictParameterFiberDuct()) {
	 * 
	 * seq = ((Number)
	 * session.createNativeQuery("SELECT FIBER_DUCT_SEQ.NEXTVAL FROM DUAL")
	 * .uniqueResult()).intValue();
	 * 
	 * String fiberDuctId = "FIBERDUCT_" + year + "_" + seq;
	 * 
	 * FiberDuct fiberDuct = new FiberDuct(); fiberDuct.setFiberDuctId(fiberDuctId);
	 * fiberDuct.setFiberPathId(fiberpathID);
	 * 
	 * // Sequence if (ductMap.get("sequence") != null &&
	 * !ductMap.get("sequence").isEmpty()) {
	 * fiberDuct.setSequenceNo(NumericUtils.parseInteger(ductMap.get("sequence")));
	 * }
	 * 
	 * // Duct Info fiberDuct.setDuctId(ductMap.get("ductId"));
	 * fiberDuct.setDuctName(ductMap.get("ductName"));
	 * 
	 * // From fiberDuct.setFromSequence(ductMap.get("fromSequence"));
	 ...
	 * 
	 * // To fiberDuct.setToSequence(ductMap.get("toSequence"));
	 ...
	 * 
	 * if (ductMap.get("geoDistance") != null &&
	 * !ductMap.get("geoDistance").isEmpty()) {
	 * fiberDuct.setGeoDistanceKm(NumericUtils.parseDouble(ductMap.get("geoDistance"
	 * ))); } session.saveOrUpdate(fiberDuct);
	 * System.out.println("After saving for fiberDuct"); counter++; if (counter % 50
	 ...
	 * AccessCableJunction accessCableJunction; String access_Junc_ID = ""; if
	 * (itemParameters.getDictParameterJunct().size() > 0) {
	 * 
	 * for (int i = 0; i < itemParameters.getDictParameterJunct().size(); i++) {
	 * accessCableJunction = new AccessCableJunction(); if
	 * (StringUtils.equalsIgnoreCase(itemParameters.getDictParameterJunct().get(i).
	 * get("junctId"), "") || StringUtils.equalsIgnoreCase(
	 * itemParameters.getDictParameterJunct().get(i).get("junctId"), null)) {
	 * synchronized (this) { access_Junc_ID = "ACCESS_JUNCTION_PORT_" + year + "_" +
	 * Integer.parseInt(
	 * session.createNativeQuery("SELECT ACCESS_JUNCTIONS FROM SEQ_TABLE")
	 * .uniqueResult().toString()); query = session.createNativeQuery(
	 * "UPDATE SEQ_TABLE SET ACCESS_JUNCTIONS = ACCESS_JUNCTIONS + 1 ");
	 * query.executeUpdate(); session.createNativeQuery("commit").executeUpdate();
	 * System.out.println("the jun port is " + access_Junc_ID); } } else {
	 * access_Junc_ID =
	 * itemParameters.getDictParameterJunct().get(i).get("junctId"); } //
	 * System.out.println(itemParameters.getDictParameterJunct().get(i).get(
	 * "rowColIndex"));
	 * 
	 * accessCableJunction.setJunctionRowID(access_Junc_ID);
	 * accessCableJunction.setFibercableID(fiberpathID);
	 * 
	 * accessCableJunction.setJunctionID(itemParameters.getDictParameterJunct().get(
	 * i).get("junID")); accessCableJunction
	 * .setJunctionName(itemParameters.getDictParameterJunct().get(i).get("junName")
	 * );
	 * 
	 * session.saveOrUpdate(accessCableJunction); session.flush(); session.clear();
	 * }
	 * 
	 * }
	 * 
	 * query =
	 * session.createNativeQuery("delete from FIBER_TUBES where FIBER_CABLE_ID='" +
	 * fiberpathID + "'"); query.executeUpdate(); query = session
	 * .createNativeQuery("delete from FIBER_STRANDS where FIBER_CABLE_ID='" +
	 * fiberpathID + "'"); query.executeUpdate(); query = session.createNativeQuery(
	 * "delete from TUBE_AUXILIARY_POINTS where FIBER_CABLE_ID='" + fiberpathID +
	 * "'"); query.executeUpdate(); query = session.createNativeQuery(
	 * "delete from STRAND_AUXILIARY_POINTS where FIBER_CABLE_ID='" + fiberpathID +
	 * "'"); query.executeUpdate();
	 * 
	 * session.flush(); session.clear();
	 * 
	 * String strandSource = ""; String strandDestination = ""; String strand_name =
	 * ""; String strandowner = ""; String stranddeployment = ""; String strandtype
	 * = "";
	 * 
	 * String tubeId = "";
	 * 
	 ...
	 * "") { strandSource =
	 * itemParameters.getDictParameterStrands().get(i).get("strandSource"); }
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("strandDestination")
	 * != "") { strandDestination =
	 * itemParameters.getDictParameterStrands().get(i).get("strandDestination"); }
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("tubeId") != "") {
	 * tubeId = itemParameters.getDictParameterStrands().get(i).get("tubeId"); } if
	 * (itemParameters.getDictParameterStrands().get(i).get("StrandName") != "") {
	 * strand_name =
	 * itemParameters.getDictParameterStrands().get(i).get("StrandName"); }
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("strandtype") != "")
	 * { strandtype =
	 * itemParameters.getDictParameterStrands().get(i).get("strandtype"); }
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("stranddeployment")
	 * != "") { stranddeployment =
	 * itemParameters.getDictParameterStrands().get(i).get("stranddeployment"); }
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("strandowner") != "")
	 * { strandowner =
	 * itemParameters.getDictParameterStrands().get(i).get("strandowner"); }
	 * System.out.print("****************the tube is " + tubeId);
	 * 
	 * String sourceWareId = "", sourceId = "", sourceName = ""; if
	 * (strandSource.contains("WARE") == true) { sourceWareId =
	 * strandSource.split(":")[0]; sourceId = strandSource.split(":")[2]; sourceName
	 * = strandSource.split(":")[1]; } else if (strandSource.contains("CUST") ==
	 * true) { sourceWareId = "null"; sourceId = strandSource.split(":")[0];
	 * sourceName = strandSource.split(":")[1]; } else if
	 * (strandSource.contains("MH") == true || strandSource.contains("HH") == true
	 * || strandSource.contains("DB") == true) { sourceWareId = "null"; sourceId =
	 * strandSource.split(":")[0]; sourceName = strandSource.split(":")[1]; } else {
	 * sourceWareId = "null"; sourceId = "null"; sourceName = strandSource; }
	 * 
	 * String dstWareId = "", dstId = "", dstName = ""; if
	 * (strandDestination.contains("WARE") == true) { dstWareId =
	 * strandDestination.split(":")[0]; dstId = strandDestination.split(":")[2];
	 * dstName = strandDestination.split(":")[1]; } else if
	 * (strandDestination.contains("CUST") == true) { dstWareId = "null"; dstId =
	 * strandDestination.split(":")[0]; dstName = strandDestination.split(":")[1]; }
	 * else if (strandDestination.contains("MH") == true ||
	 * strandDestination.contains("HH") == true || strandDestination.contains("DB")
	 * == true) { dstWareId = "null"; dstId = strandDestination.split(":")[0];
	 * dstName = strandDestination.split(":")[1]; } else { dstWareId = "null"; dstId
	 * = "null"; dstName = strandDestination; }
	 * 
	 * float totalDrivDist = 0; float totalGeoDist = 0; float lastAuxToDestDist = 0;
	 * float lastAuxToDestDrivDist = 0;
	 * 
	 * if (itemParameters.getDictParameterStrands().get(i).get("totalGeoDistance")
	 * != "") { totalGeoDist = Float
	 * .parseFloat(itemParameters.getDictParameterStrands().get(i).get(
	 * "totalGeoDistance")); } if
	 * (itemParameters.getDictParameterStrands().get(i).get("totalDrivDistance") !=
	 * "") { totalDrivDist = Float
	 * .parseFloat(itemParameters.getDictParameterStrands().get(i).get(
	 * "totalDrivDistance")); } if
	 * (itemParameters.getDictParameterStrands().get(i).get("distanceLstAuxToDest")
	 * != "") { lastAuxToDestDist = Float.parseFloat(
	 * itemParameters.getDictParameterStrands().get(i).get("distanceLstAuxToDest"));
	 * } if (itemParameters.getDictParameterStrands().get(i).get(
	 * "drivDistanceLstAuxToDest") != "") { lastAuxToDestDrivDist =
	 * Float.parseFloat( itemParameters.getDictParameterStrands().get(i).get(
	 * "drivDistanceLstAuxToDest")); }
	 * 
	 * fiberStrand = new FiberStrands();
	 * 
	 * fiberStrand.setStrandID(itemParameters.getDictParameterStrands().get(i).get(
	 * "strandId")); fiberStrand.setFibercableID(fiberpathID);
	 * fiberStrand.setTubeID(tubeId); fiberStrand
	 * .setSrcLong(itemParameters.getDictParameterStrands().get(i).get(
	 * "strandSource_Longitude"));
	 * fiberStrand.setSrcLat(itemParameters.getDictParameterStrands().get(i).get(
	 * "strandSource_Latitude")); fiberStrand.setDestLong(
	 * itemParameters.getDictParameterStrands().get(i).get(
	 * "strandDestination_Longitude")); fiberStrand.setDestLat(
	 * itemParameters.getDictParameterStrands().get(i).get(
	 * "strandDestination_Latitude")); fiberStrand.setSourceWareId(sourceWareId);
	 * fiberStrand.setSourceId(sourceId); fiberStrand.setSourceName(sourceName);
	 * fiberStrand.setDestinationWareId(dstWareId);
	 * fiberStrand.setDestinationId(dstId); fiberStrand.setDestinationName(dstName);
	 ...
	 * fiberStrand.setTotalGeoDist(totalGeoDist); fiberStrand.setStrandlength(
	 * Double.parseDouble(itemParameters.getDictParameterStrands().get(i).get(
	 * "strandLength")));
	 * fiberStrand.setDrawingtype(itemParameters.getDictParameterStrands().get(i).
	 * get("drawingType")); fiberStrand.setLastAuxToDestDistance(lastAuxToDestDist);
	 * fiberStrand.setLastAuxToDestDrivDistance(lastAuxToDestDrivDist);
	 * fiberStrand.setSrcCity(itemParameters.getDictParameterStrands().get(i).get(
	 * "SourceCity"));
	 * fiberStrand.setDestCity(itemParameters.getDictParameterStrands().get(i).get(
	 * "DestCity"));
	 * fiberStrand.setStrandNumber(itemParameters.getDictParameterStrands().get(i).
	 * get("strandNumber"));
	 * fiberStrand.setStrandColor(itemParameters.getDictParameterStrands().get(i).
	 * get("strandColor"));
	 * 
	 * session.saveOrUpdate(fiberStrand); session.flush(); session.clear(); }
	 * 
	 * String tubeSource = ""; String tubeowner = ""; String tubedeployment = "";
	 * String tubenetlevel = ""; String tubetype = ""; String tubeDestination = "";
	 * String tubeIdArray[] = new
	 * String[itemParameters.getDictParameterTubes().size()]; ArrayList<String[]>
	 * tubeIdArrayList = new ArrayList<String[]>(); List<Object[]> strandsOfTubes;
	 * 
	 ...
	 * "") { tubeDestination =
	 * itemParameters.getDictParameterTubes().get(i).get("tubeDestination");
	 * 
	 * } if (itemParameters.getDictParameterTubes().get(i).get("tubeSource") != "")
	 * { tubeSource =
	 * itemParameters.getDictParameterTubes().get(i).get("tubeSource"); }
	 * 
	 ...
	 * "") { tubedeployment =
	 * itemParameters.getDictParameterTubes().get(i).get("tubedeployment"); }
	 * 
	 * if (itemParameters.getDictParameterTubes().get(i).get("tubenetlevel") != "")
	 * { tubenetlevel =
	 * itemParameters.getDictParameterTubes().get(i).get("tubenetlevel"); }
	 * 
	 ...
	 * 
	 * String sourceWareId = "", sourceId = "", sourceName = ""; if
	 * (tubeSource.contains("WARE") == true) { sourceWareId =
	 * tubeSource.split(":")[0]; sourceId = tubeSource.split(":")[2]; sourceName =
	 * tubeSource.split(":")[1]; } else if (tubeSource.contains("CUST") == true) {
	 * sourceWareId = "null"; sourceId = tubeSource.split(":")[0]; sourceName =
	 * tubeSource.split(":")[1]; } else if (tubeSource.contains("MH") == true ||
	 * tubeSource.contains("HH") == true || tubeSource.contains("DB") == true) {
	 * sourceWareId = "null"; sourceId = tubeSource.split(":")[0]; sourceName =
	 * tubeSource.split(":")[1]; } else { sourceWareId = "null"; sourceId = "null";
	 * sourceName = tubeSource; }
	 * 
	 * String dstWareId = "", dstId = "", dstName = ""; if
	 * (tubeDestination.contains("WARE") == true) { dstWareId =
	 * tubeDestination.split(":")[0]; dstId = tubeDestination.split(":")[2]; dstName
	 * = tubeDestination.split(":")[1]; } else if (tubeDestination.contains("CUST")
	 * == true) { dstWareId = "null"; dstId = tubeDestination.split(":")[0]; dstName
	 * = tubeDestination.split(":")[1]; } else if (tubeDestination.contains("MH") ==
	 * true || tubeDestination.contains("HH") == true ||
	 * tubeDestination.contains("DB") == true) { dstWareId = "null"; dstId =
	 * tubeDestination.split(":")[0]; dstName = tubeDestination.split(":")[1]; }
	 * else { dstWareId = "null"; dstId = "null"; dstName = tubeDestination; }
	 * 
	 ...
	 * "") { totalGeoDist = Float
	 * .parseFloat(itemParameters.getDictParameterTubes().get(i).get(
	 * "totalGeoDistance")); } if
	 * (itemParameters.getDictParameterTubes().get(i).get("totalDrivDistance") !=
	 * "") { totalDrivDist = Float
	 * .parseFloat(itemParameters.getDictParameterTubes().get(i).get(
	 * "totalDrivDistance")); } if
	 * (itemParameters.getDictParameterTubes().get(i).get("distanceLstAuxToDest") !=
	 * "") { lastAuxToDestDist = Float
	 * .parseFloat(itemParameters.getDictParameterTubes().get(i).get(
	 * "distanceLstAuxToDest")); } if
	 * (itemParameters.getDictParameterTubes().get(i).get(
	 * "drivDistanceLstAuxToDest") != "") { lastAuxToDestDrivDist =
	 * Float.parseFloat(
	 ...
	 * 
	 * fiberTubes.setTubeID(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeId")); fiberTubes.setFibercableID(fiberpathID);
	 * fiberTubes.setSrcLong(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeSource_Longitude"));
	 * fiberTubes.setSrcLat(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeSource_Latitude")); fiberTubes.setDestLong(
	 * itemParameters.getDictParameterTubes().get(i).get("tubeDestination_Longitude"
	 * )); fiberTubes .setDestLat(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeDestination_Latitude")); fiberTubes.setSourceWareId(sourceWareId);
	 * fiberTubes.setSourceId(sourceId); fiberTubes.setSourceName(sourceName);
	 * fiberTubes.setDestinationWareId(dstWareId);
	 * fiberTubes.setDestinationId(dstId); fiberTubes.setDestinationName(dstName);
	 * fiberTubes.setTubeName(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeName"));
	 * fiberTubes.setSrcCity(itemParameters.getDictParameterTubes().get(i).get(
	 * "SourceCity"));
	 * fiberTubes.setDestCity(itemParameters.getDictParameterTubes().get(i).get(
	 * "DestCity")); fiberTubes.setTubeType(tubetype);
	 ...
	 * fiberTubes.setDrawingtype(itemParameters.getDictParameterTubes().get(i).get(
	 * "drawingType")); fiberTubes.setLastAuxToDestDistance(lastAuxToDestDist);
	 * fiberTubes.setLastAuxToDestDrivDistance(lastAuxToDestDrivDist);
	 * fiberTubes.setTubelength(
	 * Double.parseDouble(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeLength")));
	 * fiberTubes.setTubeNumber(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeNumber"));
	 * fiberTubes.setTubeColor(itemParameters.getDictParameterTubes().get(i).get(
	 * "tubeColor")); session.saveOrUpdate(fiberTubes); session.flush();
	 * session.clear();
	 * 
	 ...
	 * itemParameters.getDictParameterTubes().get(i).get("tubeDestination_Longitude"
	 * ),
	 * itemParameters.getDictParameterTubes().get(i).get("tubeDestination_Latitude")
	 * , sourceWareId, sourceId, sourceName, dstWareId, dstId, dstName,
	 * strandsInTubeCount, fiberpathID,
	 ...
	 * + fiberpathID + "'") .getResultList();
	 * 
	 * ////////////////////////////////////////// beginning of auxiliary tubes
	 * insert
	 * 
	 * if (itemParameters.getdictParameterTubesAux().size() > 1500) { auxArraySize =
	 * 1500; } else { auxArraySize =
	 * itemParameters.getdictParameterTubesAux().size(); }
	 * 
	 * for (int i = 0; i < auxArraySize; i++) {
	 * 
	 * fiberAuxtubes = new TubeAuxPoints(); String auxTube_ID;
	 * 
	 * synchronized (this) { // String auxTube_ID = "AUXILIARY_TUBE_PT_" + year +
	 * "_" + // appConfig.getSeqNbr(56,session); auxTube_ID = "AUXILIARY_TUBE_PT_" +
	 * year + "_" + Integer.parseInt(session
	 * .createNativeQuery("SELECT FIBER_TUBE_AUX FROM SEQ_TABLE").uniqueResult().
	 * toString()); query = session.
	 * createNativeQuery("UPDATE SEQ_TABLE SET FIBER_TUBE_AUX = FIBER_TUBE_AUX + 1 "
	 * ); query.executeUpdate();
	 * session.createNativeQuery("commit").executeUpdate(); session.flush();
	 * session.clear(); }
	 * 
	 * Double drivingDistance = itemParameters.getdictParameterTubesAux().get(i)
	 * .get("drivingDistance") == "" ? 0 :
	 * itemParameters.getdictParameterTubesAux().get(i).get("drivingDistance") ==
	 * null ? 0 :
	 * StringUtils.equalsIgnoreCase(itemParameters.getdictParameterTubesAux()
	 * .get(i).get("drivingDistance"), "null") ? 0 : Double.parseDouble(
	 * itemParameters.getdictParameterTubesAux().get(i) .get("drivingDistance"));
	 * Double geoDistance =
	 * itemParameters.getdictParameterTubesAux().get(i).get("geoDistance") == "" ? 0
	 * : itemParameters.getdictParameterTubesAux().get(i).get("geoDistance") == null
	 * ? 0 : StringUtils.equalsIgnoreCase(
	 * itemParameters.getdictParameterTubesAux().get(i).get("geoDistance"), "null")
	 * ? 0 : Double.parseDouble(itemParameters.getdictParameterTubesAux()
	 * .get(i).get("geoDistance"));
	 * 
	 * fiberAuxtubes.setAuxID(auxTube_ID);
	 * fiberAuxtubes.setFibercableID(fiberpathID);
	 * fiberAuxtubes.setTubeID(itemParameters.getdictParameterTubesAux().get(i).get(
	 * "tubeId")); fiberAuxtubes.setLong(
	 * Double.parseDouble(itemParameters.getdictParameterTubesAux().get(i).get(
	 * "aux_Longitude"))); fiberAuxtubes.setLat(
	 * Double.parseDouble(itemParameters.getdictParameterTubesAux().get(i).get(
	 * "aux_Latitude"))); fiberAuxtubes.setDistancefromsource(Double
	 * .parseDouble(itemParameters.getdictParameterTubesAux().get(i).get(
	 * "distance_From_Source")));
	 * 
	 * if
	 * (itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").contains(
	 * "WARE") == true) {
	 * 
	 ...
	 * .contains("Auxiliary_Point") == true) { fiberAuxtubes.setWareID("null");
	 * fiberAuxtubes.setAuxPointID("null"); if
	 * (itemParameters.getdictParameterTubesAux().get(i).get("aux_Name")
	 * .contains("AUXILIARY_TUBE_PT") == true) { fiberAuxtubes.setAuxPointName(
	 * itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").split(":")[1
	 * ]); } else { fiberAuxtubes
	 * .setAuxPointName(itemParameters.getdictParameterTubesAux().get(i).get(
	 * "aux_Name")); } } else if
	 * (itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").contains(
	 * "MH") == true ||
	 * itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").contains(
	 * "HH") == true ||
	 * itemParameters.getdictParameterTubesAux().get(i).get("aux_Name")
	 * .contains("DB") == true) { fiberAuxtubes.setWareID("null");
	 * fiberAuxtubes.setAuxPointID(
	 * itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").split(":")[0
	 * ]); fiberAuxtubes.setAuxPointName(
	 * itemParameters.getdictParameterTubesAux().get(i).get("aux_Name").split(":")[1
	 * ]); } else { fiberAuxtubes.setWareID("null");
	 * fiberAuxtubes.setAuxPointID("null");
	 * fiberAuxtubes.setAuxPointName(itemParameters.getdictParameterTubesAux().get(i
	 * ).get("aux_Name")); }
	 * 
	 * fiberAuxtubes.setCreationDate(cableCreationDate);
	 * fiberAuxtubes.setLastModifieddate(lastModifiedDate);
	 * fiberAuxtubes.setSeqSorting(itemParameters.getdictParameterTubesAux().get(i).
	 * get("seqSorting")); fiberAuxtubes.setDrivingDist(drivingDistance);
	 * fiberAuxtubes.setGeoDist(geoDistance);
	 * 
	 * session.saveOrUpdate(fiberAuxtubes); session.flush(); session.clear();
	 * 
	 * }
	 * 
	 * if (itemParameters.getDictParameterStrandsAux().size() > 1500) { auxArraySize
	 * = 1500; } else { auxArraySize =
	 * itemParameters.getDictParameterStrandsAux().size(); }
	 * 
	 * for (int i = 0; i < auxArraySize; i++) { String auxStrand_ID; synchronized
	 * (this) { // String auxStrand_ID = "AUXILIARY_STRAND_PT_" + year + "_" + //
	 * appConfig.getSeqNbr(57,session); auxStrand_ID = "AUXILIARY_STRAND_PT_" + year
	 * + "_" + Integer.parseInt(session
	 * .createNativeQuery("SELECT FIBER_STRAND_AUX FROM SEQ_TABLE").uniqueResult().
	 * toString()); query = session
	 * .createNativeQuery("UPDATE SEQ_TABLE SET FIBER_STRAND_AUX = FIBER_STRAND_AUX + 1 "
	 * ); query.executeUpdate();
	 * session.createNativeQuery("commit").executeUpdate(); session.flush();
	 * session.clear(); } Double drivingDistance =
	 * itemParameters.getDictParameterStrandsAux().get(i) .get("drivingDistance") ==
	 * "" ? 0 :
	 * itemParameters.getDictParameterStrandsAux().get(i).get("drivingDistance") ==
	 * null ? 0 :
	 ...
	 * 0 : itemParameters.getDictParameterStrandsAux().get(i).get("geoDistance") ==
	 * null ? 0 : StringUtils.equalsIgnoreCase(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("geoDistance"),
	 * "null") ? 0 : Double.parseDouble(itemParameters.getDictParameterStrandsAux()
	 * .get(i).get("geoDistance"));
	 * 
	 * fiberAuxstrands = new StrandAuxPoints();
	 * fiberAuxstrands.setAuxID(auxStrand_ID);
	 * fiberAuxstrands.setFibercableID(fiberpathID);
	 * fiberAuxstrands.setStrandID(itemParameters.getDictParameterStrandsAux().get(i
	 * ).get("strandId")); fiberAuxstrands.setLong(Double
	 * .parseDouble(itemParameters.getDictParameterStrandsAux().get(i).get(
	 * "aux_Longitude"))); fiberAuxstrands.setLat(
	 * Double.parseDouble(itemParameters.getDictParameterStrandsAux().get(i).get(
	 * "aux_Latitude"))); fiberAuxstrands.setDistancefromsource(100.0);
	 * 
	 * if
	 * (itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").contains(
	 * "WARE") == true) {
	 * 
	 * fiberAuxstrands.setWareID(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [0]); fiberAuxstrands.setAuxPointID(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [2]); fiberAuxstrands.setAuxPointName(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [1]); } else if
	 * (itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name")
	 * .contains("Auxiliary_Point") == true) { fiberAuxstrands.setWareID("null");
	 * fiberAuxstrands.setAuxPointID("null"); if
	 * (itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name")
	 * .contains("AUXILIARY_STRAND_PT") == true) { fiberAuxstrands.setAuxPointName(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [1]); } else { fiberAuxstrands.setAuxPointName(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name")); } } else
	 * if
	 * (itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").contains(
	 * "MH") == true ||
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").contains(
	 * "HH") == true ||
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name")
	 * .contains("DB") == true) { fiberAuxstrands.setWareID("null");
	 * fiberAuxstrands.setAuxPointID(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [0]); fiberAuxstrands.setAuxPointName(
	 * itemParameters.getDictParameterStrandsAux().get(i).get("aux_Name").split(":")
	 * [1]); } else { fiberAuxstrands.setWareID("null");
	 * fiberAuxstrands.setAuxPointID("null"); fiberAuxstrands
	 * .setAuxPointName(itemParameters.getDictParameterStrandsAux().get(i).get(
	 * "aux_Name")); }
	 * 
	 * fiberAuxstrands.setCreationDate(cableCreationDate);
	 * fiberAuxstrands.setLastModifieddate(lastModifiedDate);
	 * fiberAuxstrands.setSeqSorting(itemParameters.getDictParameterStrandsAux().get
	 * (i).get("seqSorting")); fiberAuxstrands.setDrivingDist(drivingDistance);
	 * fiberAuxstrands.setGeoDist(geoDistance);
	 * 
	 * session.saveOrUpdate(fiberAuxstrands); session.flush(); session.clear();
	 * 
	 * }
	 * 
	 ...
	 * + fiberpathID + "' ORDER BY B.SEQ_SORTING ASC") .getResultList();
	 * 
	 * rtn.put("FiberPathId", fiberpathID); rtn.put("tubeIdArrayList",
	 * tubeIdArrayList); rtn.put("strandsOfTubes", strandsOfTubes);
	 * rtn.put("fiberAux", fiberAux);
	 * 
	 * rtn.put("tubeIdArray", tubeIdArray); rtn.put("fiberTubes", fiberTubesList);
	 * rtn.put("fiberStrands", fiberStrands); session.flush(); session.clear();
	 * tx.commit();
	 * 
	 * } catch (Exception e) { tx.rollback(); sw = new StringWriter();
	 ...
	 * rtn.put("FiberPathId", null); rtn.put("tubeIdArrayList", null);
	 * rtn.put("strandsOfTubes", null); rtn.put("fiberAux", null);
	 * 
	 * rtn.put("tubeIdArray", null); rtn.put("fiberTubes", null);
					...
					if (itemParameters.getDictParameter().size() > 0) {
						...
						for (int i = 0; i < itemParameters.getDictParameter().size(); i++) {
							...
							distributionBoardMapping.setBackKitSerialNum(
									itemParameters.getDictParameter().get(i).get("backKitSerialNum") != ""
											? itemParameters.getDictParameter().get(i).get("backKitSerialNum")
											: "");
							distributionBoardMapping.setFarKitSerialNum(
									itemParameters.getDictParameter().get(i).get("farKitSerialNum") != ""
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if ("1".equals(readFiber)) {
					fiberList = session.createNativeQuery(
							"SELECT SOURCE_LNG,SOURCE_LAT,DESTINATION_LNG,DESTINATION_LAT,A.FIBER_CABLE_ID,A.SOURCE_WARE_ID,A.SOURCE_ID,A.SOURCE_NAME,A.DESTINATION_WARE_ID,A.DESTINATION_ID,A.DESTINATION_NAME,(SELECT COUNT(*) FROM FIBER_TUBES B WHERE B.FIBER_CABLE_ID=A.FIBER_CABLE_ID),(SELECT COUNT(*) FROM FIBER_STRANDS C WHERE C.FIBER_CABLE_ID=A.FIBER_CABLE_ID),FIBER_CABLE_NAME,PROJECT_ID,SOURCE_CITY,DESTINATION_CITY,NUMBER_OF_TUBES,NUMBER_OF_STRANDS,LENGTH,DRAWING_TYPE,FIBER_NETWORK_LEVEL,FIBER_OWNER,(select B.FIBER_COLOR_OWNER from FIBER_OWNER_COLOR B WHERE B.FIBER_OWNER=A.FIBER_OWNER) AS FIBER_CABLE_COLOR FROM FIBER_CABLES A WHERE A.PROJECT_ID='CurrentPhysicalLayer' ORDER BY A.LAST_MODIFIED_DATE DESC")
							.getResultList();
				...
				try {
					List<Object[]> junctionDetails = session.createNativeQuery(
							"SELECT DISTINCT A.JUNCTION_NAME,A.JUNCTION_ID,A.PHYSICAL_LAYER_ID,A.PHYSICAL_LAYER_NAME,A.LONGITUDE,A.LATITUDE,A.JUNCTION_NUMBER,A.CAPACITY,A.CITY,(SELECT COUNT(DISTINCT B.STRAND_ID_SIDE_A) FROM JUNCTION_MAPPING B WHERE B.STRAND_ID_SIDE_A!='null' AND B.JCT_ID='"
									...
									+ "'),(SELECT COUNT(DISTINCT B.STRAND_ID_SIDE_B) FROM JUNCTION_MAPPING B WHERE B.STRAND_ID_SIDE_B !='null' AND B.JCT_ID='"
									+ JunctionID
									+ "'), TO_CHAR(A.CREATION_DATE, 'MM/dd/YYYY HH:MI AM'),TO_CHAR(A.LAST_MODIFIED_DATE, 'MM/dd/YYYY HH:MI AM'),OWNER,JUNC_INSTALLER,JUNC_ENGINEER_NAME, JUNCTION_TYPE FROM JUNCTION A WHERE A.JUNCTION_ID='"
									+ JunctionID + "' ")
							.getResultList();
...
// to be deleted
	...
	/*
	 * private String getIpAddress(HttpServletRequest request) { String ipAddress =
	 * request.getHeader("X-Forwarded-For"); if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("Proxy-Client-IP"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("WL-Proxy-Client-IP"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_X_FORWARDED_FOR"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_X_FORWARDED"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_X_CLUSTER_CLIENT_IP"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_CLIENT_IP"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_FORWARDED_FOR"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getHeader("HTTP_FORWARDED"); } if (ipAddress == null ||
	 * ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) { ipAddress =
	 * request.getRemoteAddr(); }
	 * 
	 * // Handle IPv6 local address if (ipAddress.equals("0:0:0:0:0:0:0:1") ||
	 * ipAddress.equals("::1")) { ipAddress = "127.0.0.1"; // This is the IPv4
	 * loopback address }
	 * 
	 * // In some cases, IP addresses may come in IPv6 format, so you might need to
	 * // convert it to IPv4 if (ipAddress != null && ipAddress.contains(":")) {
