...
public class StrandUtilizationReportController {
	...
	@ResponseBody
	public Map<String, Object> getFiberCableDetails(Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		...
		if (LoginServices.checkSession(request, response).equals("redirect:/")) {
			...
			try {
				...
				if (session != null && session.isOpen()) {
					...
					System.out.println("listStrandsUtilization is" + mapper.writeValueAsString(listStrandsUtilization));
