...
public class Mobile_SIM_Reg {
	...
	public ResponseEntity<String> getRegions() {
		...
		try {
			...
			data = mapper.writeValueAsString(em.createNativeQuery("SELECT REGION_ID FROM REGION ").getResultList() + ":"
					+ em.createNativeQuery("SELECT REGION_NAME FROM REGION ORDER BY REGION_ID ASC").getResultList());
	...
	public ResponseEntity<Map<Object, Object>> getRegions_Areas() {
		...
		try {
			...
			regions = em.createNativeQuery("SELECT REGION_NAME FROM REGION ORDER BY REGION_ID ASC").getResultList();
			data = mapper.writeValueAsString(
					em.createNativeQuery("SELECT REGION_ID FROM REGION ").getResultList() + ":" + regions);
	...
	public ResponseEntity<String> savingAgent(@RequestBody MobileAgentAPI agentRegistration) {
		...
		try {
			...
			msisdn = agentRegistration.getMsisdn();
			if (msisdn != null && !msisdn.equalsIgnoreCase("")) {
				query = em.createNativeQuery("select MSISDN from Agent where MSISDN='" + msisdn + "'");
				...
				if (query.getSingleResult() != null) {
					...
					synchronized (this) {
						...
						agentId = "AG_" + year + "_" + Integer.parseInt(
								em.createNativeQuery("SELECT AGENT FROM SEQ_TABLE").getSingleResult().toString());
						query = em.createNativeQuery("UPDATE SEQ_TABLE SET AGENT = AGENT + 1 ");
						query.executeUpdate();
	...
	public ResponseEntity<String> updateAgentPhotosStatus(@RequestBody String agentphotostatus) {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			query = em.createNativeQuery("update AGENT set " + vcolname + "=" + val1
		...
		try {
			...
			if (agentRegistration.getMsisdn() != null && agentRegistration.getPinCode() != null) {
				...
				if (query.getResultList().size() > 0) {
					...
					} else if (query.getSingleResult().equals("Deactivated")) {
						data = "The MSISDN had been Deactivated.";
					} else if (query.getSingleResult().equals("In Progress")) {
						data = "The MSISDN is not Activated yet.";
					} else if (query.getSingleResult().equals("Cancelled")) {
	...
	public ResponseEntity<String> getStatisticalSummary(@RequestBody ClientListViewAPI client) {
		...
		try {
			emf = Persistence.createEntityManagerFactory("persistence");
			em = emf.createEntityManager();
			query = em.createNativeQuery("SELECT " + "nvl(sum(case when CREATED_DATE  >= SysDate-30 AND AGENT_NUMBER='"
					...
					+ "as timestamp) AND AGENT_NUMBER='" + agentNumber + "' then 1 else 0 end),0)" + " FROM CLIENTS");

			data = mapper.writeValueAsString(query.getResultList());
	...
	public ResponseEntity<Map<String, Object>> getPendingPicCounter(
		...
		try {
			...
			if (testingspeedcoverage.getAgentNumber() != null
				...
				rtn.put("AgentData", mapper.writeValueAsString(query.getResultList()));
				if (result.get(0)[1].equals('0') && result.get(0)[2].equals('0') && result.get(0)[4].equals('0')) {
					...
					synchronized (this) {
						// agentId = "AG_" + year + "_" + appConfig.getSequenceNbr(42);
						speedCoverageId = "TEST_" + year + "_"
								+ Integer.parseInt(em.createNativeQuery("SELECT SPEED_COVERAGE FROM SEQ_TABLE")
										.getSingleResult().toString());
						query = em.createNativeQuery("UPDATE SEQ_TABLE SET SPEED_COVERAGE = SPEED_COVERAGE + 1 ");
						query.executeUpdate();
						em.createNativeQuery("commit").executeUpdate();
					}
					...
					query = em.createNativeQuery(
							"insert into speed_coverage_test (SPEEDCOVERAGEID,SPEEDCOVERAGE_DATE,SPEEDCOVERAGE_LAT,SPEEDCOVERAGE_LNG,COVERAGE_SIGNAL,SPEED_DOWNLOAD,"
									...
									+ testingspeedcoverage.getSpeedDown() + "','" + testingspeedcoverage.getSpeedUp()
									...
									+ testingspeedcoverage.getMnc() + "','" + testingspeedcoverage.getAppVersion()
									+ "')");
					...
					if (data.equalsIgnoreCase("Saved")) {
						...
						query = em.createNativeQuery("update agent set capture_speed ='0' where msisdn = "
	...
	public ResponseEntity<Boolean> getAgentStatus(@RequestBody MobileAgentAPI agentRegistration) {
		...
		try {
			...
			if (agentRegistration.getMsisdn() != null) {
				msisdn = agentRegistration.getMsisdn().toString();
				listItem = em.createNativeQuery("select STATUS from Agent where MSISDN='" + msisdn + "'")
						.getSingleResult().toString();
	...
	public ResponseEntity<Map<String, Object>> getPendingPictures(@RequestBody ClientListViewAPI client) {
		...
		try {
			...
			clientAPI = em.createNativeQuery(
	...
	public ResponseEntity<String> updateClientPhotosStatus(@RequestBody ClientListViewAPI client) {
		...
		try {
			...
			query = em.createNativeQuery("update CLIENTS set " + vcolname
	...
	public ResponseEntity<String> deleteClient(@RequestBody ClientListViewAPI client) {
		...
		try {
			...
			query = em.createNativeQuery("Delete CLIENTS where CLIENT_ID='" + clientid + "'");
			query.executeUpdate();
			data = "deleted";
			em.getTransaction().commit();
		} catch (Exception e) {
			...
			if (clientid.equalsIgnoreCase("0")) {
				...
				synchronized (this) {
					...
					clientid = "CLT_" + year + "_" + Integer.parseInt(
							em.createNativeQuery("SELECT CLIENTS FROM SEQ_TABLE").getSingleResult().toString());
					query = em.createNativeQuery("UPDATE SEQ_TABLE SET CLIENTS = CLIENTS + 1 ");
					query.executeUpdate();
					em.createNativeQuery("commit").executeUpdate();
				}
				CreationDate = new Timestamp(new Timestamp(System.currentTimeMillis()).getTime());
	...
	public ResponseEntity<String> UpdateUSSDStatus(@RequestBody MobileClientAPI client) {
		...
		try {
			...
			query = em
					.createNativeQuery("UPDATE CLIENTS SET LAST_MODIFIED_DATE=SYSDATE,STATUS='SENT_USSD', USSD_STATUS='"
	...
	public ResponseEntity<String> UpdateClientStatus(@RequestBody String clienttest) {
		...
		try {
			...
			query = em.createNativeQuery("UPDATE CLIENTS" + " SET " + "LAST_MODIFIED_DATE=SYSDATE, RESPONSE_CODE='"
					+ response_code + "'," + "RESPONSE_MESSAGE='" + response_message + "'," + "REGISTRATION_STATUS='"
	...
	public ResponseEntity<String> UpdateAgent(@RequestBody MobileAgentAPI agentRegistration) {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			query = em.createNativeQuery("update AGENT set LAST_MODIFIED_DATE=SYSDATE ,FIRST_NAME='"
					...
					+ agentRegistration.getAddress() + "',EMAIL='" + agentRegistration.getEmail() + "',REGION_NAME='"
					+ agentRegistration.getRegionName() + "',REGION_ID='" + agentRegistration.getRegionID()
					+ "',AGENT_FRONT_ID='" + agentRegistration.getAgentfrontID() + "',AGENT_BACK_ID='"
					+ agentRegistration.getAgentbackID() + "',LONGITUDE='" + agentRegistration.getLongitude()
					+ "',LATITUDE='" + agentRegistration.getLatitude() + "' Where MSISDN='"
					+ agentRegistration.getMsisdn() + "' and PIN_CODE='" + agentRegistration.getPinCode() + "'");
			query.executeUpdate();
			data = "UPDATED";
			em.getTransaction().commit();
	...
	public ResponseEntity<String> SimRegistrationAPIWeb(@RequestBody SimRegistrationAPI sim) {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			query = em.createNativeQuery("Select COUNT(*) FROM CLIENTS WHERE CLIENT_ID_NUMBER = '" + sim.getIdNumber()
	...
	public ResponseEntity<List<SimRegReportAPI>> getSimRegReport(@RequestBody SimRegReportAPI simreport) {
		...
		try {
			...
			simReport = em.createNativeQuery(str).getResultList();
	...
	public void UpdateClientStatusfromALM(String registartionid, String clientid, String status, String response_code,
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			String ussdstatus = em.createNativeQuery("SELECT STATUS FROM CLIENTS where CLIENT_ID='" + clientid + "'")
					.getSingleResult().toString();
			if (ussdstatus.equalsIgnoreCase("SENT_USSD")) {
				clientstatus = "SENT_USSD";
			}
			query = em.createNativeQuery("UPDATE CLIENTS SET LAST_MODIFIED_DATE=SYSDATE, REGISTRATION_ID ='"
					+ registartionid + "' ,RESPONSE_CODE='" + response_code + "',RESPONSE_MESSAGE='" + response_message
					+ "',REGISTRATION_STATUS='" + status + "',STATUS='" + clientstatus + "' where CLIENT_ID='"
					+ clientid + "'");
			query.executeUpdate();
	...
	public String UpdatetkashStatusfromALM(String tkashregistartionid, String clientid, String status,
		...
		try {
			...
			if (!tkashststus.equalsIgnoreCase("Success")) {
				query = em.createNativeQuery("UPDATE CLIENTS SET LAST_MODIFIED_DATE=SYSDATE, TKASHREGISTER_ID ='"
						+ tkashregistartionid + "',TKASH_RESPONSE_CODE='" + response_code + "',TKASH_RESPONSE_MESSAGE='"
						+ response_message + "',TKASH_REGISTRATION_STATUS='" + status + "' where CLIENT_ID='" + clientid
						+ "'");
				query.executeUpdate();
				...
				query = em.createNativeQuery(
						"insert into TKASHREG_LOG (REGISTRATIONDATE,CLIENT_ID,SIM_REG_STATUS,RESPONSE_CODE,RESPONSE_MESSAGE,AGENT_NUMBER,AGENT_NAME,MOBILE_NUMBER )values (sysdate,'"
								+ clientid + "','" + status + "','" + response_code + "','" + response_message + "','"
								+ agentmsisdn + "',(SELECT AGENT.FULL_NAME FROM AGENT where MSISDN='" + agentmsisdn
								+ "'),'" + msisdn + "')");
				query.executeUpdate();
				em.getTransaction().commit();
			} else {
	...
	public ResponseEntity<String> AirtimeAPIWeb(@RequestBody String rechargeData) throws Exception {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			String agentname = em.createNativeQuery("SELECT FULL_NAME FROM AGENT where MSISDN='" + agentNumber + "'")
					.getSingleResult().toString();
	...
	public ResponseEntity<Map<String, Object>> getClientchargeListView(
		...
		try {
			...
			chargeList = em.createNativeQuery("SELECT row_num,MOBILE_NUMBER,AMOUNT,RESPONSE_MESSAGE FROM "
	...
	public ResponseEntity<Map<String, Object>> getShopsData(@RequestBody ShopsListViewAPI shopslist) {
		...
		try {
			...
			agentID = em.createNativeQuery("SELECT AGENT_ID FROM AGENT WHERE MSISDN='" + agentNumber + "'")
					.getSingleResult().toString();
			...
			query = em.createNativeQuery(
					"SELECT * FROM (select ROW_NUMBER() OVER (ORDER BY LAST_MODIFIED_DATE DESC) row_num,SHOPS_ID as shopID,SHOP_NAME as shopName,OWNER as owner,ADDRESS as address,AGENT_ID as agentID FROM SHOPS "
							...
							+ " order by LAST_MODIFIED_DATE DESC) T WHERE row_num >= '" + vfrom + "' AND row_num <='"
							+ vto + "'");

			shopsdata = query.getResultList();
	...
	public ResponseEntity<String> getRegionsAutoComplete() throws JsonProcessingException {
		...
		try {
			emf = Persistence.createEntityManagerFactory("persistence");
			em = emf.createEntityManager();
			query = em.createNativeQuery("SELECT REGION_ID,REGION_NAME,REGION_CODE FROM REGION ");
	...
	public ResponseEntity<String> SavingorUpdatingShops(@RequestBody ShopsAPI shops) {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			agentID = em.createNativeQuery("SELECT AGENT_ID FROM AGENT WHERE MSISDN='" + shops.getAgentNumber() + "'")
					.getSingleResult().toString();
			...
			if (shopId.equalsIgnoreCase("0")) {
				...
				synchronized (this) {
					// agentId = "AG_" + year + "_" + appConfig.getSequenceNbr(42);
					shopId = "AG_" + year + "_" + Integer
							.parseInt(em.createNativeQuery("SELECT SHOPS FROM SEQ_TABLE").getSingleResult().toString());
					query = em.createNativeQuery("UPDATE SEQ_TABLE SET SHOPS = SHOPS + 1 ");
					query.executeUpdate();
					em.createNativeQuery("commit").executeUpdate();
				}
			}
			...
			query = em.createNativeQuery("SELECT AGENT_SHOPS_ID from AGENT_SHOPS where SHOPS_ID='" + shopId + "'");
			if (query.getSingleResult() != null) {
				agentShopID = query.getSingleResult().toString();
			} else {
				synchronized (this) {
					// agentId = "AG_" + year + "_" + appConfig.getSequenceNbr(42);
					agentShopID = "AG_SHOPS_" + year + "_" + Integer.parseInt(
							em.createNativeQuery("SELECT AGENT_SHOPS FROM SEQ_TABLE").getSingleResult().toString());
					query = em.createNativeQuery("UPDATE SEQ_TABLE SET AGENT_SHOPS = AGENT_SHOPS + 1 ");
					query.executeUpdate();
					em.createNativeQuery("commit").executeUpdate();
				}
		...
		try {
			...
			synchronized (this) {
				...
				shopimgID = "SHOP_IMG_" + year + "_" + Integer.parseInt(
						em.createNativeQuery("SELECT SHOP_IMAGE FROM SEQ_TABLE").getSingleResult().toString());
				query = em.createNativeQuery("UPDATE SEQ_TABLE SET SHOP_IMAGE = SHOP_IMAGE + 1 ");
				query.executeUpdate();
				em.createNativeQuery("commit").executeUpdate();
			}
	...
	public ResponseEntity<List<ShopImageAPI>> getShopImage(@RequestBody ShopImageAPI shopimage) {
		...
		try {
			emf = Persistence.createEntityManagerFactory("persistence");
			em = emf.createEntityManager();
			query = em.createNativeQuery("Select IMAGE_NAME FROM SHOPS_IMAGE WHERE SHOP_ID='" + shopid + "' ");
			shopimagelist = query.getResultList();
		} catch (Exception e) {
			sw = new StringWriter();
			e.printStackTrace(new PrintWriter(sw));
			exceptionAsString = sw.toString();
	...
	public ResponseEntity<String> deleteShopImage(@RequestBody ShopImageAPI image) {
		...
		try {
			...
			query = em.createNativeQuery("delete  SHOPS_IMAGE  where IMAGE_NAME ='" + path + "'");
			query.executeUpdate();
			data = "deleted";
			em.getTransaction().commit();
		} catch (Exception e) {
	...
	public ResponseEntity<String> deleteShop(@RequestBody ShopsAPI shops) {
		...
		try {
			...
			em = emf.createEntityManager();
			em.getTransaction().begin();
			ImageCounter = em.createNativeQuery("SELECT COUNT(*) from SHOPS_IMAGE where SHOP_ID='" + ShopId + "'")
					.getSingleResult().toString();
			if (ImageCounter.equalsIgnoreCase("0")) {

				query = em.createNativeQuery("delete AGENT_SHOPS where SHOPS_ID='" + ShopId + "'");
				query.executeUpdate();

				query = em.createNativeQuery("delete SHOPS where SHOPS_ID='" + ShopId + "'");
				query.executeUpdate();
	...
	public ResponseEntity<String> savingSpeedCoverageTest(@RequestBody SpeedCoverageTestAPI testingspeedcoverage) {
		...
		if (testingspeedcoverage.getAgentNumber() != null && testingspeedcoverage.getAgentNumber() != "") {
			...
			try {
				...
				synchronized (this) {
					// agentId = "AG_" + year + "_" + appConfig.getSequenceNbr(42);
					speedCoverageId = "TEST_" + year + "_" + Integer.parseInt(
							em.createNativeQuery("SELECT SPEED_COVERAGE FROM SEQ_TABLE").getSingleResult().toString());
					query = em.createNativeQuery("UPDATE SEQ_TABLE SET SPEED_COVERAGE = SPEED_COVERAGE + 1 ");
					query.executeUpdate();
					em.createNativeQuery("commit").executeUpdate();
				}
				...
				query = em.createNativeQuery(
						"insert into speed_coverage_test (SPEEDCOVERAGEID,SPEEDCOVERAGE_DATE,SPEEDCOVERAGE_LAT,SPEEDCOVERAGE_LNG,COVERAGE_SIGNAL,SPEED_DOWNLOAD,"
								...
								+ "'" + testingspeedcoverage.getAgentNumber()
								+ "',(SELECT AGENT.FULL_NAME FROM AGENT WHERE MSISDN='"
								+ testingspeedcoverage.getAgentNumber() + "')," + "'" + testingspeedcoverage.getcid()
								+ "','" + testingspeedcoverage.getlac() + "','" + testingspeedcoverage.getTechnology()
