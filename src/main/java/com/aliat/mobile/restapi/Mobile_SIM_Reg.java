			...
			try {
				...
				if (regions.size() > 0) {
					for (int i = 0; i < regions.size(); i++) {
						...
						areaID = session.createNativeQuery(
								"SELECT AREA_ID FROM AREA where REGION_NAME ='" + regions.get(i) + "'").list();
						areaName = session
								.createNativeQuery(
					...
					if (result.get(0)[1].equals('0') && result.get(0)[2].equals('0') && result.get(0)[4].equals('0')) {
						...
						synchronized (this) {
							...
							speedCoverageId = "TEST_" + year + "_"
									+ Integer.parseInt(session.createNativeQuery("SELECT SPEED_COVERAGE FROM SEQ_TABLE")
											.uniqueResult().toString());
							query = session
									.createNativeQuery("UPDATE SEQ_TABLE SET SPEED_COVERAGE = SPEED_COVERAGE + 1 ");
							query.executeUpdate();
							session.createNativeQuery("commit").executeUpdate();
						}
		...
		if (session != null && session.isOpen()) {
			...
			try {
				String ussdstatus = session
						.createNativeQuery("SELECT STATUS FROM CLIENTS where CLIENT_ID='" + clientid + "'")
						.uniqueResult().toString();
				...
				if (session.isOpen() && session != null) {
					...
					query = session
							.createNativeQuery("UPDATE CLIENTS SET LAST_MODIFIED_DATE=SYSDATE, REGISTRATION_ID ='"
									+ registartionid + "' ,RESPONSE_CODE='" + response_code + "',RESPONSE_MESSAGE='"
									+ response_message + "',REGISTRATION_STATUS='" + status + "',STATUS='"
									+ clientstatus + "' where CLIENT_ID='" + clientid + "'");
					query.executeUpdate();
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (!tkashststus.equalsIgnoreCase("Success")) {
					query = session
							.createNativeQuery("UPDATE CLIENTS SET LAST_MODIFIED_DATE=SYSDATE, TKASHREGISTER_ID ='"
									+ tkashregistartionid + "',TKASH_RESPONSE_CODE='" + response_code
									+ "',TKASH_RESPONSE_MESSAGE='" + response_message + "',TKASH_REGISTRATION_STATUS='"
									+ status + "' where CLIENT_ID='" + clientid + "'");
					query.executeUpdate();
	...
	public ResponseEntity<String> AirtimeAPIWeb(@RequestBody String rechargeData) throws Exception {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				String agentname = session
						.createNativeQuery("SELECT FULL_NAME FROM AGENT where MSISDN='" + agentNumber + "'")
						.uniqueResult().toString();
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				if (query.uniqueResult() != null) {
					...
					synchronized (this) {
						// agentId = "AG_" + year + "_" + appConfig.getSequenceNbr(42);
						agentShopID = "AG_SHOPS_" + year + "_" + Integer.parseInt(session
								.createNativeQuery("SELECT AGENT_SHOPS FROM SEQ_TABLE").uniqueResult().toString());
	...
	public ResponseEntity<String> deleteShop(@RequestBody ShopsAPI shops) {
		...
		if (session != null && session.isOpen()) {
			...
			try {
				...
				ImageCounter = session
						.createNativeQuery("SELECT COUNT(*) from SHOPS_IMAGE where SHOP_ID='" + ShopId + "'")
						.uniqueResult().toString();
